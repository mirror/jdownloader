        ...
        if (StringUtils.startsWithCaseInsensitive(ibr.getHttpConnection().getContentType(), "text/html")) {
            try {
                ...
                if (e.getLinkStatus() == LinkStatus.ERROR_PLUGIN_DEFECT) {
                    throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Cloudflare failure - see board.jdownloader.org/showthread.php?t=83712", 10 * 60 * 60 * 1000l, e);
                } else {
                    throw e;
                }
            }
