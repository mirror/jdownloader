...
public abstract class XFileSharingProBasic extends antiDDoSForHost {
    ...
    public void correctDownloadLink(final DownloadLink link) {
        final String fuid = getFUIDFromURL(link);
        final String pluginPatternMatcher = link != null ? link.getPluginPatternMatcher() : null;
        if (fuid != null && pluginPatternMatcher != null) {
            /* link cleanup, prefer https if possible */
            try {
                final URL url = new URL(pluginPatternMatcher);
                ...
                if (type != null) {
                    ...
                    case SHORT:
                         ...
                         */
                        link.setPluginPatternMatcher(URLHelper.parseLocation(new URL(protocolCorrected + urlHost), buildShortURLPath(link, fuid)));
                        break;
                    case EMBED:
                        /*
                        ...
                        link.setPluginPatternMatcher(URLHelper.parseLocation(new URL(protocolCorrected + hostCorrected), buildNormalURLPath(link, fuid)));
                        break;
                    case FILE:
                        link.setPluginPatternMatcher(URLHelper.parseLocation(new URL(protocolCorrected + hostCorrected), buildNormalFileURLPath(link, fuid)));
                        break;
                    case NORMAL:
                        link.setPluginPatternMatcher(URLHelper.parseLocation(new URL(protocolCorrected + hostCorrected), buildNormalURLPath(link, fuid)));
                        break;
                    case IMAGE:
                        link.setPluginPatternMatcher(URLHelper.parseLocation(new URL(protocolCorrected + hostCorrected), buildImageURLPath(link, fuid)));
                        break;
                    default:
                        link.setPluginPatternMatcher(URLHelper.parseLocation(new URL(protocolCorrected + hostCorrected), buildURLPath(link, fuid, type)));
                        break;
                    }
                }
    ...
    protected boolean isOffline(final DownloadLink link, final Browser br, final String correctedBR) {
        ...
        return br.getHttpConnection().getResponseCode() == 404 || new Regex(correctedBR, "(?i)(No such file|>\\s*File Not Found\\s*<|>\\s*The file was removed by|Reason for deletion:\n|File Not Found|>\\s*The file expired|>\\s*Sorry, we can't find the page you're looking for|>\\s*File could not be found due to expiration or removal by the file owner|>\\s*The file of the above link no longer exists|>\\s*video you are looking for is not found)").matches();
    }
    ...
    protected void resolveShortURL(final Browser br, final DownloadLink link, final Account account) throws Exception {
        synchronized (link) {
            if (isShortURL(link)) {
                ...
                final URLConnectionAdapter con = openAntiDDoSRequestConnection(brc, brc.createGetRequest(pluginPatternMatcher));
                if (this.looksLikeDownloadableContent(con)) {
                    /* We got a directURL */
                    return;
                }
    ...
    public String[] scanInfo(final String html, final String[] fileInfo) {
        final String urlFUID = this.getFUIDFromURL(this.getDownloadLink());
        /*
        ...
        if (StringUtils.isEmpty(fileInfo[0])) {
            ...
            if (StringUtils.isEmpty(fileInfo[0])) {
                ...
                if (StringUtils.isEmpty(fileInfo[0])) {
                    fileInfo[0] = new Regex(html, "<h2>\\s*Download File\\s*(?:<span[^>]*>)?\\s*(.+?)\\s*(</span>)?\\s*</h2>").getMatch(0);
        ...
        if (this.supports_availablecheck_filesize_html() && StringUtils.isEmpty(fileInfo[1])) {
            ...
            if (StringUtils.isEmpty(fileInfo[1])) {
                fileInfo[1] = new Regex(html, "\\(([0-9]+\\s*bytes)\\)").getMatch(0);
                if (StringUtils.isEmpty(fileInfo[1])) {
        ...
        try {
            ...
            while (true) {
                ...
                for (final DownloadLink link : links) {
                    ...
                    sb.append(URLEncode.encodeURIComponent(link.getPluginPatternMatcher()));
                    sb.append("%0A");
                }
                {
    ...
    public void doFree(final DownloadLink link, final Account account) throws Exception, PluginException {
         ...
         * download-start-procedure! --> See e.g. LinkSnappyCom --> attemptStoredDownloadurlDownload
         */
        String dllink = null;
        ...
        do {
            ...
            /**
             * Try to find a downloadlink. Check different methods sorted from "usually available" to "rarely available" (e.g. there are a
             * lot of sites which support video embedding).
             */
    ...
    protected boolean containsBlockedIPLoginMessage(final Browser br) {
        return br != null && br.containsHTML("(?i)>\\s*You can't login from this IP");
    }
    ...
    protected String getDllinkAPI(final DownloadLink link, final Account account) throws Exception {
        ...
        if (this.enableAccountApiOnlyMode() || this.allowAPIDownloadIfApikeyIsAvailable(link, account)) {
            ...
            for (final String quality : qualities) {
                ...
                if (qualityO != null) {
                    final Map<String, Object> entries_tmp = (Map<String, Object>) qualityO;
