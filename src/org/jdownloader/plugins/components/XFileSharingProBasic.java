...
public class XFileSharingProBasic extends antiDDoSForHost {
    ...
    protected void resolveShortURL(final DownloadLink link, final Account account) throws Exception {
        synchronized (link) {
            if (isShortURL(link)) {
                ...
                super.getPage(brc, pluginPatternMatcher);
    ...
    public boolean massLinkcheckerWebsite(final DownloadLink[] urls) {
        ...
        try {
            ...
            while (true) {
                ...
                for (final DownloadLink dl : links) {
                    if (massLinkcheckerParseFileInfo(br, dl) == AvailableStatus.UNCHECKED) {
    ...
    protected boolean handleHCaptcha(final DownloadLink link, final Form captchaForm) throws Exception {
    ...
    protected boolean handleRecaptchaV2(final DownloadLink link, final Form captchaForm) throws Exception {
        /*
    ...
    public void handleCaptcha(final DownloadLink link, final Form captchaForm) throws Exception {
        ...
        if (new Regex(getCorrectBR(br), "(geetest_challenge|geetest_validate|geetest_seccode)").matches()) {
            ...
            if (handleHCaptcha(link, captchaForm)) {
                link.setProperty(PROPERTY_captcha_required, Boolean.TRUE);
            }
    ...
    public boolean loginWebsite(final DownloadLink link, final Account account, final boolean validateCookies) throws Exception {
        synchronized (account) {
            ...
            try {
                ...
                final int realMaxLoginAttempts;
                ...
                do {
                    ...
                    if (!captchaRequiredInThisRun && login_counter > 1) {
                        /*
                    ...
                    } else if (userSolvedAtLeastOneLoginCaptcha && containsInvalidLoginsMessage(br)) {
                    ...
                    if (!captchaRequiredInThisRun) {
                        /* E.g. filejoker.net, filefox.cc, novafile.com */
                        logger.info("Special case: No captcha on first run but only on 2nd");
                    }
                    login_counter++;
