...
public class XFileSharingProBasic extends antiDDoSForHost {
    ...
    public boolean massLinkcheckerAPI(final DownloadLink[] urls, final Account account, final boolean setWeakFilename) {
        if (urls == null || urls.length == 0 || account == null) {
            return false;
        }
        boolean linkcheckerHasFailed = false;
        try {
    ...
    protected CaptchaHelperHostPluginRecaptchaV2 getCaptchaHelperHostPluginRecaptchaV2(PluginForHost plugin, Browser br) throws PluginException {
        return new CaptchaHelperHostPluginRecaptchaV2(this, br) {
            ...
            protected String getSiteUrl() {
                /* By default, if a DownloadLink is given, its' host will be used but we want to always use the current browser domain! */
                /*
                 * 2020-04-06 e.g. dropgalaxy.com: Main domain is dropgalaxy.in but it will redirect to dropgalaxy.com, containing the
                 * captcha --> Using DownloadLink domain or plugin domain would lead to failure!
                 */
                return br.getHost();
            }
        };
