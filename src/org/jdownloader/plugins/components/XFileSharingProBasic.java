...
public class XFileSharingProBasic extends antiDDoSForHost {
    ...
    public boolean checkLinks(final DownloadLink[] urls) {
        ...
        if (isAPIKey(apiKey) && this.supports_mass_linkcheck_over_api()) {
    ...
    public AvailableStatus requestFileInformationAuto(final DownloadLink link, final Account account, final boolean downloadsStarted) throws Exception {
        if (this.supports_single_linkcheck_over_api()) {
            ...
            String apiKey = account != null ? getAPIKeyFromAccount(account) : null;
            if (!isAPIKey(apiKey)) {
     ...
     * if o.g. officially only logged-in users can see filesize or filesize is not given in html code for whatever reason.<br />
     * Often needed for <b><u>IMAGEHOSTER</u> ' s</b>.<br />
     ...
     * TODO: Move this into a more generic class - maybe we already have something like this or something that is better. <br />
     *
     * @param filename
        ...
        if (StringUtils.isEmpty(directurl) || !directurl.startsWith("http")) {
            ...
            try {
                ...
                if (con.getResponseCode() == 503) {
                    ...
                    return directurl;
                } else if (con.getLongContentLength() > -1 && isDownloadableContent(con)) {
    ...
    protected AccountInfo fetchAccountInfoWebsite(final Account account) throws Exception {
        ...
        if (ai == null) {
            /* E.g. API failure. Initialize this and fill it with data obtained from website. */
            ai = new AccountInfo();
        }
    ...
    /**
     * Tries to find apikey on website which, if given, usually camn be found on /?op=my_account Example host which has 'API mod' installed:
     * </br> clicknupload.org </br> apikey will usually be located here: "/?op=my_account"
     */
    protected String findAPIKey(final String src) throws Exception {
        ...
        if (StringUtils.isEmpty(apikey) && generate_apikey_url != null) {
            ...
            try {
                ...
                apikey = regexAPIKey(br2.toString());
            } catch (final Throwable e) {
                e.printStackTrace();
            }
            if (apikey == null) {
    ...
    protected String regExTrafficLeft() {
        ...
        String availabletraffic = new Regex(this.correctedBR, "Traffic available(?:\\s*today)?\\s*[^<>]*:?(?:<[^>]*>)?</TD>\\s*<TD[^>]*>\\s*(?:<b[^>]*>)?\\s*([^<>\"']+)").getMatch(0);
    ...
    protected boolean isDownloadableContent(URLConnectionAdapter con) throws IOException {
        return con != null && con.isOK() && (con.isContentDisposition() || StringUtils.equalsIgnoreCase(con.getContentType(), "application/octet-stream") || !StringUtils.containsIgnoreCase(con.getContentType(), "text"));
    }
    ...
    protected final void loginAPI(final Browser br, final Account account) throws Exception {
        synchronized (account) {
            ...
            try {
                br.setCookiesExclusive(true);
                if (this.getAPIKeyFromAccount(account) == null) {
                ...
                getPage(br, this.getAPIBase() + "/account/info?key=" + getAPIKeyFromAccount(account));
     ...
     */
    public boolean massLinkcheckerAPI(final DownloadLink[] urls, final String apikey, final boolean setWeakFilename) {
    ...
    protected final String getAPIKeyFromAccount(final Account account) {
        synchronized (account) {
            final String apiKey = account.getStringProperty(PROPERTY_apikey, null);
            if (isAPIKey(apiKey)) {
     ...
     * future when we may have XFS plugin settings e.g. for video quality! TODO: Add setting for this
     */
    private final boolean internal_videohost_prefer_OfficialOriginalVideoDownload() {
        return false;
    }

    @Override
    public boolean internal_supportsMassLinkcheck() {
        return supports_single_linkcheck_over_api() || supports_mass_linkcheck_over_website();
    }
