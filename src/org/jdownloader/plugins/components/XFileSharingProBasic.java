...
public class XFileSharingProBasic extends antiDDoSForHost {
    ...
    protected void fixFilename(final URLConnectionAdapter connection, final DownloadLink link) {
        ...
        if (!StringUtils.isEmpty(servNameExt) && !StringUtils.contains(servNameExt, ".")) {
            final String mimeExt = connection != null ? DirectHTTP.getExtensionFromMimeType(connection.getContentType()) : null;
            if (mimeExt != null) {
    ...
    protected String getDllinkAPI(final DownloadLink link, final Account account) throws Exception {
         ...
         */
        if (this.enableAccountApiOnlyMode() || this.allow_api_download_if_apikey_is_available(account)) {
