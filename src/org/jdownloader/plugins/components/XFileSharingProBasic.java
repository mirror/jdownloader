    ...
    public void correctDownloadLink(final DownloadLink link) {
        ...
        if (fuid != null && pluginPatternMatcher != null) {
            ...
            try {
                ...
                if (pluginPatternMatcher.matches(TYPE_SHORTURL)) {
    ...
    protected void resolveShortURL(final DownloadLink link, final Account account) throws Exception {
        synchronized (link) {
            ...
            if (pluginPatternMatcher.matches(TYPE_SHORTURL)) {
                ...
                final Browser restore = br;
                final Browser brc = restore.cloneBrowser();
                try {
                    // TODO: some methods access br field of Plugin
                    br = brc;
                    brc.setFollowRedirects(true);
                    getPage(brc, pluginPatternMatcher);
                    if (isOffline(link)) {
                        throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
                    }
                    ...
                    if (realFUID == null || !realFUID.matches("[a-z0-9]{12}")) {
                        /**
                         * The usual XFS errors can happen here in which case we won't be able to find the long FUID. </br> Even while a
                         * limit is reached, such URLs can sometimes be checked via: "/?op=check_files" but we won't do this for now!
                         */
