...
public class XFileSharingProBasic extends antiDDoSForHost {
    ...
    protected AccountInfo fetchAccountInfoWebsite(final Account account) throws Exception {
        ...
        }
        {
             ...
             * find a key here if the user at least once pressed the "Generate API Key" button or if the XFS api mod used by the website
             * admin is configured to display apikeys by default for all users.
             */
            final Pattern apikeyPattern = Pattern.compile("/api/account/info\\?key=([a-z0-9]+)");
            String apikey = new Regex(correctedBR, apikeyPattern).getMatch(0);
            String generate_apikey_url = new Regex(correctedBR, "\"([^\"]*?op=my_account[^\"]*?generate_api_key=1[^\"]*?token=[a-f0-9]{32}[^\"]*?)\"").getMatch(0);
            /*
            ...
            if (apikey == null && generate_apikey_url != null) {
                if (Encoding.isHtmlEntityCoded(generate_apikey_url)) {
                    /*
                     * 2019-07-28: E.g. flix555.com has "&&amp;" inside URL (= buggy) - also some XFS hosts will only allow apikey
                     * generation once and when pressing "change key" afterwards, it will always be the same. This may also be a serverside
                     * XFS bug.
                     */
