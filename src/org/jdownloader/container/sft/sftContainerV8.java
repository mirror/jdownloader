...
public class sftContainerV8 extends sftContainer {
    ...
    public sftContainerV8(DelphiFormBinLoader dfm, byte[] magicDecryptionKey) throws IOException, NoSuchAlgorithmException {
        ...
        byte[] eHeader = new BASE64Decoder().decodeBuffer(sft_root.find(NODE_HEADER).getValue());
        ...
        try {
            ...
            if (decryptMatcher.matches()) {
                ...
                for (int i = node.getChildLength() - 1; i >= 0; i--) {
                    ...
                    if (bHost != null) manipulateMagicString(magicStringCopy, bHost[bHost.length - 1]);
                    if (bUsername != null) manipulateMagicString(magicStringCopy, bUsername[bUsername.length - 1]);
                    if (bPassword != null) manipulateMagicString(magicStringCopy, bPassword[bPassword.length - 1]);
                    if (bDirname != null) manipulateMagicString(magicStringCopy, bDirname[bDirname.length - 1]);
                }
                ...
                for (int i = 0; i < node.getChildLength(); ++i) {
                    ...
                    if (bDirname != null) {
                        if (!toBase64String(magicStringCopy, bDirname)) return false;
                    }
                    if (bPassword != null) {
                        if (!toBase64String(magicStringCopy, bPassword)) return false;
                    }
                    if (bUsername != null) {
                        if (!toBase64String(magicStringCopy, bUsername)) return false;
                    }
                    if (bHost != null) {
                        if (!toBase64String(magicStringCopy, bHost)) return false;
                    }

                    if (bHost != null) bHost = decodeCoreInformation(bHost);
                    if (bUsername != null) bUsername = decodeCoreInformation(bUsername);
                    if (bPassword != null) bPassword = decodeCoreInformation(bPassword);
                    if (bDirname != null) bDirname = decodeCoreInformation(bDirname);
    ...
    private boolean toBase64String(byte[] magicString, byte[] encrypted) {
        ...
        if (!isBase64(encrypted)) return false;
    ...
    private void manipulateMagicString(byte[] magicStringCopy, byte lastByte) {
        int p = ((int) lastByte & 0xFF) % magicStringCopy.length;
