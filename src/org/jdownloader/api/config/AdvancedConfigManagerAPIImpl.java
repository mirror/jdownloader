...
public class AdvancedConfigManagerAPIImpl implements AdvancedConfigManagerAPI {
	...
	public ArrayList<AdvancedConfigAPIEntry> list(String pattern,
		...
		java.util.List<AdvancedConfigEntry> entries = AdvancedConfigManager
				.getInstance().list();
		Pattern cPat = StringUtils.isEmpty(pattern) ? null : Pattern.compile(
				pattern, Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
		for (AdvancedConfigEntry entry : entries) {
			String key = entry.getKeyHandler().getStorageHandler()
					.getConfigInterface().getName()
					+ "." + entry.getKeyHandler().getKey();
			if (cPat == null || cPat.matcher(key).matches()) {
				ret.add(new AdvancedConfigAPIEntry(entry, returnDescription,
						returnValues, returnDefaultValues));
			}
		}
		return ret;
	}
	...
	private KeyHandler<Object> getKeyHandler(String interfaceName,
			String storage, String key) {
		StorageHandler<?> storageHandler = StorageHandler.getStorageHandler(
