...
public class DownloadLinkPropertiesPanel extends MigPanel implements ActionListener, GenericConfigEventListener<Boolean>, DownloadControllerListener {
    ...
    private void update() {
        ...
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_SAVE_TO_VISIBLE.isEnabled()) addSaveTo(height, p);
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_FILENAME_VISIBLE.isEnabled()) addFilename(height, p);
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_PACKAGENAME_VISIBLE.isEnabled()) addPackagename(height, p);
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_DOWNLOAD_FROM_VISIBLE.isEnabled()) addDownloadFrom(height, p);
        //
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_DOWNLOAD_PASSWORD_VISIBLE.isEnabled()) addDownloadPassword(height, p);
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_CHECKSUM_VISIBLE.isEnabled()) addChecksum(height, p);
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_COMMENT_VISIBLE.isEnabled()) addCommentLine(height, p);
        if (CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_ARCHIVEPASSWORD_VISIBLE.isEnabled()) addArchiveLine(height, p);
    ...
    public void update(final FilePackage pkg) {
        if (!isVisible()) return;
        if (saving) return;
        ...
        new Thread("PropertiesPanelUpdater") {
            public void run() {
                ...
                final List<Archive> archives = ArchiveValidator.validate(new SelectionInfo<FilePackage, DownloadLink>(pkg, null, null, null, null, null)).getArchives();
    ...
    public void update(final DownloadLink link) {
        if (!isVisible()) return;
        if (saving) return;
        ...
        new Thread("PropertiesPanelUpdater") {
            public void run() {
                ...
                final List<Archive> archives = ArchiveValidator.validate(new SelectionInfo<FilePackage, DownloadLink>(link, null, null, null, null, null)).getArchives();
    ...
    protected void updateInEDT(final DownloadLink link, final FilePackage pkg) {
        if (pkg == null) {
        ...
        priority.setSelectedItem(link.getPriorityEnum());

    }

    protected void updateDownloadFrom(final DownloadLink link) {
        boolean ld = setting;
        setting = true;
        try {
    ...
    public void fillPopup(JPopupMenu pu) {
        ...
        pu.add(new CheckboxMenuItem(_GUI._.LinkgrabberPropertiesHeader_comment(), CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_COMMENT_VISIBLE));
        pu.add(new CheckboxMenuItem(_GUI._.LinkgrabberPropertiesHeader_archivepassword(), CFG_GUI.DOWNLOADS_PROPERTIES_PANEL_ARCHIVEPASSWORD_VISIBLE));
    }
