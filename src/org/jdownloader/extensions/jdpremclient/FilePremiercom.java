...
public class FilePremiercom extends PluginForHost implements JDPremInterface {
    ...
    private boolean handleFilePremier(DownloadLink link) throws Exception {
        ...
        synchronized (LOCK) {
            ...
            acc = AccountController.getInstance().getValidAccount("filepremier.com");
    ...
    public int getMaxSimultanDownload(final Account account) {
        if (plugin != null) {
            if (PremiumCompoundExtension.preferLocalAccounts() && account != null) {
                ...
                /* FilePremier */
                synchronized (LOCK) {
                    if (premiumHosts.contains(plugin.getHost()) && AccountController.getInstance().getValidAccount("filepremier.com") != null) return Integer.MAX_VALUE;
                }
            }
            return plugin.getMaxSimultanDownload(account);
        }
