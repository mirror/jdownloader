...
public class NeembuuGui extends AddonPanel<NeembuuExtension> {
	...
	public NeembuuGui(NeembuuExtension plg) {
		super(plg);
		this.panel = new SwitchPanel(new MigLayout("ins 0,wrap 1",
				"[grow,fill]", "[grow,fill][]")) {
	...
	public void addSession(JDDownloadSession jdds) {
		tabbedPane.add(jdds.getDownloadLink().getBrowserUrl(), jdds
				.getWatchAsYouDownloadSession().getFilePanel());
		/*tabbedPane.setTabComponentAt(
				tabbedPane.indexOfTab(jdds.getDownloadLink().getBrowserUrl()),
				new ButtonTabComponent(tabbedPane, jdds, this));*/
	}

	public void removeSession(JDDownloadSession jdds) {
		tabbedPane.removeTabAt(tabbedPane.indexOfTab(jdds.getDownloadLink()
				.getBrowserUrl()));
	}
	...
	private void layoutPanel() {
		...
		settingsPanel.add(new JLabel(
				"TODO:A few basic settings to be added here."));
