...
public class JDPremServServer implements ControlListener {
    ...
    // password.length() == 0 ||
    // !UserController.getInstance().isUserAllowed(username, password)) {
    ...
    // if (AccountController.getInstance().getValidAccount(plugin.getHost()) !=
    // null) {
    ...
    // DownloadLink ret =
    // JDPremServController.getInstance().getDownloadLink(Encoding.urlDecode(wantedUrl,
    // true));
    ...
    // return;
    // }
    // if (!UserController.getInstance().isUserAllowed(username, password,
    // ret.getHost())) {
    ...
    // !ret.getLinkStatus().isPluginActive()) {
    // /* finished? */
    ...
    // response.setFileServe(ret.getFileOutput(), 0, -1, new
    // File(ret.getFileOutput()).length(), false);
    // } else {
    // String[] dat = new Regex(request.getHeader("range"),
    // "bytes=(\\d+)-(\\d+)?").getRow(0);
    // if (dat[1] == null) {
    // response.setFileServe(ret.getFileOutput(), Long.parseLong(dat[0]), -1,
    // new File(ret.getFileOutput()).length(), true);
    // } else {
    // response.setFileServe(ret.getFileOutput(), Long.parseLong(dat[0]),
    // Long.parseLong(dat[1]), new File(ret.getFileOutput()).length(), true);
    // }
    // }
    // } else {
    // /* request status info */
    ...
    // /* ERROR: -50 = failed */
    // response.setReturnStatus(Response.ERROR);
    // response.addContent("ERROR: -50 || " +
    // ret.getLinkStatus().getStatusString());
    // } else {
    // /* not finished? */
    ...
    // } else {
    // synchronized (LOCK) {
    // ArrayList<DownloadLink> userSt =
    // startedLinks.get(username.toLowerCase());
    // if (userSt == null) {
    ...
    // if (res.getAdditionalData() != null && res.getAdditionalData() instanceof
    // DownloadLink) {
    // link = (DownloadLink) res.getAdditionalData();
    // JDPremServController.getInstance().removeRequestedDownload(link);
    // }
    // if (username != null && password != null && res.getAdditionalData() !=
    // null && res.getAdditionalData() instanceof DownloadLink) {
    // PremServUser user =
    // UserController.getInstance().getUserByUserName(username);
    // if (user != null) user.addTrafficLog(link.getHost(), Math.max(0,
    // res.getFileBytesServed()));
    // }
    //
    // }
    ...
    public void controlEvent(ControlEvent event) {
        ...
        case ControlEvent.CONTROL_DOWNLOAD_FINISHED:
            if (event.getCaller() instanceof SingleDownloadController) {
                DownloadLink lastDownloadFinished = (DownloadLink) event.getParameter();
                synchronized (LOCK) {
