...
public class SocketClient extends Client {
    ...
    public Captcha upload(byte[] img, String challenge, int type, byte[] banner, String banner_text, String grid) throws IOException, Exception {
        ...
        try {
            args.put("captcha", Base64.encodeToByte(img, false)).put("swid", Client.SOFTWARE_VENDOR_ID).put("challenge", challenge).put("grid", grid).put("type", Integer.toString(type)).put("banner_text", banner_text);
            if (banner != null) {
                args.put("banner", Base64.encodeToByte(banner, false));
            }
        } catch (JSONException e) {
            // System.out.println(e);
        }
