...
public class CaptchaResolutorCaptchaSolver extends ChallengeSolver<String> {
	...
	@Override
	public void solve(final SolverJob<String> job) throws InterruptedException,	SolverException, IOException {
		String ret = "captcha-null";
		String address[] = ((String) InetAddress.getByName(SERVER_PRODUCTION).toString()).split(_ADDRESS_SEPARATOR);
		if (StringUtils.isEmpty(config.getUser())|| StringUtils.isEmpty(config.getPass()))
			return;
