...
public class BytesMappedFile implements FileBytesCacheFlusher {
    ...
    public BytesMappedFile(File file) {
        this(file, new FileBytesMap());
    }
    
    public BytesMappedFile(File file, FileBytesMap fileBytesMap) {
        this.file = file;
    ...
    public synchronized void flushed() {
        if (raf != null && flushFlag) {
            try {
                ...
                if (this.ioException != null) this.ioException = e;
            } finally {
                flushFlag = false;
                for (BytesMappedFileCallback callback : callBacks) {
                    if (callback != null) callback.onFlush(this, this.ioException);
                }
            }
        }
    }
