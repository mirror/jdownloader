...
public class MySpaceCom extends PluginForDecrypt {
    ...
    //private static final Pattern PATTERN_SUPPORTED 			= Pattern.compile("http://[\\w\\.]*?myspace\\.(com|de)/index\\.cfm\\?fuseaction=user\\.viewprofile&friendid=(\\d+)", Pattern.CASE_INSENSITIVE);
    ...
    //private static final String DOWNLOAD_INFORMATIONS = "<song bsid=\"\\d+\" title=\"("+ANY_ATTRIBUTE+")\" songid=\"\\d+\" plays=\"\\d+\" comments=\""+ANY_ATTRIBUTE+"\" rate=\""+ANY_ATTRIBUTE+"\" downloadable=\"("+ANY_URL+")\" imagename=\""+ANY_URL+"\" imagedesc=\""+ANY_ATTRIBUTE+"\" filename=\""+ANY_ATTRIBUTE+"\" url=\""+ANY_URL+"\" durl=\"("+ANY_URL+")\" token=\""+ANY_ATTRIBUTE+"\" curl=\""+ANY_URL+"\"/>";
    ...
    //Pattern.compile("<title>$\\s+?^\\s+?.+?www.myspace.com/(.+)$",Pattern.MULTILINE);
    //private static final String  MYSPACEURL 	 = "<td><div align=\"left\">&nbsp;&nbsp;<span class=\"searchMonkey-displayURL\">http://www.myspace.com/.+?</span>&nbsp;&nbsp;</div></td>";
    ...
    private static final String  FLASH_PLAYER_MUSICPLAYLIST_US 		
    	= "http://www\\.musicplaylist\\.us/loadplaylist\\.php\\?playlist=\\d+";
    
    private static final String FLASH_PLAYER_MP3_ASSET_COM 
    	="(?s)http://www\\.mp3asset\\.com/swf/mp3/myflashfetish-mp3-player\\.swf.+?<param name=\"flashvars\" value=\"myid=\\d+&path=\\d+/\\d+/\\d+&";
    
    
    private static final Pattern PATTERN_PAGE_INFOS 				
    	= Pattern.compile("("+NICK_NAME+"|"+FLASH_PLAYER_MINI+"|"+FLASH_PLAYER_MUSIC+"|"+FLASH_PLAYER_MUSICPLAYLIST_US+"|"+FLASH_PLAYER_MP3_ASSET_COM+")",Pattern.CASE_INSENSITIVE|Pattern.MULTILINE);
    ...
    //Keys für die Config
    /** Configuarations-Propperty: Gibt an ob alle Downloads in downloads/myspace.com/ gespeichert werden sollen */
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink cryptedLink) throws Exception {
    	...
    	Regex mainPageInfos = new Regex(br.getPage(cryptedLink.getCryptedUrl()),PATTERN_PAGE_INFOS);
    	String[][] matches = mainPageInfos.getMatches();
    	String nick = "";
    	...
    	else if(playerUrl.matches(FLASH_PLAYER_MINI)){
    		logger.info("myspace-mini-player gefunden");
    		String strUserId = new Regex(playerUrl,"\\&o=(.+?)(\\&|\")").getMatch(0);
    		String[][] data = parseXmlStandartPlayer(Encoding.Base64Decode(strUserId));
    		titel = data[0];
    		dUrls = data[1];
    	}
    	else if(playerUrl.matches(FLASH_PLAYER_MUSIC)){
    		logger.info("myspace-music-player gefunden");
    		String strUserId = new Regex(playerUrl,"\\&d=(.+?)(\\&|\")").getMatch(0);
    		String[][] data = parseXmlStandartPlayer(new Regex(Encoding.Base64Decode(strUserId),"(\\d+)?\\^").getMatch(0));
    		titel = data[0];
    		dUrls = data[1];
    	}
    	else if(playerUrl.matches(FLASH_PLAYER_MUSICPLAYLIST_US)){
    		logger.info("musicplaylist.us-player gefunden");
    		String[][] data = parseXmlMusicPlayListUs(playerUrl);
    		titel = data[0];
    		dUrls = data[1];
    	}
    	else if(playerUrl.matches(FLASH_PLAYER_MP3_ASSET_COM)){
    		logger.info("mp3asset.com-player gefunden");
    		Regex reg = new Regex(playerUrl, FLASH_PLAYER_MP3_ASSET_COM_UID);
    	...
    	assert titel.length==dUrls.length:"Fehler Anzahl-Titel und Anzahl-Downloads stimmen nicht überein!";
    	//Erstelle DownloadLinks
    	for(int i=0;i<titel.length;i++){
    		String einTitel = titel[i];
    		String link = dUrls[i];
            DownloadLink dl_link = createDownloadlink("httpmyspace://"+link);
            logger.info(einTitel + " ---> "+ link);
            
            if(getPluginConfig().getBooleanProperty(ENABLE_SUBFOLDERS1)){
            	dl_link.addSubdirectory("myspace.de");
            }
            ...
            if(getPluginConfig().getBooleanProperty(ENABLE_DL_NAME_MODIFICATION)){
            	dl_link.setStaticFileName(new StringBuilder(nick).append(" - ").append(einTitel).append(".mp3").toString());
            }else{
            ...
            filePackage.setName("myspace - " + nick);
    ...
    private String[][] parseXmlStandartPlayer(String userId) throws IOException{
    	...
    	System.out.println("http://www.myspace.com//services/media/musicplayerxml.ashx?b="+userId);
    	String[][] ret = new String[2][];
    	//Titel
    	ret[0] = new Regex(page, TITEL).getColumn(0);
    	//Durls
    	ret[1] = new Regex(page, DURLS).getColumn(0);
    	return ret;
    }
    private String[][] parseXmlMusicPlayListUs(String url)throws IOException{
    	...
    	String[][] ret = new String[2][];
    	//Titel
    	ret[0] = new Regex(page, TITEL_MUSICPLAYLIST_US).getColumn(0);
    	//Durls
    	ret[1] = new Regex(page, DURLS_MUSICPLAYLIST_US).getColumn(0);
    	return ret;
    }
    private String[][] parseXmlMusicPlayerMp3Asset(String path, String userid) throws IOException{
    	String[][] ret = new String[2][];
    	StringBuilder sb = new StringBuilder("http://www.mp3asset.com/xml/");
    	//"http://www.mp3asset.com/xml/"+path+"/"+userid+".xml"
    	sb.append(path); sb.append("/"); sb.append(userid); sb.append(".xml");
    	String page = br.getPage(sb.toString());
    	//Titel
    	ret[0] = new Regex(page, TITEL_MP3_ASSEST).getColumn(0);
    	//Durls
    	ret[1] = new Regex(page, DURLS_MP3_ASSEST).getColumn(0);
    	return ret;
    }
    @Override
