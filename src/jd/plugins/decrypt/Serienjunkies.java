...
public class Serienjunkies extends PluginForDecrypt {
    ...
    private Pattern patternSupported = Pattern
            .compile("http://serienjunkies.safehost.be/sa.e/rc[/|.|a-zA-Z0-9|_|-]*");
    private Pattern Formp = Pattern
            .compile(
    ...
    public Vector<String> decryptLink(String cryptedLink) {
        ...
        try {
            ...
            firePluginEvent(new PluginEvent(this,
                    PluginEvent.PLUGIN_PROGRESS_MAX, 1));
            String html=getRequest(new URL(cryptedLink))
            .getHtmlCode();
            String link="http://rapidshare.com/files/"
                + getFirstMatch(html, Formp, 1);
            decryptedLinks
                    .add(link);
            firePluginEvent(new PluginEvent(this,
