...
public class Serienjunkies extends PluginForDecrypt {
	...
	private String[] mirrorManagement = new String[] {
			JDLocale.L("plugins.decrypt.serienjunkies.usePremiumLinks",
					"use premiumlinks if possible"),
			JDLocale.L(
					"plugins.decrypt.serienjunkies.automaticMirrorManagment",
					"automatic mirror managment"),
			JDLocale.L("plugins.decrypt.serienjunkies.noMirrorManagment",
					"no mirror managment") };
	...
	public synchronized boolean canHandle(String data) {
		...
		if (data.contains("serienjunkies.org")
				&& (data.contains("/?cat=") || data.contains("/?p="))) {
			cat = getSerienJunkiesCat(data.contains("/?p=")) != sCatNoThing;
		}
		boolean rscom = (Boolean) getPluginConfig().getProperty(
				"USE_RAPIDSHARE_V2", true);
		boolean rsde = (Boolean) getPluginConfig().getProperty(
				"USE_RAPIDSHAREDE_V2", true);
		boolean net = (Boolean) getPluginConfig().getProperty("USE_NETLOAD_V2",
				true);
		boolean uploaded = (Boolean) getPluginConfig().getProperty(
				"USE_UPLOADED_V2", true);
		// boolean simpleupload = (Boolean) getPluginConfig().getProperty(
		// "USE_SIMLEUPLOAD_V2", true);
		boolean simpleupload = false;
		boolean filefactory = (Boolean) getPluginConfig().getProperty(
				"USE_FILEFACTORY_V2", true);
		next = false;
		String hosterStr = "";
		...
		if (matcher.find()) {
			...
			String[] links = new Regex(data,
					"http://[\\w\\.]{3,10}\\.serienjunkies.org/.*",
					Pattern.CASE_INSENSITIVE).getColumn(-1);
			Pattern pat = Pattern
					.compile(
							"http://[\\w\\.]{3,10}\\.serienjunkies.org/.*(rc[\\_\\-]|rs[\\_\\-]|nl[\\_\\-]|ut[\\_\\-]|su[\\_\\-]|ff[\\_\\-]|cat\\=[\\d]+|p\\=[\\d]+).*",
	...
	private DownloadLink createdl(String parameter, String[] info) {
		...
		DownloadLink dlink = new DownloadLink(null, name, getHost(), "sjdp://"
				+ hostname + "/" + linkName, true);
	...
	public String cutMatches(String data) {
		return data
				.replaceAll(
						"(?i)http://[\\w\\.]*?(serienjunkies\\.org|85\\.17\\.177\\.195|serienjunki\\.es).*",
						"--CUT--");
	}
	...
	public ArrayList<DownloadLink> decryptIt(String parameter) throws Exception {
				...
				if (isP) {
					...
					try {
						...
						for (int i = 2; i < pages + 1; i++) {
							htmlcode += "\n"
									+ br
											.getPage("http://serienjunkies.org/?cat="
													+ cat + "&paged=" + i);
						}
					} catch (Exception e) {
						// TODO: handle exception
		...
		if (info == null) {
			br.getPage("http://serienjunkies.org/?s="
	...
	public String[] getDecryptableLinks(String data) {
		String[] links = new Regex(
				data,
				"http://[\\w\\.]*?(serienjunkies\\.org|85\\.17\\.177\\.195|serienjunki\\.es)[^\"]*",
				Pattern.CASE_INSENSITIVE).getColumn(-1);
	...
	public String getVersion() {
		String ret = new Regex("$Revision$",
				"\\$Revision: ([\\d]*?) \\$").getMatch(0);
		...
		new Dialog(((SimpleGUI) JDUtilities.getGUI()).getFrame()) {
			...
			void init() {
				...
				if (isP) {
					...
					meths[1] = new meth(
							"Alle Serien in dieser Staffel hinzufügen",
							sCatGrabb);
				} else {
					meths = new meth[3];
					meths[0] = new meth("Kategorie nicht hinzufügen",
							sCatNoThing);
					meths[1] = new meth(
	...
	private void setConfigElements() {
		...
		config.addEntry(cfg = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
				getPluginConfig(), "USE_RAPIDSHARE_V2", "Rapidshare.com"));
		cfg.setDefaultValue(true);
		config.addEntry(cfg = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
				getPluginConfig(), "USE_RAPIDSHAREDE_V2", "Rapidshare.de"));
		cfg.setDefaultValue(true);
		config.addEntry(cfg = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
				getPluginConfig(), "USE_NETLOAD_V2", "Netload.in"));
		cfg.setDefaultValue(true);
		config.addEntry(cfg = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
				getPluginConfig(), "USE_UPLOADED_V2", "Uploaded.to"));
		cfg.setDefaultValue(true);
		// config.addEntry(cfg = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
		// getPluginConfig(), "USE_SIMLEUPLOAD_V2", "SimpleUpload.net"));
		// cfg.setDefaultValue(true);
		config.addEntry(cfg = new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
				getPluginConfig(), "USE_FILEFACTORY_V2", "FileFactory.com"));
		cfg.setDefaultValue(true);
	}
