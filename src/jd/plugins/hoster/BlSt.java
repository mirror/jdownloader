...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "bl.st" }, urls = { "https?://(www\\.)?bl\\.st/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class BlSt extends PluginForHost {

    private String                         correctedBR                  = "";
    private String                         passCode                     = null;
    private static final String            PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    /* primary website url, take note of redirects */
    private static final String            COOKIE_HOST                  = "http://bl.st";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(bl\\.st)";
    ...
    // mods: requestFileInformation[Added another FNF text], checkErrors[Added another premiumonly text], heavily modified, DO NOT UPGRADE!
    ...
    public BlSt(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "class=\"btn btn-lg btn-success\">(\\d+)</span>").getMatch(0);
        }
        if (ttt != null) {
            ...
            try {
                ...
                if (br.containsHTML("No htmlCode read")) {
                    if ("de".equalsIgnoreCase(System.getProperty("user.language"))) {
                        throw new PluginException(LinkStatus.ERROR_PREMIUM, "\r\nServerfehler, bitte kontaktiere den bl.st Support!", PluginException.VALUE_ID_PREMIUM_DISABLE);
                    } else {
                        throw new PluginException(LinkStatus.ERROR_PREMIUM, "\r\nServer error, please contact the bl.st support!", PluginException.VALUE_ID_PREMIUM_DISABLE);
                    }
                }
        ...
        if (account.getBooleanProperty("nopremium")) {
            ...
            if (dllink == null) {
                ...
                for (int i = 1; i <= 3; i++) {
                    ...
                    if (redirect != null && redirect.matches("https?://(www\\.)?bl\\.st/[a-z0-9]{12}")) {
                        getPage(redirect);
                    } else {
                        break;
                    }
