...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "gaiafile.com" }, urls = { "http://(www\\.)?gaiafile\\.com/[a-z0-9]{12}" }, flags = { 2 })
public class GaiaFileCom extends PluginForHost {

    public GaiaFileCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private static final String COOKIE_HOST   = "http://gaiafile.com";
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        doSomething();
        if (BRBEFORE.contains("<h3>The file was removed by administrator</h3>") || BRBEFORE.contains("No such file") || BRBEFORE.contains("No such user exist") || BRBEFORE.contains("File not found") || BRBEFORE.contains(">File Not Found<")) {
        ...
        if (filesize == null) {
            ...
            if (filesize == null) {
                ...
                if (filesize == null) filesize = new Regex(BRBEFORE, "<strong>Size:</strong>(.*?)</p>").getMatch(0);
            }
        }
    ...
    public AccountInfo fetchAccountInfo(Account account) throws Exception {
        ...
        String space = br.getRegex(Pattern.compile(">Used space:</td><td colspan=\\d+><b>(.*?)</b>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE)).getMatch(0);
        ...
        String availabletraffic = new Regex(BRBEFORE, ">Traffic available today:</td><td colspan=\\d+><b>(.*?)</b>").getMatch(0);
    ...
    public void handlePremium(DownloadLink link, Account account) throws Exception {
        ...
        if (NOPREMIUM) {
            ...
            logger.info("Final downloadlink = " + dllink + " starting the download...");
            // Limits couldn't be tested because downloadlinks always timed out
            // (also in browser)
