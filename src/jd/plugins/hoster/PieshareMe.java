...
public class PieshareMe extends PluginForHost {
    public PieshareMe(PluginWrapper wrapper) {
        super(wrapper);
        // this.enablePremium("");
    }
    ...
    public String getAGBLink() {
        return "https://pieshare.me/terms.html";
    }

    private static List<String[]> getPluginDomains() {
        final List<String[]> ret = new ArrayList<String[]>();
        // each entry in List<String[]> will result in one PluginForHost, Plugin.getHost() will return String[0]->main domain
        ret.add(new String[] { "pieshare.me" });
        return ret;
    }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        String filename = br.getRegex("<title>([^<>\"]+) - PieShare</title>").getMatch(0);
        if (filename == null) {
            /* Fallback1: Try to obtain filename from URL */
            filename = new Regex(link.getPluginPatternMatcher(), this.getSupportedLinks()).getMatch(2);
        }
        String filesize = br.getRegex("<strong>Size</strong>\\s*:([^<>\"]+)</p>").getMatch(0);
        if (filename != null) {
    ...
    private void doFree(final DownloadLink link, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            if (true) {
                /* 2021-04-01: Free download broken -> All files are premiumonly (?) */
