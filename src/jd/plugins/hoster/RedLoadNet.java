...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "redload.net" }, urls = { "https?://(www\\.)?redload\\.net/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class RedLoadNet extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://redload.net";
    // domain names used within download links.
    private final String               DOMAINS                      = "(redload\\.net)";
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        if (space == null || space.length == 0) {
            space = cbr.getRegex("Used space.*?([0-9\\.]+) of [0-9\\.]+ ?(KB|MB|GB|TB)?\\s*</").getRow(0);
        }
