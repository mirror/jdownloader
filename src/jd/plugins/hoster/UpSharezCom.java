...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "upsharez.com" }, urls = { "http://(www\\.)?upsharez\\.com/(?!faq|register|login|terms|report_file)[a-z0-9]+" }, flags = { 2 })
public class UpSharezCom extends PluginForHost {

    public UpSharezCom(PluginWrapper wrapper) {
        super(wrapper);
        this.enablePremium("http://upsharez.com/register.html");
    }

    // DTemplate Version 0.1.6-psp
    // mods: added some errorhandling
    // non account: 1 * 2
    ...
    private final String        MAINPAGE               = "http://upsharez.com";
    private final String        TYPE                   = "html";
    private final boolean       RESUME                 = true;
    private final int           MAXCHUNKS              = 1;
    private static final String SIMULTANDOWNLOADSLIMIT = "e=You+have+reached+the+maximum+concurrent+downloads";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (br.getURL().contains(SIMULTANDOWNLOADSLIMIT)) {
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        ...
        if (br.getURL().contains(SIMULTANDOWNLOADSLIMIT)) throw new PluginException(LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE, "Simultan downloads limit reached!", 5 * 60 * 1000l);
        ...
        if (!dl.getConnection().isContentDisposition()) {
            ...
            if (br.containsHTML(">You have reached the maximum concurrent downloads")) throw new PluginException(LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE, "Simultan downloads limit reached!", 5 * 60 * 1000l);
