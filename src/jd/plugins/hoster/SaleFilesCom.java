...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "salefiles.com" }, urls = { "https?://(www\\.)?salefiles\\.com/(?:embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class SaleFilesCom extends PluginForHost {
     ...
     * mods: login[Added errorhandling for banned accounts]<br />
     * limit-info:<br />
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                if (correctedBR.contains(">Your account was banned by administrator")) {
                    if ("de".equalsIgnoreCase(System.getProperty("user.language"))) {
                        throw new PluginException(LinkStatus.ERROR_PREMIUM, "\r\nDein Account wurde vom Administrator gesperrt!", PluginException.VALUE_ID_PREMIUM_DISABLE);
                    } else {
                        throw new PluginException(LinkStatus.ERROR_PREMIUM, "\r\n>Your account was banned by an administrator!", PluginException.VALUE_ID_PREMIUM_DISABLE);
                    }
                } else if (br.getCookie(COOKIE_HOST, "login") == null || br.getCookie(COOKIE_HOST, "xfss") == null) {
                    if ("de".equalsIgnoreCase(System.getProperty("user.language"))) {
