...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "stiahni.si" }, urls = { "http://(www\\.)?stiahni\\.si/download\\.php\\?id=\\d+" }, flags = { 0 })
public class StiahniSi extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        String filename = br.getRegex(">Stiahnuť: <a href=\"#\">([^<>\"]*?)</a>").getMatch(0);
        String filesize = br.getRegex("class=\"name\">Veľkosť</td>[\t\n\r ]+<td class=\"value\">([^<>\"]*?)</td>").getMatch(0);
