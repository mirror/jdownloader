...
@HostPlugin(revision = "$Revision: 19032 $", interfaceVersion = 3, names = { "unrestrict.li" }, urls = { "http://\\w+\\.(unrestrict|unr)\\.li/dl/\\w+/.+" }, flags = { 2 })
public class UnrestrictLi extends PluginForHost {
	...
	@Override
	public AvailableStatus requestFileInformation(DownloadLink dl)
	...
	public void handleMultiHost(DownloadLink link, Account acc)
			throws Exception {
		setBrowser();
		login(acc, false);
		...
		br.postPage(
				"http://unrestrict.li/unrestrict.php",
				"jdownloader=1&domain=long&link="
						+ Encoding.urlEncode(link.getDownloadURL())
						+ (link.getStringProperty("pass", null) != null ? "&download_password="
								+ Encoding.urlEncode(link.getStringProperty(
										"pass", null))
								: ""));
		...
		if (br.containsHTML("invalid\":\"Error receiving page")) {
			...
			String msg = "(" + link.getLinkStatus().getRetryCount() + 1 + "/"
					+ 3 + ")";
			throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE,
					"Error fetching file information:" + msg, 20 * 1000l);
		}
	...
	private void handleDL(DownloadLink link, String generated) throws Exception {
		...
		dl = jd.plugins.BrowserAdapter.openDownload(br, link, generated, true,
				0);
	...
	private void removeHostFromMultiHost(DownloadLink link, Account acc)
			throws PluginException {
		Object supportedHosts = acc.getAccountInfo().getProperty(
				"multiHostSupport", null);
	...
	public AccountInfo fetchAccountInfo(Account account) throws Exception {
		...
		try {
			String apihosts = br.cloneBrowser().getPage(
					"http://unrestrict.li/api/jdownloader/hosts.php");
			String[] hosts = new Regex(apihosts, "<host>(.*?)</host>")
					.getColumn(0);
			...
			logger.info("Failed to load Unrestrict.li hosts list. Error:"
					+ e.toString());
		}
		return ai;
	}
				...
				if (ret != null && ret instanceof HashMap<?, ?> && !force) {
					...
					if (account.isValid()) {
						for (final Map.Entry<String, String> cookieEntry : cookies
							...
							this.br.setCookie("http://unrestrict.li", key,
									value);
						}
						return;
					}
