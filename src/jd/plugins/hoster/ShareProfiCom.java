...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "shareprofi.com" }, urls = { "https?://(www\\.)?shareprofi\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class ShareProfiCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://shareprofi.com";
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String space[] = cbr.getRegex(">Used space:</span>[\r\n\t ]+<span[^>]+>([0-9\\.]+) ?(KB|MB|GB|TB)?").getRow(0);
        ...
        String availabletraffic = cbr.getRegex("Traffic available:</span>[\r\n\t ]+<span[^>]+>([^<>\"']+)").getMatch(0);
