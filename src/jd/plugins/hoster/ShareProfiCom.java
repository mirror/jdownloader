...
@HostPlugin(revision = "$Revision: 19496 $", interfaceVersion = 2, names = { "shareprofi.com" }, urls = { "https?://(www\\.)?shareprofi\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class ShareProfiCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private static final String        COOKIE_HOST                  = "http://shareprofi.com";
    // domain names used within download links.
    private static final String        DOMAINS                      = "(shareprofi\\.com)";
     ...
     * */
    public ShareProfiCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            ...
            if (fileInfo[0] == null) {
                fileInfo[0] = new Regex(br, "(type=\"hidden\")? name=\"fname\" value=\"(.*?)\"").getMatch(1);
                if (fileInfo[0] == null) {
                    fileInfo[0] = new Regex(correctedBR, "<h1>Download File (.*?)</h1>").getMatch(0);
                    if (fileInfo[0] == null) {
                        // can cause new line finds, so check if it matches.
                        // fileInfo[0] = new Regex(correctedBR, "Download File:? ?(<[^>]+> ?)+?([^<>\"\\']+)").getMatch(1);
                        // traits from download1 page below.
                        if (fileInfo[0] == null) {
                            fileInfo[0] = new Regex(correctedBR, "Filename:</b></td>[\r\n\t ]+<td[^>]+>([^<>\"\\']+)").getMatch(0);
    ...
    private void getDllink() {
        ...
        if (dllink != null) dllink = dllink.replaceFirst(":////", "://");
    }
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String space[] = new Regex(correctedBR, ">Used space:</span>[\r\n\t ]+<span[^>]+>([0-9\\.]+) ?(KB|MB|GB|TB)?").getRow(0);
        ...
        final String availabletraffic = new Regex(correctedBR, "Traffic available:</span>[\r\n\t ]+<span[^>]+>([^<>\"\\']+)").getMatch(0);
        ...
        if (account.getBooleanProperty("nopremium")) {
            ...
            final String expireDay = new Regex(correctedBR, "(\\d{4}\\.\\d{1,2}\\.\\d{1,2} \\d{1,2}:\\d{1,2})").getMatch(0);
            if (expireDay != null) {
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                if (!new Regex(correctedBR, "(Premium(\\-| )Account expire|>Renew premium<|>Premium expire:</span>)").matches()) {
    ...
    public void correctDownloadLink(DownloadLink link) {
        ...
        // does the site enforce the user of https?
        if (enforcesHTTPS) {
