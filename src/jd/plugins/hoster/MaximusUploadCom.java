...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "maximusupload.com" }, urls = { "https?://(www\\.)?maximusupload\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class MaximusUploadCom extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://maximusupload.com";
    ...
    // mods:most of their errormessages are in spanish and can't be
    // ->Changed limit and "only for premium" handling, everything else
    // is still template!
    // non account: 1 * 1
    ...
    public MaximusUploadCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public void checkErrors(DownloadLink theLink, boolean checkAll, String passCode) throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(You have reached the download(\\-| )limit|You have to wait|para baixar o proximo arquivo<)").matches()) {
            // adjust this regex to catch the wait time string for COOKIE_HOST
            String WAIT = new Regex(correctedBR, "((You have reached the download(\\-| )limit|You have to wait)[^<>]+)").getMatch(0);
            String tmphrs = new Regex(WAIT, "\\s+(\\d+)\\s+hours?").getMatch(0);
            if (tmphrs == null) tmphrs = new Regex(correctedBR, "Espere.*?\\s+(\\d+)\\s+hours?").getMatch(0);
            String tmpmin = new Regex(WAIT, "\\s+(\\d+)\\s+minutes?").getMatch(0);
            if (tmpmin == null) tmpmin = new Regex(correctedBR, "Espere.*?\\s+(\\d+)\\s+minutes?").getMatch(0);
            String tmpsec = new Regex(WAIT, "\\s+(\\d+)\\s+seconds?").getMatch(0);
            if (tmpsec == null) tmpsec = new Regex(correctedBR, "\\s+(\\d+)\\s+seconds?").getMatch(0);
