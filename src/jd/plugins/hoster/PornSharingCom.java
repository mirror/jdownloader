...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "pornsharing.com" }, urls = { "http://(www\\.)?pornsharing\\.com/[a-z0-9\\-_]+" }, flags = { 0 })
public class PornSharingCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (DLLINK == null) {
            br.getPage("http://pornsharing.com/videoplayer/nvplaylist_ps_beta.php?hq=1&id=" + lid);
            final String decrypted = decryptRC4HexString("TubeContext@Player", br.toString().trim());
            DLLINK = new Regex(decrypted, "\"fileUrl\":\"(http:[^<>\"]*?)\"").getMatch(0);
            if (DLLINK == null) {
