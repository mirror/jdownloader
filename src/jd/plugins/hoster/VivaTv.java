...
public class VivaTv extends PluginForHost {
    ...
    /* Important data - each contains one "%s" where the mgid belongs */
    ...
    public static final String   url_service_feed_NICK_DE                            = "http://feeds.mtvnservices.com/od/feed/intl-mrss-player-feed?language=de-DE&countryName=de&mgid={uri}&accountOverride=intl.mtvi.com&arcEp=nick.de&imageEp=nick.de&clusterName=SWEMEA&language=de-DE&mgid=%s&accountOverride=intl.mtvi.com&stage=live";
     ...
     * Sites that did not work serverside: nick.com <br />
     ...
     */
    /**
     * About feeds: Feeds are the common way to get the video urls. Each MTV site has different feed-URLs. The easiest way to find them is
    ...
    public static final String   PROPERTY_original_host                              = "original_host";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        final String original_host = link.getStringProperty(PROPERTY_original_host, null);
        String ext = null;
        ...
        this.feed_url = mgidGetFeedurlForMgid(this.mgid, original_host);
        this.mediagen_url = mgidGetMediagenurlForMgid(this.mgid);
        if (this.feed_url == null) {
            logger.info("Failed to find feed_url for mgid: " + this.mgid);
    ...
    public void handleFree(final DownloadLink link) throws Exception, PluginException {
        ...
        link.setProperty("page_url", br.getURL());
        /*
        ...
        if (mediagen_url == null) {
            ...
            if (br.getRequest() == null) {
                ...
                this.feed_url = mgidGetFeedurlForMgid(this.mgid, original_host);
                if (this.feed_url == null) {
                    logger.warning("Failed to find feed_url");
                }
                br.getPage(feed_url);
            }
        ...
        if (isJson) {
            ...
            entries = (Map<String, Object>) JavaScriptEngineFactory.walkJson(entries, "package/video/item/{0}/rendition/{0}");
        ...
        if (httpurl != null) {
            downloadHTTP(link, httpurl);
        } else if (hlsurl != null) {
            downloadHLS(link, hlsurl);
        } else {
    ...
    private String feedGetMediagenURL() {
        ...
        if (mediagen_url == null) {
            /* 2020-04-23: Nick.de */
            mediagen_url = br.getRegex("(?:\\'|\")(https?://[^<>\"]*?/mediagenerator/[^<>\"/]*?)(?:\\'|\")").getMatch(0);
        }
    ...
    private static String convertRTMPtoHTTP(final String rtmpurl) {
        String httpurl = null;
    ...
    public static HashMap<String, String> feedURLs                    = new HashMap<String, String>(new HashMap<String, String>() {
        {
            put("ALL_OTHERS", url_service_feed_api_mtvnn_v2);
                                                                               ...
                                                                               */
            put("nick.de", url_service_feed_NICK_DE);
    ...
    public static HashMap<String, String> mediagenURLs                = new HashMap<String, String>(new HashMap<String, String>() {
        {
                                                                               ...
                                                                               * before to get the mediagen - URL. This means that having
                                                                               * the mgid is not always enough to get the final URLs.
                                                                               */
            ...
            put("nick.de", url_service_mediagen_ESPERANTO);
                                                                              /*
                                                                               * Do not use pre - defined mediagen for uk . viva . tv to
                                                                               * increase chances of avoiding their GEO - block!
                                                                               */
            ...
            put("mtv.com.au", url_service_mediagen_intl_mtvnservices);
                                                                              /*
                                                                               * Prefer mediautils api url as this can avoid geo - blocks
                                                                               * more often
                                                                               */
    ...
    public static HashMap<String, String> embedURLs                   = new HashMap<String, String>(new HashMap<String, String>() {
        {
                                                                               ...
                                                                               * which contains the feed - URL so this list might be useful
                                                                               * in the future. Strong format - - > Put mgid in.
                                                                               */
    ...
    public static String mgidGetFeedurlForMgid(final String mgid) {
        return mgidGetFeedurlForMgid(mgid, null);
    }

    /** TODO: finish this! */
    /** Returns a mediagen-feed URL based on the domain. */
    public static String mgidGetFeedurlForMgid(final String mgid, final String host_fallback) {
        if (mgid == null) {
            return null;
        }
        final String host_from_mgid = mgidGetHost(mgid);
        ...
        if (mgid_type == null) {
            ...
            feedurl = feedURLs.get(host_from_mgid);
            if (feedurl == null && host_fallback != null) {
                feedurl = feedURLs.get(host_fallback);
            }
        }
        if (feedurl != null) {
            feedurl = String.format(feedurl, mgid);
