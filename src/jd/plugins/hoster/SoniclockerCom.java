...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "soniclocker.com" }, urls = { "https?://(www\\.)?soniclocker\\.com/(embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class SoniclockerCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                   = "http://soniclocker.com";
    private static final String            NICE_HOST                     = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty             = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                       = "(soniclocker\\.com)";
    ...
    /* DEV NOTES */
    // XfileSharingProBasic Version 2.6.9.0
    ...
    @SuppressWarnings("deprecation")
    public SoniclockerCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (SUPPORTS_AVAILABLECHECK_ABUSE && true) {
            /* Special handling as filenames inside html are cut */
            fileInfo[0] = this.getFnameViaAbuseLink(altbr, link);
    ...
    private void fixFilename(final DownloadLink downloadLink) {
        ...
        if (orgName.equalsIgnoreCase(fuid.toLowerCase())) {
            ...
            FFN = orgName + servExt;
        } else if (inValidate(orgExt) && !inValidate(servExt) && orgNameExt.length() > orgNameExt.length()) {
            /* Server filename with extension is longer than name we got via HTML --> Use server filename */
            FFN = servNameExt;
        } else if (true) {
            FFN = servNameExt;
        } else {
