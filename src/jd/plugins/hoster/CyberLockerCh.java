...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "cyberlocker.ch" }, urls = { "https?://(www\\.)?cyberlocker\\.ch/[a-z0-9]{12}" }, flags = { 2 })
public class CyberLockerCh extends PluginForHost {
    ...
    // mods: added filename workaround
    // non account: 2 * 20
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, String directlinkproperty) throws Exception, PluginException {
        ...
        try {
            // add a download slot
            controlFree(+1);
            // filename workaround because extensions may be missing
            String ext = dllink.substring(dllink.lastIndexOf("."));
            if (ext != null && ext.length() < 5 && !downloadLink.getFinalFileName().endsWith(ext)) {
                downloadLink.setFinalFileName(downloadLink.getFinalFileName() + ext);
            }
            // start the dl
            dl.startDownload();
        } finally {
