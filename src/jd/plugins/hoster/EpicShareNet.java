...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "epicshare.net" }, urls = { "https?://(www\\.)?epicshare\\.net/[a-z0-9]{12}" }, flags = { 0 })
public class EpicShareNet extends PluginForHost {
    ...
    // mods: filename/size handling, getdllink
    // non account: 1 * 2
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            dllink = new Regex(correctedBR, "\"(http://\\d+\\.\\d+\\.\\d+\\.\\d+:\\d+/d/[a-z0-9]+/[^<>\"]*?)\"").getMatch(0);
        }
        return dllink;
    }
