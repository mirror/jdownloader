...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "rockdizfile.com" }, urls = { "https?://(www\\.)?rockdizfile\\.com/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
@SuppressWarnings("deprecation")
public class RockDizFileCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://rockdizfile.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(rockdizfile\\.com)";
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            ...
            if (inValidate(fileInfo[0])) {
                ...
                if (inValidate(fileInfo[0])) {
                    fileInfo[0] = cbr.getRegex("<h1>(.*?)</h1>").getMatch(0);
                    if (inValidate(fileInfo[0])) {
        ...
        if (inValidate(fileInfo[1])) {
            ...
            if (inValidate(fileInfo[1])) {
                ...
                if (inValidate(fileInfo[1])) {
                    fileInfo[1] = cbr.getRegex("<b>Size:</b>([^<>\"]*?)<br />").getMatch(0);
                    if (inValidate(fileInfo[1])) {
                        try {
