...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "uploadstore.net" }, urls = { "http://(www\\.)?uploadstore\\.net/[a-z0-9]{12}" }, flags = { 2 })
public class UploadStoreNet extends PluginForHost {
    ...
    public AccountInfo fetchAccountInfo(Account account) throws Exception {
        ...
        if (!NOPREMIUM) {
            ...
            if (expire == null) expire = new Regex(BRBEFORE, "<TD>Premium valid until:</TD><TD>\\&nbsp;\\&nbsp;</TD><TD><b>(.*?)</b>").getMatch(0);
            if (expire == null) {
                ai.setExpired(true);
                account.setValid(false);
                return ai;
