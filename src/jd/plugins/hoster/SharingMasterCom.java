...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "sharingmaster.com" }, urls = { "https?://(www\\.)?sharingmaster\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class SharingMasterCom extends PluginForHost {
    ...
    private static final String  COOKIE_HOST                  = "http://sharingmaster.com";
    // domain names used within download links.
    private static final String  DOMAINS                      = "(sharingmaster\\.com)";
    ...
    // mods: filename/size regexes and dllink function
    // non account: 1 * 1
    // free account: 1 * 20
    // premium account: untested, set standard limits (1 * 10)
    // protocol: no https
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    ...
                    if (dllink == null) {
                        dllink = new Regex(correctedBR, "\"(http://\\d+\\.\\d+\\.\\d+\\.\\d+/cgi\\-bin/dl\\.cgi/[a-z0-9]+/[^<>\"/]*?)\"").getMatch(0);
                        if (dllink == null) {
