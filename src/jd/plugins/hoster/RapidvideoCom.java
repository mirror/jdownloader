...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "rapidvideo.com" }, urls = { "https?://(?:www\\.)?rapidvideo\\.com/(?:\\?v=|embed/)[A-Za-z0-9]+" })
public class RapidvideoCom extends PluginForHost {
    ...
    public String getAGBLink() {
        return "https://www.rapidvideo.com/?c=tos";
    }

    public void correctDownloadLink(final DownloadLink link) {
        final String fid = getFID(link);
        link.setLinkID(fid);
        link.setUrlDownload("https://www." + this.getHost() + "/?v=" + fid);
    }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        int maxquality = 0;
        String sources_source = this.br.getRegex("(?:\")?sources(?:\")?\\s*?:\\s*?\\[(.*?)\\]").getMatch(0);
        if (sources_source != null) {
            sources_source = sources_source.replace("\\", "");
            final String[] qualities = new Regex(sources_source, "((?:\")?file(?:\")?\\s*?:\\s*?\".*?\\})").getColumn(0);
            for (final String quality_info : qualities) {
                final String p = new Regex(quality_info, "label(?:\")?\\s*?:\\s*?\"(\\d+)p").getMatch(0);
                int pint = 0;
                if (p != null) {
