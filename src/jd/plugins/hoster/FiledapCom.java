...
@HostPlugin(revision = "$Revision: 22600 $", interfaceVersion = 2, names = { "filedap.com" }, urls = { "http://(www\\.)?filedap\\.com/.*" }, flags = { 0 })
public class FiledapCom extends PluginForHost {
    ...
    public FiledapCom(final PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        br.getPage("http://www.filedap.com/");
        br.getPage(downloadLink.getDownloadURL());

        if (br.containsHTML("Not Found")) return AvailableStatus.FALSE;

        final String filename = br.getRegex("fname\" value=\"(.*?)\"").getMatch(0);
        if (filename == null) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);

        downloadLink.setName(Encoding.htmlDecode(filename.trim()));
        clickDownload();
        final String filesize = br.getRegex("<small>\\((.*?)\\)</small>").getMatch(0);
        if (filesize != null) {
    ...
    public void doFree(final DownloadLink downloadLink) throws Exception {
        ...
        final String link = br.getRegex("dotted #bbb.*<a href=\"(.*?)\" onclick").getMatch(0);
    ...
    public String getAGBLink() {
        return "http://filedap.com/tos.html";
    }
