...
public class GuShareCom extends PluginForHost {
	...
	public AvailableStatus requestFileInformation(DownloadLink link)
		...
		if (br.containsHTML("(File นี้ไม่มีในระบบแล้ว|Size </font></div>)"))
	...
	public void handleFree(DownloadLink downloadLink) throws Exception,
		...
		if (!br.containsHTML("file_download\\.php"))
			throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
		br.postPage("http://v3.gushare.com/file_download.php",
				"button=Download&file="
						+ new Regex(downloadLink.getDownloadURL(),
								"file\\.php\\?file=(.+)").getMatch(0));
		if (br
				.containsHTML("File นี้เฉพาะ PREMIUM DOWNLOAD เท่านั้น</font> <BR/>ท่านสามารถสมัคร"))
			throw new PluginException(LinkStatus.ERROR_FATAL, JDL.L(
					"plugins.hoster.gusharecom.only4premium",
		...
		dl = jd.plugins.BrowserAdapter.openDownload(br, downloadLink, dlForm,
				false, 1);
