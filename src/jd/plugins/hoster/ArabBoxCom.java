...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "arab-box.com" }, urls = { "https?://(www\\.)?arab\\-box\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class ArabBoxCom extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://arab-box.com";
    ...
    // mods: enhanced fnf errorhandling
    // non account: 20 * 20
    ...
    public ArabBoxCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "<b>اسم الملف:</b></td><td nowrap>([^<>\"]*?)</td>").getMatch(0);
            if (fileInfo[0] == null) fileInfo[0] = new Regex(correctedBR, "<Title>Download ([^<>\"]*?)</Title>").getMatch(0);
        }
        if (fileInfo[1] == null) {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, String directlinkproperty) throws Exception, PluginException {
        if (INMAINTENANCE) throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, MAINTENANCEUSERTEXT, 2 * 60 * 60 * 1000l);
