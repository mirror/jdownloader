...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "fileinstant.com" }, urls = { "https?://(?:www\\.)?fileinstant\\.com/[A-Za-z0-9]+" })
public class FileinstantCom extends PluginForHost {

    public FileinstantCom(PluginWrapper wrapper) {
        super(wrapper);
     ...
     * mods: requestFileInformation[Added one filename RegEx]<br />
     * limit-info:<br />
    ...
    /* Basic constants */
    private final String                   mainpage                                     = "http://fileinstant.com";
    private final String                   domains                                      = "(fileinstant\\.com)";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            filename = this.br.getRegex("addthis:title=\"([^<>\"\\']+)\"").getMatch(0);
            if (filename == null) {
                /* Sometimes we get crippled results with the 2nd RegEx so use this one first */
