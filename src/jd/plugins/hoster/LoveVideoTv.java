...
@HostPlugin(revision = "$Revision: 19496 $", interfaceVersion = 2, names = { "lovevideo.tv" }, urls = { "https?://(www\\.)?lovevideo\\.tv/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
@SuppressWarnings("deprecation")
public class LoveVideoTv extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://lovevideo.tv";
    // domain names used within download links.
    private final String               DOMAINS                      = "(lovevideo\\.tv)";
    ...
    // captchatype: null
    // other: no redirects
    // mods: custom dllinkRegex, regexDllink, doFree: dl headers
     ...
     * */
    public LoveVideoTv(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    private String regexDllink(final String source) {
        ...
        if (links != null && links.length != 0) {
            for (String link : links) {
                if (new Regex(link, dllinkRegex).matches() && inValidate(dllink)) {
                    result = link;
