...
public class VeevrCom extends PluginForHost {
    ...
    public void correctDownloadLink(final DownloadLink link) {
        link.setUrlDownload(link.getDownloadURL().replace("/embed/", "/videos/"));
    }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        downloadLink.setName(new Regex(downloadLink.getDownloadURL(), "([A-Za-z0-9_\\-]+)$").getMatch(0) + ".mp4");
        if (HDS) {
        ...
        br.getHeaders().put("Referer", "http://jw.static.asset.veevr.com/flowplayer-8/flowplayer.commercial-3.2.18.swf");
    ...
    private void hdsCheck() throws PluginException {
        if (HDS) {
            throw new PluginException(LinkStatus.ERROR_FATAL, "HDS streaming isn't supported by JD yet");
        }
    }
