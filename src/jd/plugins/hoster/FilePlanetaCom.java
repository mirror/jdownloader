...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "fileplanet.com.ua", "fileplaneta.com" }, urls = { "BLABLAUNUSED-REGEXZEJFRIFHJZOJNEODCKREOHKUJP", "https?://(www\\.)?(fileplanet\\.com\\.ua|fileplaneta\\.com)/[a-z0-9]{12}" }, flags = { 0, 0 })
public class FilePlanetaCom extends PluginForHost {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, boolean getLinkWithoutLogin) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            br.getPage("http://fileplaneta.com/?op=download2&id=" + fid + "&s=" + s + "&ajax=1");
            doSomething();
            checkErrors(downloadLink, true, passCode);
            dllink = getDllink();
