...
@HostPlugin(revision = "$Revision: 28488 $", interfaceVersion = 3, names = { "fileplaneta.com" }, urls = { "https?://(www\\.)?(fileplanet\\.com\\.ua|fileplaneta\\.com)/[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class FilePlanetaCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://fileplaneta.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(fileplaneta\\.com|fileplanet\\.com\\.ua)";
    ...
    public String rewriteHost(String host) {
        if (host == null || "fileplanet.com.ua".equals(host) || "fileplaneta.com".equals(host)) {
    ...
    private Browser prepBrowser(final Browser prepBr) {
        ...
        prepBr.setCookie(COOKIE_HOST, "rgoods_1", "1");
        prepBr.setCookie(COOKIE_HOST, "ad_rot", "0");
        ...
        if (inValidate(dllink)) {
            ...
            for (int i = 0; i <= repeat; i++) {
                ...
                if (inValidate(dllink)) {
                    ...
                    getPage("/?op=download2&id=" + fuid + "&s=" + s + "&ajax=1");
                    checkErrors(downloadLink, account, true);
                    getDllink();
                    /** End of unusual part */
                    if (inValidate(dllink)) {
    ...
    private void checkErrors(final DownloadLink theLink, final Account account, final boolean checkAll) throws NumberFormatException, PluginException {
        if (checkAll) {
            ...
            if (cbr.containsHTML("\">Skipped countdown<|\"msg\":\"skipped countdown\"")) {
        ...
        synchronized (ACCLOCK) {
            try {
                ...
                if (cookies != null) {
                    ...
                    this.br.getPage("http://fileplaneta.com/?op=my_files");
                    if (this.br.containsHTML("\\?op=logout")) {
                        /* Save timestamp */
