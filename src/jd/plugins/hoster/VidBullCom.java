...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "vidbull.com" }, urls = { "https?://(www\\.)?vidbull\\.com/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
@SuppressWarnings("deprecation")
public class VidBullCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://vidbull.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(vidbull\\.com)";
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            ...
            if (inValidate(fileInfo[0])) {
                ...
                if (inValidate(fileInfo[0])) {
                    fileInfo[0] = cbr.getRegex("Verifying Video Request  -  (.*?)[\r\n\t ]+</h3>").getMatch(0);
                    if (inValidate(fileInfo[0])) {
    ...
    private void doFree(final DownloadLink downloadLink, final Account account) throws Exception, PluginException {
        ...
        if (inValidate(dllink) && (useVidEmbed || (useAltEmbed && downloadLink.getName().matches(".+\\.(asf|avi|flv|m4u|m4v|mov|mkv|mp4|mpeg4?|mpg|ogm|vob|wmv|webm)$")))) {
            ...
            if (inValidate(dllink) && useAltEmbed) {
                ...
                getPage("/embed-" + fuid + "-640x318.html");
                getDllink();
            }
