...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "vidbull.com" }, urls = { "http://(www\\.)?vidbull\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class VidBullCom extends PluginForHost {
    ...
    private static final String COOKIE_HOST   = "http://vidbull.com";
    public boolean              NOPREMIUM     = false;

    public VidBullCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        String filename = new Regex(BRBEFORE, "<h3>Verifying Video Request  \\- ([^<>\"]*?)</h3>").getMatch(0);
        if (filename == null) {
            filename = new Regex(BRBEFORE, "<td nowrap>([^<>\"]*?)</td>").getMatch(0);
        }
        String filesize = new Regex(BRBEFORE, "onFocus=\"copy\\(this\\);.*? \\- (\\d+(\\.\\d+)? [^<>\"/]*?)\\[/URL\\]").getMatch(0);
    ...
    // XfileSharingProBasic Version 2.1.1.4, improved a filename regex, added a
    // filesize regex, added a dllink regex
    @Override
