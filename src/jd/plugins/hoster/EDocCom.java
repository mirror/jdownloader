...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "edoc.com" }, urls = { "https?://(www\\.)?edoc\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class EDocCom extends PluginForHost {

    private String              correctedBR         = "";
    private static final String PASSWORDTEXT        = "(<br><b>Password:</b> <input|<br><b>Passwort:</b> <input)";
    private static final String COOKIE_HOST         = "http://edoc.com";
    ...
    public EDocCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        doSomething();
        if (new Regex(correctedBR, Pattern.compile("(No such file|>File Not Found<|>The file was removed by|Reason (of|for) deletion:\n)", Pattern.CASE_INSENSITIVE)).matches() || br.getURL().equals("http://edoc.com/")) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        if (correctedBR.contains(MAINTENANCE)) {
            link.getLinkStatus().setStatusText(JDL.L("plugins.hoster.xfilesharingprobasic.undermaintenance", MAINTENANCEUSERTEXT));
            return AvailableStatus.TRUE;
        }
        if (br.getURL().equals("http://edoc.com/")) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
