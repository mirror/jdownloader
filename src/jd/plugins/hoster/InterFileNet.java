...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "interfile.net" }, urls = { "https?://(www\\.)?interfile\\.net/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class InterFileNet extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private static final String        COOKIE_HOST                  = "http://interfile.net";
    // domain names used within download links.
    private static final String        DOMAINS                      = "(interfile\\.net)";
    ...
    // last XfileSharingProBasic compare :: 3.0.3.1
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String space[] = new Regex(br, ">Disk usage:</td>.*?<td.*?b>([0-9\\.]+) ?(KB|MB|GB|TB)?</b>").getRow(0);
        ...
        final String availabletraffic = new Regex(br, "Available traffic.*?:</TD><TD[^>]+><b>([^<>\"\\']+)</b>").getMatch(0);
        ...
        if (account.getBooleanProperty("nopremium")) {
            ...
            if (expireDay == null || useAlternativeExpire) {
                ...
                getPage("/?op=payments");
                String expireSecond = new Regex(correctedBR, "Premium-account valid till:</b><br>([^\n\r]+)<").getMatch(0);
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                if (!new Regex(correctedBR, "(Premium(\\-| )Account expire|>Renew premium<|>Premium-account valid till:<)").matches()) {
