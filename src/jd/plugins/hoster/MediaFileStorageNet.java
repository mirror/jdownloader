...
@HostPlugin(revision = "$Revision: 19496 $", interfaceVersion = 2, names = { "multishared.me" }, urls = { "https?://(www\\.)?(mediafilestorage\\.net|multishared\\.me)/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class MediaFileStorageNet extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private static final String        COOKIE_HOST                  = "http://multishared.me";
    // domain names used within download links.
    private static final String        DOMAINS                      = "(mediafilestorage\\.net|multishared\\.me)";
    ...
    // DEV NOTES
    // XfileShare Version 3.0.3.1
    // last XfileSharingProBasic compare :: 2.6.2.1
    // mods: checkLinks
    // protocol: no https
    // captchatype: 4dignum
    // other: no redirects
    // other: account is required, guest account disabled, using api checker
    ...
    private void setConstants(Account account) {
        if (account != null && account.getBooleanProperty("nopremium")) {
            ...
            // prem account
            chunks = -10; // NOT tested
            resumes = true;
            acctype = "Premium Account";
            directlinkproperty = "premlink";
        } else {
            // non account
            chunks = 1; // NOT tested, not possible to download without an account **
            resumes = false;
            acctype = "Non Account";
     ...
     * */
    public MediaFileStorageNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        setConstants(null);
        throw new PluginException(LinkStatus.ERROR_FATAL, "Account required in order to download");
