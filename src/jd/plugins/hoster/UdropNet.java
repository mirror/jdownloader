...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "udrop.net" }, urls = { "https?://(?:www\\.)?udrop\\.net/[A-Za-z0-9]+" })
public class UdropNet extends antiDDoSForHost {
    public UdropNet(PluginWrapper wrapper) {
        super(wrapper);
     ...
     * YetiShareBasic Version 1.1.7-psp<br />
     * mods:<br />
     * limit-info:<br />
     * captchatype: null<br />
    ...
    /* Basic constants */
    private final String                   mainpage                                     = "http://udrop.net";
    private final String                   domains                                      = "(udrop\\.net)";
        ...
        synchronized (LOCK) {
            try {
                ...
                if (useOldLoginMethod) {
                    postPage(this.getProtocol() + this.getHost() + "/login." + type, "submitme=1&username=" + Encoding.urlEncode(account.getUser()) + "&password=" + Encoding.urlEncode(account.getPass()));
