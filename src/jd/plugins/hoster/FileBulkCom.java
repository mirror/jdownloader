...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "filebulk.com" }, urls = { "https?://(www\\.)?filebulk\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class FileBulkCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, ">File Not Available<|>Unfortunately the link you have clicked is not available|>REASONS FOR THIS MAY INCLUDE").matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "<b>Download </b>([^<>\"]*?)<").getMatch(0);
        }
        if (fileInfo[1] == null) {
