...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "royalvids.eu" }, urls = { "https?://(www\\.)?royalvids\\.eu/(tt/)?[a-z0-9]{12}" }, flags = { 0 })
public class RoyalVidsEu extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://royalvids.eu";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(royalvids\\.eu)";
    ...
    // mods: removed filesize(not given), getDllink[Added one more RegEx]
    // limit-info:
    // protocol: no https
    // captchatype: null
    // other: VIDEOHOSTER & VIDEOHOSTER_2 == true
    // TODO: Add case maintenance + alternative filesize check

    @SuppressWarnings("deprecation")
    @Override
    public void correctDownloadLink(final DownloadLink link) {
        link.setUrlDownload(link.getDownloadURL().replace("/tt/", "/"));
    ...
    private String getDllink() {
        ...
        if (dllink == null) {
            dllink = new Regex(correctedBR, "file: \"(http://[^<>\"]*?\\.(?:flv|mp4))\"").getMatch(0);
        }
        return dllink;
    }
