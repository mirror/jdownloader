...
public class TusfilesCom extends XFileSharingProBasic {
    public TusfilesCom(final PluginWrapper wrapper) {
        super(wrapper);
     ...
     * limit-info: 2019-11-21: Premium untested, set FREE account limits <br />
     * captchatype-info: 2019-11-21: null<br />
     * other:<br />
     */
    public static List<String[]> getPluginDomains() {
        final List<String[]> ret = new ArrayList<String[]>();
        // each entry in List<String[]> will result in one PluginForHost, Plugin.getHost() will return String[0]->main domain
        ret.add(new String[] { "tusfiles.com", "tusfiles.net", "tusfiles.co", "tusfiles.nz", "tusfil.es" });
        return ret;
    }
    ...
    public AvailableStatus requestFileInformationWebsite(final DownloadLink link, final Account account, final boolean downloadsStarted) throws Exception {
        ...
        try {
            ...
            if (!con.getContentType().contains("text") && con.isOK() && con.getLongContentLength() > -1 && con.isContentDisposition()) {
                link.setVerifiedFileSize(con.getLongContentLength());
                link.setFinalFileName(getFileNameFromDispositionHeader(con));
                /* Set URL as final downloadurl so template free download handling will work! */
                storeDirecturl(link, account, con.getURL().toString());
                return AvailableStatus.TRUE;
            }
            /* Close connection and continue with template handling */
        } finally {
            try {
                con.disconnect();
