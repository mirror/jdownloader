...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "gorillavid.in" }, urls = { "https?://(www\\.)?gorillavid\\.in/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
...
public class GorillaVidIn extends PluginForHost {
    ...
    private void waitTime(final long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (inValidate(ttt)) ttt = cbr.getRegex(">Wait <span id=\"[a-z0-9]+\">(\\d+)</span> seconds<").getMatch(0);
        if (!inValidate(ttt)) {
