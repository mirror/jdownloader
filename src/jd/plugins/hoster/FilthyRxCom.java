...
public class FilthyRxCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(DownloadLink downloadLink) throws IOException, PluginException {
        ...
        if (br.containsHTML("(title>404: File Not Found at Filthyrx</title>|The file you have requested was not found on this server)") || br.getURL().contains("filthyrx.com/404.php")) {
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
        String filename = br.getRegex("<div class=\"videoleft\">[\t\n\r ]+<h1>(.*?)</h1>").getMatch(0);
        if (filename == null) {
            filename = br.getRegex("<title>(.*?) at Filthyrx</title>").getMatch(0);
        }
        DLLINK = br.getRegex("settings=(http://.*?)\"").getMatch(0);
        if (DLLINK == null) {
            DLLINK = br.getRegex("(http://(www\\.)?filthyrx\\.com/playerConfig\\.php\\?.*?)\"").getMatch(0);
        }
