...
public class ToutBoxFr extends PluginForHost {
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            br.postPage("/action/License/Download", "fileId=" + fileid + "&__RequestVerificationToken=" + Encoding.urlEncode(requestvtoken));
            dllink = br.getRegex("\"redirectUrl\":\"(http[^<>\"]*?)\"").getMatch(0);
            if (dllink == null) {
                /* We might have to skip low-traffic warning */
                ...
                br.postPage("/action/License/acceptLargeTransfer", "fileId=" + fileid + "&orgFile=" + Encoding.urlEncode(orgfile) + "&userSelection=" + Encoding.urlEncode(userSelection) + "&__RequestVerificationToken=" + Encoding.urlEncode(requestvtoken));
                dllink = br.getRegex("\"redirectUrl\":\"(http[^<>\"]*?)\"").getMatch(0);
            }
            if (dllink == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                br.getPage(MAINPAGE);
                br.getHeaders().put("X-Requested-With", "XMLHttpRequest");
                br.postPage("/action/login/loginWindow", "Redirect=true");
                br.postPage("/action/login/login", "RememberMe=true&__RequestVerificationToken=undefined&RedirectUrl=&Redirect=True&FileId=0&Login=" + Encoding.urlEncode(account.getUser()) + "&Password=" + Encoding.urlEncode(account.getPass()));
