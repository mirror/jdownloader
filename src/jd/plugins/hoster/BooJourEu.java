...
public class BooJourEu extends PluginForHost {
    ...
    public void handleFree(DownloadLink downloadLink) throws Exception, PluginException {
        ...
        final Regex postVars = br.getRegex("type=\"hidden\" value=\"([a-z0-9]+)\" class=\"textbox\" name=\"([a-z0-9]+)\" disabled=\"disabled\">");
        if (postVars.getMatches().length < 1) throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
        br.postPage(downloadLink.getDownloadURL(), postVars.getMatch(0) + "=" + postVars.getMatch(1) + "&x=" + new Random().nextInt(100) + "&y=" + new Random().nextInt(100));
