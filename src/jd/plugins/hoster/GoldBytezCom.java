...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "goldbytez.com" }, urls = { "https?://(www\\.)?goldbytez\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class GoldBytezCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://goldbytez.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(goldbytez\\.com)";
    ...
    private void doFree(final DownloadLink downloadLink, final Account account) throws Exception, PluginException {
        ...
        if (inValidate(dllink)) {
            ...
            if (download1 != null) {
                ...
                while (download1.hasInputFieldByName("method_premium")) {
                    download1.remove("method_premium");
                }
                sendForm(download1);
                checkErrors(downloadLink, account, false);
