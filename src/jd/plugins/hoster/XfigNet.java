...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "xfig.net" }, urls = { "http://(www\\.)?xfig\\.net/videos/\\d+/[a-z0-9\\-_]+/" }, flags = { 0 })
public class XfigNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws IOException, PluginException {
        ...
        String filename = br.getRegex("h2>[\tr\n\r ]+<strong>([^<>]*?)</strong>").getMatch(0);
        if (filename == null) {
            filename = br.getRegex("<title>([^<>]*?) \\- xFig Free Porn Videos</title>").getMatch(0);
        }
