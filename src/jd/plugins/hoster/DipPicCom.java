...
@HostPlugin(revision = "$Revision: 17424 $", interfaceVersion = 2, names = { "dippic.com" }, urls = { "https?://(www\\.)?dippic\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class DipPicCom extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://dippic.com";
    ...
    public DipPicCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "class=\"my_img\" alt=\"([^\"]+)").getMatch(0);
            if (fileInfo[0] == null) {
                fileInfo[0] = new Regex(correctedBR, "alt=\"([^\"]+)\"><img src=").getMatch(0);
            }
        }
        if (fileInfo[1] == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    ...
                    if (dllink == null) {
                        ...
                        if (dllink == null) {
                            dllink = new Regex(correctedBR, "(https?://(\\w+\\.)?" + this.getHost() + "/i/\\d+/[^\"\\']+)").getMatch(0);
                            if (dllink == null) {
