...
public class WUploadCom extends PluginForHost implements ControlListener {
	...
	public boolean checkLinks(final DownloadLink[] urls) {
		...
		try {
			...
			while (true) {
				...
				br.postPage("http://api.wupload.com/link?method=getInfo", sb
						.toString());
				for (final DownloadLink dllink : links) {
					final String id = getPureID(dllink);
					final String hit = br.getRegex(
							"link><id>" + id + "(.*?)</link>").getMatch(0);
					if (hit == null || hit.contains("status>NOT_AVAILABLE")) {
						dllink.setAvailable(false);
					} else {
						String name = new Regex(hit, "filename>(.*?)</filename")
								.getMatch(0);
						if (name.startsWith("<![CDATA")) {
							name = new Regex(name, "CDATA\\[(.*?)\\]\\]>")
									.getMatch(0);
						}
						final String size = new Regex(hit, "size>(\\d+)</size")
								.getMatch(0);
	...
	private String downloadAPI(final Browser useBr, final Account account,
		...
		if (pwUsw != null) {
			pw = "&passwords[" + getPureID(link) + "]="
					+ Encoding.urlEncode(pwUsw);
		}
		final String page = br
				.getPage("http://api.wupload.com/link?method=getDownloadLink&u="
						+ Encoding.urlEncode(account.getUser())
						+ "&p="
						+ Encoding.urlEncode(account.getPass())
						+ "&format=xml&ids=" + getPureID(link) + pw);
		if (page.contains("FSApi_Auth_Exception")) {
	...
	private void errorHandling(final DownloadLink downloadLink, final Browser br)
			throws PluginException {
		if (br
				.containsHTML("The server is temporarily offline for maintenance")) {
			throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE,
					"Server Maintenance", 60 * 60 * 1000l);
		}
		if (br
				.containsHTML("The file that you're trying to download is larger than")) {
			final String size = br.getRegex(
					"trying to download is larger than (.*?)\\. <a href=\"")
					.getMatch(0);
			if (size != null) {
				throw new PluginException(
						...
						JDL
								.LF(
										"plugins.hoster.wupload.onlypremiumsize",
										"Only premium users can download files larger than %s.",
										size.trim()));
			} else {
				throw new PluginException(LinkStatus.ERROR_FATAL, JDL.L(
						"plugins.hoster.wupload.onlypremium",
						"Only downloadable for premium users!"));
			}
		}
		if (br.containsHTML("only download 1 file at a time")) {
			throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, JDL.L(
					"plugins.hoster.wupload.alreadyloading",
					"This IP is already downloading"), 5 * 60 * 1000l);
		}
		if (br.containsHTML("Free user can not download files")) {
			throw new PluginException(LinkStatus.ERROR_FATAL, JDL.L(
					"plugins.hoster.wupload.largefree",
					"Free user can not download files over 400MB"));
		}
		if (br.containsHTML("Download session in progress")) {
			throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, JDL.L(
					"plugins.hoster.wupload.inprogress",
					"Download session in progress"), 10 * 60 * 1000l);
		}
		if (br
				.containsHTML("We have detected some suspicious behaviour coming from your IP address")) {
			throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, JDL.L(
					"plugins.hoster.wupload.ipabuse",
					"Hoster blocking IP address due to abuse"),
					6 * 60 * 60 * 1000l);
		}
		...
		if (br
				.containsHTML("((?i)An Error Occurred(?-i)|>404 Not Found<|>The server is temporarily offline for maintenance, please try again later<)")) {
			throw new PluginException(
					LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE,
					JDL.L("plugins.hoster.wupload.servererror", "Server error"),
					20 * 60 * 1000l);
		}
		if (br.containsHTML("This file is available for premium users only\\.")) {
			throw new PluginException(LinkStatus.ERROR_FATAL,
					"Premium only file. Buy Premium Account");
		}
		if (br
				.containsHTML("(You can not access this page directly\\. Please use the|>website to start your download</a>|<p>If the problem persists, clear your cookies and try again\\.</p>)")) {
			logger.warning(br.toString());
			throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE,
					"Unknown server error", 10 * 60 * 1000l);
		}
	}
	...
	public AccountInfo fetchAccountInfo(final Account account) throws Exception {
		synchronized (LOCK) {
			...
			try {
				...
				ai
						.setStatus("Account invalid! Try Email as Login and don't use special chars in PW!");
				account.setValid(false);
				return ai;
			}
			final String expiredate = br.getRegex("expiration>(.*?)</premium")
					.getMatch(0);
			if (expiredate != null) {
		...
		try {
			...
			if (geoDomain == null) {
				...
				if (geoDomain == null) {
					...
					final String domain = new Regex(br.getRedirectLocation(),
							"https?://.*?(wupload\\..*?)(/|$)").getMatch(0);
					if (domain == null) {
						logger.severe("getDomain: failed(2) "
								+ br.getRedirectLocation() + " "
								+ br.toString());
						geoDomain = defaultDomain;
					} else {
	...
	private synchronized String getDomainAPI() {
		try {
			...
			br.setFollowRedirects(true);
			br
					.getPage("http://api.wupload.com/utility?method=getWuploadDomainForCurrentIp");
			final String domain = br.getRegex(
					"response>.*?wupload(\\..*?)</resp").getMatch(0);
			if (domain != null) {
				return "http://www.wupload" + domain;
			}
		} catch (final Throwable e) {
	...
	public String getID(final DownloadLink link) {
		String id = new Regex(link.getDownloadURL(), "/file/([0-9]+(/.+)?)")
				.getMatch(0);
		if (id == null) {
			id = new Regex(link.getDownloadURL(),
					"/file/[a-z0-9]+/([0-9]+(/.+)?)").getMatch(0);
		}
		return id;
	}
	...
	public String getPureID(final DownloadLink link) {
		String id = new Regex(link.getDownloadURL(), "/file/([0-9]+)")
				.getMatch(0);
		if (id == null) {
			id = new Regex(link.getDownloadURL(), "/file/[a-z0-9]+/([0-9]+)")
					.getMatch(0);
		}
		return id;
	}
	...
	public void handleFree(final DownloadLink downloadLink) throws Exception {
		...
		String freeDownloadLink = br.getRegex(".*href=\"(.*?start=1.*?)\"")
				.getMatch(0);
		if (!freeDownloadLink.startsWith("http://")) {
			String domain = new Regex(br.getURL(), "(http.*?\\..*?/)")
					.getMatch(0);
		...
		if (downloadUrl == null) {
			...
			if (ajax
					.containsHTML("This file is available for premium users only\\.")) {

				throw new PluginException(LinkStatus.ERROR_FATAL,
						"Premium only file. Buy Premium Account");
			}
			final String countDownDelay = ajax.getRegex(
					"countDownDelay = (\\d+)").getMatch(0);
			if (countDownDelay != null) {
				...
				if (Long.parseLong(countDownDelay) > 300) {
					...
					throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, Long
							.parseLong(countDownDelay) * 1001l);
				}
				this.sleep(Long.parseLong(countDownDelay) * 1000, downloadLink);
				final String tm = ajax.getRegex(
						"name=\\'tm\\' value=\\'(.*?)\\' />").getMatch(0);
				final String tm_hash = ajax.getRegex(
						"name=\\'tm_hash\\' value='(.*?)' />").getMatch(0);
		...
		if (dl.getConnection() != null
				...
				&& (dl.getConnection().getContentType().contains("html") || dl
						.getConnection().getContentType().contains("unknown"))) {
			br.followConnection();
	...
	public void handlePremium(final DownloadLink downloadLink,
		...
		if (url == null) {
			/* check for needed pw */
			final String status = new Regex(resp, "status>(.*?)</status")
					.getMatch(0);
	...
	private String loginAPI(final Browser useBr, final Account account)
		...
		br.setFollowRedirects(true);
		final String page = br
				.getPage("http://api.wupload.com/user?method=getInfo&u="
	...
	public AvailableStatus requestFileInformation(
			final DownloadLink downloadLink) throws Exception {
