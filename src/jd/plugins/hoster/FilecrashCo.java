...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "filecrash.co" }, urls = { "https?://(www\\.)?filecrash\\.co/(embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class FilecrashCo extends PluginForHost {

    private String                         correctedBR                  = "";
    private String                         passCode                     = null;
    private static final String            PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    /* primary website url, take note of redirects */
    private static final String            COOKIE_HOST                  = "http://filecrash.co";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(filecrash\\.co)";
    /* Linktypes */
    ...
    // limit-info: untested, all final urls lead to 404, set standard limits
    // protocol: no https
    // captchatype: null
    // other:
    ...
    @SuppressWarnings("deprecation")
    public FilecrashCo(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, ">([^<>\"]*?)</font> \\[<font style").getMatch(0);
        }
        if (fileInfo[1] == null) {
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "id=\"countdown_str\".+id=\"[A-Za-z0-9]+\">(\\d+)</span>").getMatch(0);
        }
        if (ttt != null) {
