...
@HostPlugin(revision = "$Revision: 26673$", interfaceVersion = 2, names = { "dopefile.com" }, urls = { "https?://(www\\.)?dopefile\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class DopeFileCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://dopefile.com";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(dopefile\\.com)";
    ...
    // mods: scanInfo[Added another filename RegEx]
    // limit-info:
    // protocol: no https
    // captchatype: null
    // other:
    ...
    public DopeFileCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "fileinfonew\">([^<>\"]*?)<").getMatch(0);
        }
        if (fileInfo[0] == null) {
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        /* Correct encoding for finallink */
        dllink = dllink.replace(" ", "%20").replace("?", "%E2%80%93");
