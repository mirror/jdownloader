...
public class ExLoadCom extends antiDDoSForHost {
    ...
    private final String         DOMAINS                      = "(ex-load\\.com)";
    ...
    private final long           useLoginIndividual           = 24 * 3480000l;
    ...
    // other: no redirects
    // mods: checkErrors: Added another premiumonly String (String "pr"), ttt (waittime), form2 using br as its hidden until timer expires
    // and then shown with js
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            fileInfo[0] = cbr.getRegex("class=\"fa fa-download\"></i>([^<>\"]*?)</h1>").getMatch(0);
        }
        if (inValidate(fileInfo[1])) {
    ...
    private void waitTime(final long timeBefore, final DownloadLink downloadLink) throws PluginException {
        /** Ticket Time */
        String ttt = cbr.getRegex("id=\"countdown_str\"[^>]*>(?:\\s*<[^>]+>){0,}\\s*Wait[^>]+>(?:\\s*<[^>]+>)*\\s*(\\d+)\\s*</span>").getMatch(0);
        if (inValidate(ttt)) {
    ...
    private void checkErrors(final DownloadLink theLink, final Account account, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        // monitor this
        if (cbr.containsHTML("class=\"err\"[^>]*>You have reached the download") || br.containsHTML("class=\"err\"[^>]*>You have reached the download")) {
            /*
             ...
             */
            if (account != null) {
                throw new AccountUnavailableException("Your account ( " + account.getUser() + " @ " + acctype + " ) has been temporarily disabled for going over the download session limit.", 5 * 60 * 60 * 1000l);
            } else {
        ...
        final String pr = "(Please Buy Premium To download this file<|>This file is available for Premium Users only\\.<|>This file is available for Premium Users only<)";
        // let the fun begin!
