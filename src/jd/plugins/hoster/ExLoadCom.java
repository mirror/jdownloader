...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "ex-load.com" }, urls = { "https?://(www\\.)?ex\\-load\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class ExLoadCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|>Please wait, searching file)").matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
