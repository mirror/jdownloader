...
public class CometFilesCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration)))").matches()) {
    ...
    public void checkErrors(final DownloadLink theLink, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(You have reached the download(\\-| )limit|You have to wait|>You must wait)").matches()) {
            ...
            if (tmpmin == null) {
                tmpmin = new Regex(correctedBR, "must wait (\\d+) minutes").getMatch(0);
            }
        ...
        if (br.containsHTML("You are already downloading one file from this server")) {
            throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, 10 * 60 * 1001l);
        }
