...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "supashare.net" }, urls = { "http://(www\\.)?supashare\\.net/[a-z0-9]{12}" }, flags = { 0 })
public class SupaShareNet extends PluginForHost {
    ...
    // XfileSharingProBasic Version 2.5.0.2, added filename- and size regexes
    @Override
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
                ...
                if (filename == null) {
                    ...
                    if (filename == null) {
                        ...
                        if (filename == null) {
                            ...
                            if (filename == null) {
                                filename = new Regex(BRBEFORE, "<strong>Name:</strong>(.*?)<br />").getMatch(0);
                                if (filename == null) {
                                    filename = new Regex(BRBEFORE, "<title>Download (.*?) \\- Supashare\\.net</title>").getMatch(0);
                                }
                            }
                        }
                    }
        ...
        if (filesize == null) {
            ...
            if (filesize == null) {
                ...
                if (filesize == null) {
                    filesize = new Regex(BRBEFORE, ">File Size:</strong> (.*?)<br />").getMatch(0);
                    if (filesize == null) {
                        filesize = new Regex(BRBEFORE, "File Size: ([^\"\\']+)\\.\">").getMatch(0);
                    }
                }
            }
        }
