...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "filejoker.net" }, urls = { "https?://(www\\.)?filejoker\\.net/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class FileJokerNet extends PluginForHost {
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) fileInfo[0] = new Regex(correctedBR, "class=\"name\\-size\">([^<>\"]*?)<").getMatch(0);
        if (fileInfo[1] == null) {
    ...
    public void checkErrors(final DownloadLink theLink, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(You have reached the download(\\-| )limit|You have to wait|until the next download becomes available<)").matches()) {
            ...
            if (tmphrs == null) tmphrs = new Regex(correctedBR, "Please wait.*?\\s+(\\d+)\\s+hours?").getMatch(0);
            String tmpmin = new Regex(WAIT, "\\s+(\\d+)\\s+minutes?").getMatch(0);
            if (tmpmin == null) tmpmin = new Regex(correctedBR, "Please wait.*?\\s+(\\d+)\\s+minutes?").getMatch(0);
            String tmpsec = new Regex(WAIT, "\\s+(\\d+)\\s+seconds?").getMatch(0);
            if (tmpsec == null) tmpsec = new Regex(correctedBR, "Please wait.*?\\s+(\\d+)\\s+seconds?").getMatch(0);
