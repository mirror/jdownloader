...
public class MovDivxCom extends PluginForHost {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, String directlinkproperty) throws Exception, PluginException {
        ...
        if (dl.getConnection().getContentType().contains("html")) {
            ...
            br.followConnection();
            correctBR();
            if (correctedBR.contains("Wrong IPP")) {
    ...
    public void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(?i)<title>The page is temporarily unavailable</title>").matches()) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Temporarily unavailable. Please try again later", 2 * 60 * 60 * 1000l);
        }
