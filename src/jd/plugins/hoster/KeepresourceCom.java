...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "keepresource.com" }, urls = { "https?://(www\\.)?keepresource\\.com/(?:embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class KeepresourceCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                        = "http://keepresource.com";
    private static final String            NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                            = "(keepresource\\.com)";
     ...
     * mods: waitTime[Extra waittime RegEx]<br />
    ...
    public KeepresourceCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void waitTime(final DownloadLink downloadLink, final long timeBefore) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "id=\"[^\"]+\">(\\d+)</span></FONT> seconds</div>").getMatch(0);
        }
        if (ttt != null) {
