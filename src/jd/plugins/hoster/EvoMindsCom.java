...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "evominds.com" }, urls = { "https?://(www\\.)?evominds\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class EvoMindsCom extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://evominds.com";
    ...
    // mods: getdllink
    // non account: 1 * 1, didn't have enough examplelinks
    ...
    public EvoMindsCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            dllink = new Regex(correctedBR, "\"window\\.open\\(\\'/popup\\.html\\'\\);\" href=\"(http://[^<>\"]*?)\"").getMatch(0);
            if (dllink == null) {
                dllink = new Regex(correctedBR, "\"(http://\\d+\\./\\d+\\./\\d+\\./\\d+/em/cgi\\-bin/dl\\.cgi\\?[^<>\"]*?)\"").getMatch(0);
                if (dllink == null) {
