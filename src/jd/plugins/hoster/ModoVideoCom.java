...
public class ModoVideoCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://modovideo.com";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(modovideo\\.com)";
    private static final String            MAINTENANCE                  = ">This server is in maintenance mode|>Not able to generate video link";
    ...
    // mods: rtmp, JD2 only., requestFileInformation[Added another FNF text]
    // limit-info:
    // protocol: no https
    // captchatype: null
    // other: html5 player seems to fail in browser...
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|>Not able to generate video link)").matches()) {
        ...
        if (dllink != null && dllink.startsWith("rtmp://")) {
            try {
                ...
                if (timesFailed <= 5) {
                    ...
                    logger.info("timesfailed_unknown_rtmp_error -> Retrying");
                    ...
                    logger.info("timesfailed_unknown_rtmp_error - disabling current host!");
                    throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Unknown fatal server error", 5 * 60 * 1000l);
                }
            }
        } else {
            // html5 player (fails in browser and jd)
            br.getHeaders().put("Referer", "http://modovideo.com/player/player.swf");
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                String in = new Regex(correctedBR, "jwplayer\\([^\\{]+\\{[^\\}]+\\}").getMatch(-1);
                if (in != null) {
                    String rtmp = new Regex(in, "rtmp://[^\"]+").getMatch(-1);
                    String swf = new Regex(in, "http://[^\"]+\\.swf").getMatch(-1);
                    String path = new Regex(in, "file:\\s*\"([^\"]+)").getMatch(0);
                    if (swf != null && rtmp != null && path != null) {
                        dllink = rtmp + "@" + path + "@" + swf;
                    }
                }
            }
            if (dllink == null) {
                // html5 **NOTE DOES NOT WORK in Browser or JD**
                final String a = new Regex(correctedBR, "file:\\s*\"(.*?)\"").getMatch(0);
                if (a != null) {
                    dllink = (!a.startsWith("/") ? "/" + a : a);
                }
            }
        }
        return dllink;
