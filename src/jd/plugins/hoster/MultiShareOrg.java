...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "multishare.org" }, urls = { "http://(www\\.)?multishare\\.org/file/[a-z0-9]{12}/.*?\\.html" }, flags = { 0 })
public class MultiShareOrg extends PluginForHost {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, boolean checkFastWay) throws Exception, PluginException {
        ...
        if (BRBEFORE.contains("\"download1\"")) {
            br.postPage(downloadLink.getDownloadURL(), "op=download1&usr_login=&id=" + new Regex(downloadLink.getDownloadURL(), COOKIE_HOST.replace("http://", "") + "/file/" + "([a-z0-9]{12}).*?").getMatch(0) + "&fname=" + Encoding.urlEncode(downloadLink.getName()) + "&referer=&method_free=Free+Download");
            doSomething();
        }
