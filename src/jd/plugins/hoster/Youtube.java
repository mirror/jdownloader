...
public class Youtube extends PluginForHost {
	...
	public void correctDownloadLink(final DownloadLink link) throws Exception {
		link.setUrlDownload(link.getDownloadURL().replaceFirst("httpJDYoutube",
				"http"));
	}
	...
	public AccountInfo fetchAccountInfo(final Account account) throws Exception {
		...
		ai.setStatus(JDL
				.L("plugins.hoster.youtube.accountok", "Account is OK."));
		ai.setValidUntil(-1);
	...
	public void handleFree(final DownloadLink downloadLink) throws Exception {
		...
		this.dl = jd.plugins.BrowserAdapter.openDownload(this.br, downloadLink,
				downloadLink.getDownloadURL(), true, 0);
	...
	public void login(final Account account, Browser br, boolean refresh,
		...
		synchronized (Youtube.lock) {
			...
			try {
				...
				String checkConnection = br.getRegex("iframeUri: '(https.*?)'")
						.getMatch(0);
				...
				form.put(
						"dnConn",
						"https%3A%2F%2Faccounts.youtube.com&continue=http%3A%2F%2Fwww.youtube.com%2Fsignin%3Faction_handle_signin%3Dtrue%26nomobiletemp%3D1%26hl%3Den_US%26next%3D%252F");
				...
				final String cook = br.getCookie("http://www.google.com",
						"GALX");
				if (cook == null) {
				...
				if (br.getRedirectLocation() == null) {
					...
					final String red = new Regex(page, "url='(http://.*?)'")
							.getMatch(0);
					if (red == null) {
						account.setValid(false);
				...
				if (br.containsHTML("Google will check if this")) {
					if (showDialog)
						UserIO.getInstance()
								.requestMessageDialog(0, "Youtube Login Error",
										"Please logout and login again at youtube.com, account check needed!");
					account.setValid(false);
					throw new PluginException(LinkStatus.ERROR_PREMIUM,
							PluginException.VALUE_ID_PREMIUM_DISABLE);
				}
	...
	private void postprocess(final DownloadLink downloadLink) {
		if (downloadLink.getProperty("convertto") != null) {
			final DestinationFormat convertto = DestinationFormat
					.valueOf(downloadLink.getProperty("convertto").toString());
			DestinationFormat InType = DestinationFormat.VIDEOFLV;
			if (convertto.equals(DestinationFormat.VIDEOWEBM)
					|| convertto.equals(DestinationFormat.VIDEOMP4)
					|| convertto.equals(DestinationFormat.VIDEO3GP)) {
				InType = convertto;
			}
	...
	public AvailableStatus requestFileInformation(
		...
		for (int i = 0; i < 4; i++) {
			if (downloadLink.getBooleanProperty("valid", true)) {
				downloadLink.setFinalFileName(downloadLink.getStringProperty(
						"name", "video.tmp"));
				downloadLink.setDownloadSize((Long) downloadLink.getProperty(
						"size", 0l));
				return AvailableStatus.TRUE;
			} else {
				downloadLink.setFinalFileName(downloadLink.getStringProperty(
				...
				downloadLink.setDownloadSize((Long) downloadLink.getProperty(
						"size", 0l));
				final PluginForDecrypt plugin = JDUtilities
						.getPluginForDecrypt("youtube.com");
				if (plugin == null) {
					throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT,
							"cannot decrypt videolink");
				}
				if (downloadLink.getStringProperty("fmtNew", null) == null) {
					throw new PluginException(LinkStatus.ERROR_FATAL,
							"You have to add link again");
				}
				...
				final HashMap<Integer, String[]> LinksFound = ((TbCm) plugin)
						.getLinks(downloadLink.getStringProperty("videolink",
								null), this.prem, this.br, 0);

				if (LinksFound == null || LinksFound.isEmpty()) {
					if (this.br
							.containsHTML("<div\\s+id=\"verify-age-actions\">")) {
						throw new PluginException(LinkStatus.ERROR_FATAL,
								"The entered account couldn't pass the age verification!");
					}
					throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
				}
				if (LinksFound
						.get(downloadLink.getIntegerProperty("fmtNew", 0)) == null) {
					// too fast connections??
					Thread.sleep(5000);
					continue;

				}
				downloadLink.setUrlDownload(LinksFound.get(downloadLink
						.getIntegerProperty("fmtNew", 0))[0]);
				return AvailableStatus.TRUE;
			}
		}
	...
	public void resetDownloadlink(final DownloadLink downloadLink) {
		downloadLink.setFinalFileName(downloadLink.getStringProperty("name",
				"video.tmp"));
		downloadLink.setDownloadSize((Long) downloadLink
				.getProperty("size", 0l));
		downloadLink.setProperty("valid", false);
	}
	...
	private void setConfigElements() {
		getConfig().addEntry(
				new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
						getPluginConfig(), FAST_CHECK, JDL.L(
								"plugins.hoster.youtube.fast",
								"Fast LinkCheck?")).setDefaultValue(false));
		getConfig().addEntry(
				new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
						getPluginConfig(), IDASFILENAME, JDL.L(
								"plugins.hoster.youtube.idasfilename",
								"Use Video-ID as filename?"))
						.setDefaultValue(false));
		getConfig().addEntry(
				new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
						getPluginConfig(), ALLOW_SUBTITLES, JDL.L(
		...
		getConfig().addEntry(
				new ConfigEntry(ConfigContainer.TYPE_CHECKBOX,
						getPluginConfig(), PROXY_ACTIVE, JDL.L(
								"plugins.hoster.youtube.proxyactive",
								"Use HTTP Proxy?")).setDefaultValue(false));
		getConfig().addEntry(
				new ConfigEntry(ConfigContainer.TYPE_TEXTFIELD,
						getPluginConfig(), PROXY_ADDRESS, JDL.L(
								"plugins.hoster.youtube.proxyaddress",
								"Proxy Address")));
		getConfig().addEntry(
				new ConfigEntry(ConfigContainer.TYPE_TEXTFIELD,
						getPluginConfig(), PROXY_PORT, JDL.L(
								"plugins.hoster.youtube.proxyport",
								"Proxy Port")));
	}
}