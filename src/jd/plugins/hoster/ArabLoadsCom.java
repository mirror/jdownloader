...
public class ArabLoadsCom extends PluginForHost {
    ...
    public String rewriteHost(String host) {
        if ("arabloads.com".equals(getHost())) {
            if (host == null || "arabloads.com".equals(host)) {
                return "arabloads.net";
            }
        }
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[1] == null) {
            /* Special: Use BR here as this information is in outcommented html */
            fileInfo[1] = br.getRegex("\\&filesize=([^<>\"]*?)\\&").getMatch(0);
        }
        if (fileInfo[1] == null) {
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        /** Ticket Time */
        final String ttt = new Regex(correctedBR, "id=\"countdown\">[^<>\"]+<span( class=\"[^<>\"]+\")?>([\n ]+)?(\\d+)([\n ]+)?</span>").getMatch(2);
        if (ttt != null) {
    ...
    public void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if (correctedBR.equals("Wrong IP")) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error: 'Wrong IP'");
        }
    }
