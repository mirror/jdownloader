...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "chronos.to" }, urls = { "https?://(www\\.)?chronos\\.to/(embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class ChronosTo extends PluginForHost {

    private String                         correctedBR                   = "";
    private String                         passCode                      = null;
    private static final String            PASSWORDTEXT                  = "<br><b>Passwor(d|t):</b> <input";
    /* primary website url, take note of redirects */
    private static final String            COOKIE_HOST                   = "http://chronos.to";
    private static final String            NICE_HOST                     = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty             = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                       = "(chronos\\.to)";
    /* Linktypes */
    ...
    @SuppressWarnings("deprecation")
    public ChronosTo(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            Form download1 = null;
            do {
                download1 = getFormByKey("op", "download1");
                if (download1 == null) {
                    download1 = this.br.getFormbyKey("next");
                }
                if (download1 != null) {
                    ...
                    checkErrors(downloadLink, false);
                    dllink = getDllink();
                    final String fname = new Regex(correctedBR, "class=\"pic\" alt=\"([^<>\"]*?)\"").getMatch(0);
                    if (fname != null) {
                        downloadLink.setName(Encoding.htmlDecode(fname));
                    }
                }
            } while (download1 != null);
        }
        if (dllink == null) {
