...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "hipfile.com" }, urls = { "https?://(www\\.)?hipfile\\.com/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class HipFileCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://hipfile.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(hipfile\\.com)";
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[1])) {
            ...
            if (inValidate(fileInfo[1])) {
                ...
                if (inValidate(fileInfo[1])) {
                    fileInfo[1] = cbr.getRegex("<b>Size:</b></td><td>(\\d+(\\.\\d+)? ?(KB|MB|GB))").getMatch(0);
                    if (inValidate(fileInfo[1])) {
