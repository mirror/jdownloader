...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "bestreams.net" }, urls = { "https?://(www\\.)?bestreams\\.net/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 0 })
...
public class BeStreamsNet extends PluginForHost {
    ...
    private final String               DOMAINS                      = "(bestreams\\.net)";
    ...
    private void getDllink() {
        ...
        if (dllink == null && cbr.containsHTML("rtmp://")) {
            final String i = cbr.getRegex("image:\\s*\"(https?://[^/]+/)").getMatch(0);
            final String h = cbr.getRegex("\\?h=([a-z0-9]{30,70})").getMatch(0);
            if (h != null && i != null) {
    ...
    private void waitTime(final long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (inValidate(ttt)) {
            ttt = cbr.getRegex(">Wait <span id=\"[a-z0-9]+\">(\\d+)</span> seconds</span>").getMatch(0);
        }
        if (!inValidate(ttt)) {
