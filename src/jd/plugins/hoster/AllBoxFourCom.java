...
@HostPlugin(revision = "$Revision: 17424 $", interfaceVersion = 2, names = { "allbox4.com" }, urls = { "https?://(www\\.)?allbox4\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class AllBoxFourCom extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://allbox4.com";
    ...
    private String[] scanInfo(String[] fileInfo) {
        ...
        if (fileInfo[1] == null) {
            fileInfo[1] = new Regex(correctedBR, "<span><strong>(\\d+)</strong>").getMatch(0);
            if (fileInfo[1] == null) {
    ...
    private void waitTime(long timeBefore, DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null || ttt.length() == 0) {
            ttt = new Regex(correctedBR, "<span>Por favor, espere <sup id=\"[^<>\"]+\">(\\d+)</sup> Segundos</span>").getMatch(0);
        }
        if (ttt != null) {
