...
@HostPlugin(revision = "$Revision: 16510 $", interfaceVersion = 2, names = { "pandamemo.com" }, urls = { "https?://(www\\.)?pandamemo\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class PandamemoCom extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://pandamemo.com";
    ...
    public String getAGBLink() {
        return this.getHost() + "/tos.html";
    }

    public PandamemoCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        if (filename == null) {
            ...
            if (filename == null) {
                ...
                if (filename == null) {
                    filename = new Regex(correctedBR, "File(name)?:? ?(<[^>]+> ?)+?([^<>\"\\']+)").getMatch(2);
                }
            }
        }
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    ...
                    if (dllink == null) {
                        dllink = new Regex(correctedBR, "<a href=\"(https?://[^\"]+)\"><img src=\"[^\"]+ title=\"(Click to Download|Download File)").getMatch(0);
                        if (dllink == null) {
                            dllink = new Regex(correctedBR, "(https?://\\w+\\.pandamemo\\.com[^/]+/files/[^\"]+)").getMatch(0);
                            if (dllink == null) {
