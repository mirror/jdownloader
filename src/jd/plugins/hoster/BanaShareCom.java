...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "banashare.com" }, urls = { "https?://(www\\.)?banashare\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class BanaShareCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://banashare.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(banashare\\.com)";
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            ...
            if (inValidate(fileInfo[0])) {
                ...
                if (inValidate(fileInfo[0])) {
                    fileInfo[0] = cbr.getRegex("<div id=\"file_name\">(.*?)</div>").getMatch(0);
                    if (inValidate(fileInfo[0])) {
