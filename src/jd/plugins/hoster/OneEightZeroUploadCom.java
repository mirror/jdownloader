...
public class OneEightZeroUploadCom extends PluginForHost {
    ...
    // captchatype: null
    // other: no redirects
    // other: they allow one -2 connection to each fileserver, ngnix throws header errors with more than 1 maxdl to that server ip, they
    // have multiple fileservers ip allowing session to each, but no solution coded for that yet.
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        String filename = new Regex(br, "<b>Filename:</b></td><td nowrap>([^<>]+)").getMatch(0);
        if (filename == null) {
