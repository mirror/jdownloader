...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "datasbit.com" }, urls = { "https?://(www\\.)?datasbit\\.com/(?:embed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class DatasbitCom extends PluginForHost {
     ...
     * limit-info: only free account is tested, set these limits for all download modes<br />
    ...
    public DatasbitCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private void waitTime(final DownloadLink downloadLink, final long timeBefore) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "id=\"[a-z0-9]+\">(\\d+)</span> seconds</span>").getMatch(0);
        }
        if (ttt != null) {
