...
public class DdlStorageCom extends PluginForHost {
    ...
    private final boolean              useAlternativeExpire         = true;
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration)))").matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        /** Ticket Time */
        String ttt = new Regex(correctedBR, "limit: (\\d+),").getMatch(0);
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        // free account
        // <td class="td_title">Used space:<span class="td_descr">(<span id="usr_disk_used">0.00</span> GB of <span id="usr_disk_available">10</span> GB)</span></td>
        // data based account = same as above
        final String space[] = new Regex(correctedBR, ">Used space:[^>]+>[^>]+>([0-9\\.]+)</span> (KB|MB|GB|TB)").getRow(0);
        ...
        // free account
        // <td class="td_title">Monthly bandwidth used:<span class="td_descr">(<span id="usr_bandwidth_used">2</span> Mb of <span id="usr_bandwidth_available">1500</span> Mb)</span></td>
        // <td class="td_title">Monthly bandwidth used:<span class="td_descr">(<span id="usr_bandwidth_used">0.00</span> Gb of <span id="usr_bandwidth_available">UNLIMITED</span>)</span></td>
        final String[] availabletraffic = new Regex(correctedBR, ">Monthly bandwidth used:[^>]+>[^>]+>([0-9\\.]+)</span> (KB|MB|GB|TB) of <[^>]+>(([0-9\\.]+)</span> (KB|MB|GB|TB)|UNLIMITED)").getRow(0);
        ...
        if (account.getBooleanProperty("nopremium")) {
            ...
            if (expireDay == null || useAlternativeExpire) {
                ...
                getPage("/?op=payments");
                String expireSecond = new Regex(correctedBR, "Premium(\\-| )Account expires?:([^\n\r]+)").getMatch(1);
    ...
    private Browser prepBrowser(Browser prepBr) {
        ...
        prepBr.setCookie(COOKIE_HOST, "design", "2");
        return prepBr;
    }
    ...
    private void ipBlock() throws PluginException {
        if (new Regex(correctedBR, "(Access from [^ ]+ is not allowed|DDLStorage temporarily unavailable from your region)").matches()) {
