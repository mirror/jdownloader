...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "uploadc.com" }, urls = { "http://(www\\.)?uploadc\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class UploadCCom extends PluginForHost {
    ...
    // XfileSharingProBasic Version 2.3.0.0, with major changes
    @Override
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
                ...
                if (filename == null) {
                    ...
                    if (filename == null) {
                        ...
                        if (filename == null) {
                            ...
                            if (filename == null) {
                                filename = new Regex(BRBEFORE, "Filename[\t\n\r ]+: <b>(.*?)</b></nobr><br>").getMatch(0);
                                if (filename == null) {
                                    filename = new Regex(BRBEFORE, "<img alt=\"Download or play this song  (.*?) \"").getMatch(0);
                                    if (filename == null) {
                                        filename = new Regex(BRBEFORE, "<h2 style=\"text\\-align:center;color:#EE5F00;\">(.*?)</h2>").getMatch(0);
                                    }
                                }
                            }
                        }
        ...
        if (filesize == null) {
            ...
            if (filesize == null) {
                ...
                if (filesize == null) filesize = new Regex(BRBEFORE, ">File Size :</strong> </span><span class=\"file_code\">(.*?)</span></td>").getMatch(0);
            }
        }
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks) throws Exception, PluginException {
        checkErrors(downloadLink, true, null);
        br.getPage("http://www.uploadc.com/download-" + new Regex(downloadLink.getDownloadURL(), "uploadc\\.com/(.+)").getMatch(0) + ".html");
        ...
        if (decoded != null) {
            ...
            if (finallink == null) {
                ...
                if (finallink == null) {
                    ...
                    if (finallink == null) {
                        finallink = new Regex(decoded, "location\\.href=\\'(http://.*?)\\'").getMatch(0);
                    }
                }
            }
        }
