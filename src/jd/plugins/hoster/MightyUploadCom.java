...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "mightyupload.com" }, urls = { "https?://(www\\.)?mightyupload\\.com/((vid)?embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
...
public class MightyUploadCom extends PluginForHost {
    ...
    private final String               DOMAINS                      = "(mightyupload\\.com)";
    ...
    // captchatype: null
    // other: no redirects
    // other: rockfilediz sister site?? its mentioned in source
    // mods:
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            ...
            if (inValidate(fileInfo[0])) {
                ...
                if (inValidate(fileInfo[0])) {
                    fileInfo[0] = cbr.getRegex("<h3>(.*?)</h3>").getMatch(0);
                    if (inValidate(fileInfo[0])) {
