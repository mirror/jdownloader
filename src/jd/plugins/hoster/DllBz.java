...
public class DllBz extends PluginForHost {
    ...
    public DllBz(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    private static final String COOKIE_HOST = "http://dll.bz";
    ...
    public AvailableStatus requestFileInformation(DownloadLink parameter) throws Exception {
        ...
        String filename = br.getRegex("\"http://dll\\.bz/file/[0-9]+/(.*?)\\.html\"").getMatch(0);
        if (filename == null) filename = br.getRegex("<title>(.*?)</title>").getMatch(0);
        String filesize = br.getRegex("<h2>.*? \\((.*?)\\)").getMatch(0);
