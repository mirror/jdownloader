...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "x-share.ru" }, urls = { "https?://(?:www\\.)?x\\-share\\.ru/[A-Za-z0-9]+" }, flags = { 2 })
public class XShareRu extends PluginForHost {

    public XShareRu(PluginWrapper wrapper) {
        super(wrapper);
    ...
    /* Basic constants */
    private final String         mainpage                                     = "http://x-share.ru";
    private final String         domains                                      = "(x\\-share\\.ru)";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            handleErrors();
            if (br.getURL().contains("/error." + type) || br.getURL().contains("/index." + type) || (!br.containsHTML("class=\"downloadPageTable(V2)?\"") && !br.containsHTML("download\\-timer'")) || br.getHttpConnection().getResponseCode() == 404) {
                throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
            }
