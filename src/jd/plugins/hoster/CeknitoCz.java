 ...
 * 
 */
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "ceknito.cz" }, urls = { "http://[\\w\\.]*?ceknito\\.cz/video/\\d+" }, flags = { 0 })
public class CeknitoCz extends PluginForHost {
    private String dlink = null;

    public CeknitoCz(PluginWrapper wrapper) {
        super(wrapper);
    }

    @Override
    public String getAGBLink() {
        return "http://www.ceknito.cz/pravidla";
    }
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        String filename = br.getRegex("<title>(.*?) - video | ceknito.cz</title>").getMatch(0);
        if (filename == null) {
