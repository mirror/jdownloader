...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "batshare.com" }, urls = { "https?://(www\\.)?batshare\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class BatShareCom extends PluginForHost {
    ...
    // mods: costum filename regexes, doFree
    // non account: 20 * 20
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "<div class=\"tite\"><h1>([^<>\"]*?)</h1>").getMatch(0);
            if (fileInfo[0] == null) {
                fileInfo[0] = new Regex(correctedBR, "<meta title=\"([^<>\"]*?)\"").getMatch(0);
            }
        }
        if (fileInfo[1] == null) {
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        br.getPage("http://batshare.com/pl-" + downloadLink.getDownloadURL().substring(downloadLink.getDownloadURL().lastIndexOf("/") + 1) + ".mp3");
