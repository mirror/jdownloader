...
public class BatShareCom extends PluginForHost {
    ...
    private static final String COOKIE_HOST         = "http://batshare.com";
    ...
    // mods: AvailableStatus, getDllink
    // non account: 20 * 20
    // free account:
    // premium account:
    // protocol: no https (not setup correctly)
    // captchatype: null
    // other: no redirects
    ...
    public BatShareCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        String filename = new Regex(correctedBR, "<div class=\"tite\"><h1>([^<]+)").getMatch(0);
        if (filename == null) {
            filename = new Regex(correctedBR, "<meta title=\"([^\"]+)").getMatch(0);
            if (filename == null) {
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    ...
                    if (dllink == null) {
                        dllink = new Regex(correctedBR, "(?i)<a href=\"(https?://[^\"]+)\">Download</a>").getMatch(0);
                        if (dllink == null) {
    ...
    public void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(?i)>Error happened when generating Download Link\\.<br>Please try again or Contact administrator\\.<").matches()) throw new PluginException(LinkStatus.ERROR_FATAL, "Server error");
