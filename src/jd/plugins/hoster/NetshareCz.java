...
public class NetshareCz extends antiDDoSForHost {
    public NetshareCz(PluginWrapper wrapper) {
        super(wrapper);
        this.enablePremium(mainpage + "/upgrade." + type);
    }

    /* 1st domain = current domain! */
    public static String[] domains = new String[] { "netshare.cz" };
     ...
     * YetiShareBasic Version 1.1.8-psp<br />
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            final boolean isFileWebsite = br.containsHTML("id=\"dwn-speed\"");
            ...
            if (filesize == null) {
                /* 2019-01-23: Special */
                filesize = br.getRegex("<h1>[^<>]+ \\((\\d+[^<>\"\\'\\)]+)\\)</h1>").getMatch(0);
                if (filesize == null) {
            ...
            if (filename == null) {
                /* 2019-01-23: Special */
                filename = br.getRegex("<title>([^<>\"]+) \\- NETSHARE\\.cz</title>").getMatch(0);
                if (filename == null) {
    ...
    private void handleErrors() throws PluginException {
        final String waittimeBetweenDownloads = br.getRegex("You must wait (\\d+)\\&nbsp;minutes between downloads\\.").getMatch(0);
        if (waittimeBetweenDownloads != null) {
            /* 2019-01-23: Special */
            throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, Integer.parseInt(waittimeBetweenDownloads) * 60 * 1001l);
        } else if (br.containsHTML("Error: Too many concurrent download requests")) {
    ...
    private Browser prepBrowser(final Browser br) {
        ...
        try {
            /* 2019-01-23: Special - required as they do not have any language cookie. */
            br.getPage("https://www." + this.getHost() + "/?_t=English+%28en%29");
        } catch (final Throwable e) {
        }
        return br;
    }
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        final String premiumTrafficLeft = br.getRegex("My account \\(([^<>\"\\']+)\\)<").getMatch(0);
        if (!br.containsHTML("class=\"badge badge\\-success\">(?:PAID USER|USUARIO DE PAGO|VIP)</span>") && premiumTrafficLeft == null) {
            ...
            if (premiumTrafficLeft != null) {
                final long trafficLeft = SizeFormatter.getSize(premiumTrafficLeft.replace(",", "."));
