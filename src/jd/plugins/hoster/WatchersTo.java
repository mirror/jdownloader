...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "watchers.to" }, urls = { "https?://(?:www\\.)?watchers\\.to/(?:embed\\-)?[a-z0-9]{12}" })
public class WatchersTo extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                        = "http://watchers.to";
    private static final String  NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String  DOMAINS                            = "(watchers\\.to)";
     ...
     * mods: 2017-02-07: scanInfo[Extra filename- and filesize RegExes]<br />
     * limit-info:<br />
    ...
    public WatchersTo(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|File Not Found|>The file expired|>File was locked)").matches()) {
                ...
                if (inValidate(fileInfo[1])) {
                    ...
                    if (inValidate(fileInfo[1])) {
                        ...
                        if (inValidate(fileInfo[1])) {
                            ...
                            // special failover#1
                            if (inValidate(fileInfo[1])) {
                                fileInfo[1] = new Regex(correctedBR, "(\\d+(?:\\.\\d+)? ?(KB|MB|GB))</td").getMatch(0);
                            }
                            // generic failover#1
                            if (inValidate(fileInfo[1])) {
                                fileInfo[1] = new Regex(correctedBR, "(\\d+(?:\\.\\d+)? ?(MB|GB))").getMatch(0);
                            }
                            // generic failover#2
                            if (inValidate(fileInfo[1])) {
                            ...
                            // generic failover#3
                            if (inValidate(fileInfo[1])) {
                                fileInfo[1] = new Regex(correctedBR, "(\\d+(?:\\.\\d+)? ?KB)").getMatch(0);
                            }
                        }
                    }
                }
