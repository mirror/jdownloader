...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "vidtodo.com" }, urls = { "https?://(?:www\\.)?vids?o?todo?\\.(?:com|me|pro)/(?:embed\\-)?[a-z0-9]{12}" })
public class VidtodoCom extends antiDDoSForHost {
    ...
    private static final String  COOKIE_HOST                        = "https://vidtod.me";
    private static final String  NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String  DOMAINS                            = "(vids?o?todo?\\.(me|com|pro))";
    ...
    @SuppressWarnings("deprecation")
    public VidtodoCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            fileInfo[0] = new Regex(correctedBR, "<div id=\"content\" class=\"left\">\\s*<h2>\\s*(.*?)\\s*</h2>").getMatch(0);
            if (inValidate(fileInfo[0])) {
        ...
        if (inValidate(fileInfo[0])) {
            fileInfo[0] = new Regex(correctedBR, "<title>(?:Watch )?([^<>]*?)(?: mp4)?</title>").getMatch(0);
        }
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            if (download1 != null) {
                ...
                /* end of backward compatibility */
                /* 2017-11-21: Special: Waittime at this place */
    ...
    private String getDllink() {
        ...
        if (dllink == null) {
            dllink = new Regex(correctedBR, "sources: \\[\"([^\"]+)\"").getMatch(0);
        }
        return dllink;
    }
    ...
    private void checkErrors(final DownloadLink theLink, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        if (correctedBR.contains(">Video is processing now")) {
            /* 2017-02-07: Special */
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error 'Video is processing now'", 10 * 60 * 1000l);
        }
