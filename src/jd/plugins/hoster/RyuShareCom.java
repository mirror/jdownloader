...
public class RyuShareCom extends PluginForHost {
    ...
    private void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if (br.getRequest().getHttpConnection().getResponseCode() == 404) throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error 404", 30 * 60 * 1000l);
        ...
        try {
            ...
            if (config.getBooleanProperty("premAdShown", Boolean.FALSE) == false) {
                if (config.getProperty("premAdShown2") == null) {
                    File checkFile = JDUtilities.getResourceFile("tmp/ryush");
                    if (!checkFile.exists()) {
                        checkFile.mkdirs();
                        showFreeDialog("ryushare.com");
                    }
                } else {
                    config = null;
                }
