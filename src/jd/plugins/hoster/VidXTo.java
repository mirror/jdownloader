...
@HostPlugin(revision = "$Revision: 19496 $", interfaceVersion = 2, names = { "vidx.to" }, urls = { "https?://(www\\.)?vidx\\.to/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class VidXTo extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://vidx.to";
    // domain names used within download links.
    private final String               DOMAINS                      = "(vidx\\.to)";
     ...
     * */
    public VidXTo(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        String space[] = cbr.getRegex(">Used space.*?<b>([0-9\\.]+) ?(KB|MB|GB|TB)?").getRow(0);
        if (space == null || space.length == 0) space = cbr.getRegex(">Used space.*?<b>([0-9\\.]+) of [0-9\\.]+ ?(KB|MB|GB|TB)?").getRow(0);
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (ACCLOCK) {
            try {
                ...
                sendForm(loginform);
                String test1 = br.getCookie(COOKIE_HOST, "login");
                String test2 = br.getCookie(COOKIE_HOST, "xfss");
                String test3 = br.getCookie(COOKIE_HOST, "xfsts");
