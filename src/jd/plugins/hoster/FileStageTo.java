...
public class FileStageTo extends PluginForHost {
    ...
    public FileStageTo(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    public String getAGBLink() {
        return "http://www.filestage.to/agb/";
    }
    ...
    public AvailableStatus requestFileInformation(DownloadLink downloadLink) throws IOException, PluginException {
        ...
        if (filename.equals("http://www.filestage.to")) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        if (filename.substring(filename.length() - 4, filename.length()).contains(".")) filename = filename.replace(filename.substring(filename.length() - 4, filename.length()), "");
