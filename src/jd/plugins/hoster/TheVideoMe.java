...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "thevideo.me" }, urls = { "https?://(www\\.)?thevideo\\.me/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class TheVideoMe extends PluginForHost {
    ...
    private String[] scanInfo(final String[] fileInfo) {
        ...
        if (fileInfo[0] == null) {
            fileInfo[0] = new Regex(correctedBR, "<div class=\"videoTIT\">([^<>\"]*?)<b class=").getMatch(0);
        }
        if (fileInfo[1] == null) {
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null && VIDEOHOSTER_2) {
            try {
                ...
                final String embed_access = "http://" + COOKIE_HOST.replace("http://", "") + "/embed-" + fuid + ".html?play=1&confirm=Close+Ad+and+Watch+as+Free+User";
