...
public class KrotixNet extends PluginForHost {
    ...
    public KrotixNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    /* Basic constants */
    private final String         mainpage                                     = "http://krotix.net";
    private final String         domains                                      = "(krotix\\.net)";
    ...
    private void login(final Account account, boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                br.getPage(loginstart + this.getHost() + "/manage." + type);
                if (!br.containsHTML("class=\"badge badge\\-success\">PAID USER</span>|class=\"btn btn\\-success\">PREMIUM</span>")) {
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        if (account.getBooleanProperty("free", false)) {
            ...
            if (expire_milliseconds > 0 && (expire_milliseconds - System.currentTimeMillis()) <= 0) {
                account.setProperty("free", true);
                try {
                    account.setType(AccountType.FREE);
                    account.setMaxSimultanDownloads(account_FREE_MAXDOWNLOADS);
