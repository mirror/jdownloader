...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "lunaticfiles.com" }, urls = { "https?://(www\\.)?lunaticfiles\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class LunaticFilesCom extends PluginForHost {
    ...
    public LunaticFilesCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink != null) {
            // Wait some time or the directlinks are not valid anymore
    ...
    public void checkErrors(final DownloadLink theLink, final boolean checkAll) throws NumberFormatException, PluginException {
        ...
        if (new Regex(correctedBR, "(You have reached the download(\\-| )limit|You have to wait|Musisz odczekać jeszcze )").matches()) {
            ...
            if (tmpmin == null) tmpmin = new Regex(correctedBR, "Musisz odczekać jeszcze (\\d+) minut").getMatch(0);
        ...
        if (correctedBR.contains(">Pobierasz już jeden plik z naszych serwerów")) { throw new PluginException(LinkStatus.ERROR_IP_BLOCKED, 10 * 60 * 1001l); }
        if (br.getURL().contains("/?op=login&redirect=")) {
            logger.info("Only downloadable via premium");
            throw new PluginException(LinkStatus.ERROR_FATAL, PREMIUMONLY2);
        }
