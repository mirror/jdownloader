...
public class IGetFileCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (inValidate(fileInfo[0])) {
            if (cbr.containsHTML("(You have reached the download(-| )limit|You have to wait[^\r\n]+till next download)")) {
            ...
            if (inValidate(fileInfo[0])) {
                ...
                if (inValidate(fileInfo[0])) {
                    ...
                    if (inValidate(fileInfo[0])) {
                        ...
                        if (inValidate(fileInfo[0])) {
                            fileInfo[0] = cbr.getRegex("<font color=[^>]+><strong>Download: (.*?) \\([\\d\\.]+ (KB|MB|GB)\\)</strong></font>").getMatch(0);
    ...
    private void doFree(final DownloadLink downloadLink, final Account account) throws Exception, PluginException {
        ...
        if (dl.getConnection().getContentType().contains("html")) {
            if (dl.getConnection().getResponseCode() == 503 && dl.getConnection().getHeaderFields("server").contains("nginx")) {
                ...
                correctBR();
                checkServerErrors();
                if ("23764902a26fbd6345d3cc3533d1d5eb".equalsIgnoreCase(Hash.getMD5(cbr.toString()))) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
        } else {
