...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "filerev.cc" }, urls = { "https?://(www\\.)?filerev\\.cc/(?:embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class FilerevCc extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                     = "http://filerev.cc";
    private static final String            NICE_HOST                       = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty               = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                         = "(filerev\\.cc)";
     ...
     * mods: scanInfo[Small filesize RegEx change]<br />
     * limit-info:<br />
    ...
    public FilerevCc(PluginWrapper wrapper) {
        super(wrapper);
                ...
                if (fileInfo[1] == null) {
                    ...
                    if (fileInfo[1] == null) {
                        ...
                        if (fileInfo[1] == null) {
                            ...
                            // generic failover#1
                            if (fileInfo[1] == null) {
                                fileInfo[1] = new Regex(correctedBR, "(\\d+(?:\\.\\d+)? ?(KB|MB|GB))</font>\\]").getMatch(0);
                            }
                            // generic failover#2
                            if (fileInfo[1] == null) {
                                fileInfo[1] = new Regex(correctedBR, "(\\d+(?:\\.\\d+)? ?(?:B(?:ytes?)?))</font>\\]").getMatch(0);
                            }
                            // generic failover#3
                            if (fileInfo[1] == null) {
