...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "arivoweb.com" }, urls = { "https?://(?:www\\.)?arivoweb\\.com/[A-Za-z0-9]+" })
public class ArivowebCom extends antiDDoSForHost {
    public ArivowebCom(PluginWrapper wrapper) {
        super(wrapper);
     ...
     * YetiShareBasic Version 1.1.5-psp<br />
     * mods:<br />
     * limit-info:<br />
     * captchatype: 2018-04-11: null<br />
    ...
    /* Basic constants */
    private final String                   mainpage                                     = "http://arivoweb.com";
    private final String                   domains                                      = "(arivoweb\\.com)";
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            if (filename == null) {
                /* 2018-04-11: Special */
                filename = this.br.getRegex("name=\"description\" content=\"Information about ([^<>\"]+)\"").getMatch(0);
            }
            filesize = br.getRegex("(?:Filesize|Dateigröße|حجم الملف|Tamanho|Boyut):[\t\n\r ]*?</td>[\t\n\r ]*?<td(?: class=\"responsiveInfoTable\")?>([^<>\"]*?)<").getMatch(0);
            if (filesize == null) {
                filesize = br.getRegex("<small>([^<>\"]+)</small></h4>").getMatch(0);
            }
            try {
