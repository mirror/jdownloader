...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "exclusiveloader.com", "exclusivefaile.com" }, urls = { "https?://(www\\.)?(exclusivefaile\\.com|exclusiveloader\\.com)/((vid)?embed-)?[a-z0-9]{12}", "NOTUSED_4812jdmSDusd4aSAJD415uauadfjadafasadx" }, flags = { 2, 0 })
@SuppressWarnings("deprecation")
public class ExclusiveFaileCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://exclusiveloader.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(exclusivefaile\\.com|exclusiveloader\\.com)";
    ...
    // captchatype: 4dignum
    // other: no redirects
    // mods: fetchAccount regex for space (same as wallobit)
    ...
    private void checkServerErrors() throws NumberFormatException, PluginException {
        if (cbr.containsHTML("No file")) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error 'No file'", 10 * 60 * 1000l);
        } else if (cbr.containsHTML("Wrong IP")) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error 'Wrong IP'", 10 * 60 * 1000l);
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        String space[] = cbr.getRegex(">Used space.*?>([0-9\\.]+) of [0-9\\.]+ ?(KB|MB|GB|TB)?</").getRow(0);
        if (space == null || space.length == 0) {
            space = cbr.getRegex(">Used space.*?>([0-9\\.]+) ?(KB|MB|GB|TB)?</").getRow(0);
        }
        ...
        String availabletraffic = cbr.getRegex("Traffic available.*?:</TD><TD>([^<>\"']+)</").getMatch(0);
