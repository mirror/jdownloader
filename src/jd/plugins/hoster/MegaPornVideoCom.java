...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "megaporn.com/video" }, urls = { "http://[\\w\\.]*?(megaporn|megarotic|sexuploader)\\.com/(.*?v=|v/)[a-zA-Z0-9]+" }, flags = { 2 })
public class MegaPornVideoCom extends PluginForHost {

    private static String       agent     = RandomUserAgent.generate();
    private final static Object LOGINLOCK = new Object();

    public MegaPornVideoCom(PluginWrapper wrapper) {
        super(wrapper);
        this.enablePremium("http://www.megaporn.com/?c=premium");
    }

    @Override
    public String getAGBLink() {
        return "http://www.megaporn.com/?c=terms";
    }
    ...
    private void antiJDBlock(Browser br) {
        try {
            ...
            br.setCookie("http://www.megaporn.com", "l", "en");
        } catch (Throwable e) {
    ...
    public void login(final Account account, final boolean cookielogin) throws IOException, PluginException {
        ...
        synchronized (LOGINLOCK) {
            ...
            if (cookielogin && user != null) {
                this.br.setCookie("http://www.megaporn.com", "l", "en");
                this.br.setCookie("http://www.megaporn.com", "user", user);
                return;
            } else {
                if (account.getUser().trim().equalsIgnoreCase("cookie")) {
                    this.setBrowserExclusive();
                    this.br.setCookie("http://www.megaporn.com", "l", "en");
                    this.br.setCookie("http://www.megaporn.com", "user", account.getPass());
                    this.br.getPage("http://www.megaporn.com/");
                } else {
                    this.setBrowserExclusive();
                    this.br.setCookie("http://www.megaporn.com", "l", "en");
                    this.br.getPage("http://www.megaporn.com/?s=account");
                    this.br.postPage("http://www.megaporn.com/?s=account", "login=1&username=" + Encoding.urlEncode(account.getUser()) + "&password=" + Encoding.urlEncode(account.getPass()));
                }
                user = this.br.getCookie("http://www.megaporn.com", "user");
                this.br.setCookie("http://www.megaporn.com", "user", user);
    ...
    public boolean isPremium(final Account account, final Browser br, final boolean refresh, boolean cloneBrowser) throws IOException {
        synchronized (LOGINLOCK) {
            ...
            if (account.getBooleanProperty("typeknown", false) == false || refresh) {
                ...
                brc.getPage("http://www.megaporn.com/?c=account");
    ...
    public void handlePremium(DownloadLink downloadLink, Account account) throws Exception {
        ...
        brc.getPage("http://www.megaporn.com/video/xml/player_login.php?u=" + br.getCookie("http://www.megaporn.com", "user") + "&v=" + getDownloadID(downloadLink));
    ...
    public void handleFree(DownloadLink link) throws Exception {
        ...
        Browser brc = br.cloneBrowser();
        brc.getPage("http://www.megaporn.com/video/xml/videolink.php?v=" + ID);
        if (brc.containsHTML("hd=\"1\"")) {
            ...
            url = "http://www" + s + ".megaporn.com/files/" + decrypt(un, Integer.parseInt(k1), Integer.parseInt(k2)) + "/";
        }
    ...
    public AvailableStatus requestFileInformation(DownloadLink parameter) throws Exception {
        ...
        Browser brc = br.cloneBrowser();
        brc.getPage("http://www.megaporn.com/video/xml/videolink.php?v=" + getDownloadID(parameter));
    ...
    /*
     * decrypts the crypted link from megaporn un = input
     */
