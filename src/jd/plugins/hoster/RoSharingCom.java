...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "rosharing.com" }, urls = { "https?://(www\\.)?(rosharing\\.com|roshare\\.info)/(embed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class RoSharingCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                  = "http://rosharing.com";
    private static final String            NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                      = "(rosharing\\.com|roshare\\.info)";
    ...
    // mods: waitTime[Added another RegEx, added extra waittime]
    // limit-info:
    // protocol: no https
    // captchatype: null
    // other:
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "class=\"timme2\" id=\"[a-z0-9]+\">(\\d+)</span>").getMatch(0);
        }
        if (ttt != null) {
            int wait = Integer.parseInt(ttt) + 10;
            wait -= passedTime;
