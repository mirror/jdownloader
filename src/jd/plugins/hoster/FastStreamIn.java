...
public class FastStreamIn extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://faststream.in";
    ...
    public FastStreamIn(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        } else if (!br.getURL().contains("faststream.in/")) {
            logger.info("File offline as old domain redirects to new one without old content");
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        final String fuid = new Regex(downloadLink.getDownloadURL(), "/([A-Za-z0-9]{12})$").getMatch(0);
        /* Do they provide video hosting? */
        if (dllink == null && VIDEOHOSTER_2) {
            try {
                logger.info("Trying to get link via vidembed_2");
                final Browser brv = br.cloneBrowser();
                brv.getPage("/embed-" + fuid + ".html");
                dllink = brv.getRegex("file: \"(https?://[^<>\"]*?)\"").getMatch(0);
                if (dllink == null) {
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            dllink = new Regex(correctedBR, "\"(http://\\d+\\.\\d+\\.\\d+\\.\\d+:\\d+/[a-z0-9]+/[^<>\"/]*?)\"").getMatch(0);
            if (dllink == null) {
