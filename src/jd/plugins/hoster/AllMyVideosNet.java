...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "allmyvideos.net" }, urls = { "https?://(www\\.)?allmyvideos\\.net/[a-z0-9]{12}" }, flags = { 0 })
public class AllMyVideosNet extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        String filename = new Regex(correctedBR, "copy\\(this\\);.+>(.+) \\- [\\d\\.]+ (KB|MB|GB)</a></textarea>[\r\n\t ]+</div>").getMatch(0);
        if (filename == null) {
