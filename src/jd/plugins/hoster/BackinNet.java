...
public class BackinNet extends antiDDoSForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        if (br.getRequest().getHttpConnection().getResponseCode() == 403) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Blocked?", 5 * 60 * 1000l);
        }
    ...
    private void checkServerErrors() throws NumberFormatException, PluginException {
        ...
        if (cbr.containsHTML("Expired")) {
            throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error 'Expired'", 30 * 60 * 1000l);
        }
    }
    ...
    private Form captchaForm(DownloadLink downloadLink, Form form) throws Exception {
        ...
        if (form.containsHTML(";background:#(?:ccc|ggg);[^>]*text-align:") || form.hasInputFieldByName("code")) {
            ...
            if (letters == null || letters.length == 0) {
                letters = form.getRegex("<span style='.*?position:absolute;padding-left:(\\d+)px;padding-top:\\d+px;'>(&#\\d+;)</span>").getMatches();
                if (letters == null || letters.length == 0) {
