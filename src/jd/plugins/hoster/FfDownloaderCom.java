...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "ffdownloader.com" }, urls = { "" })
public class FfDownloaderCom extends PluginForHost {
    ...
    public FfDownloaderCom(PluginWrapper wrapper) {
        ...
        this.enablePremium("https://ffdownloader.com/premium");
    }

    @Override
    public String getAGBLink() {
        return "https://ffdownloader.com/rule";
    }
    ...
    public void handleMultiHost(final DownloadLink link, final Account account) throws Exception {
        ...
        /* request Download */
        this.login(account);
        br.postPage("https://ffdownloader.com/api/v1/directLink", "_format=json&scheme=https&link=" + Encoding.urlEncode(link.getDownloadURL()));
        handleAPIErrors(br, account, link);
        String dllink = br.getRegex("\"direct_link\":\"(http[^\"]+)").getMatch(0);
        if (dllink == null) {
    ...
    private void handleAPIErrors(final Browser br, final Account account, final DownloadLink downloadLink) throws PluginException {
        ...
        try {
            ...
            case 200:
                ...
                return;
            case 204:
                statusMessage = "Hoster not possible";
                /* only disable plugin for this link */
                throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, statusMessage, 60 * 60 * 1000l);
            case 401:
