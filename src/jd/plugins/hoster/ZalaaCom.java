...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "zalaa.com" }, urls = { "https?://(www\\.)?zalaa\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class ZalaaCom extends PluginForHost {

    private String              correctedBR         = "";
    private static final String PASSWORDTEXT        = "(<br><b>Password:</b> <input|<br><b>Passwort:</b> <input)";
    private static final String COOKIE_HOST         = "http://www.zalaa.com";
    ...
    // no https.
    // mods: filesize, free download postPage fields,
    // extra download step, decodeDownloadLink regex.
    ...
    public ZalaaCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        doSomething();
        if (new Regex(correctedBR, Pattern.compile("(File Not Found|This file has been removed due to)", Pattern.CASE_INSENSITIVE)).matches()) {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, boolean getLinkWithoutLogin) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            if (correctedBR.contains("\"ipcount_val\"")) {
                br.postPage(downloadLink.getDownloadURL(), "ipcount_val=10&op=download2&usr_login=&id=" + new Regex(downloadLink.getDownloadURL(), ".+/([A-Za-z0-9]{12})").getMatch(0) + "&fname=" + downloadLink.getName() + "&referer=&method_free=\"Slow access\"");
                doSomething();
                checkErrors(downloadLink, false, passCode);
            }
        ...
        if (dllink == null) {
            Form dlForm = br.getFormbyProperty("name", "soho");
            // if (dlForm == null) throw new
