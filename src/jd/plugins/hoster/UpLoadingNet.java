...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "up-loading.net" }, urls = { "http://(www\\.)?up\\-loading\\.net/[A-Za-z0-9]+" }, flags = { 0 })
public class UpLoadingNet extends PluginForHost {

    public UpLoadingNet(PluginWrapper wrapper) {
        super(wrapper);
    }

    // For sites which use this script: http://www.yetishare.com/
    // YetiShareBasic Version 0.2.9-psp
    // mods: doFree[Added additional dllink RegEx]
    // protocol: no https
    // captchatype: null
    ...
    /* Other constants */
    private final String         MAINPAGE                                     = "http://up-loading.net";
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resume, final int maxchunks) throws Exception, PluginException {
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            if (continue_link == null) {
                continue_link = br.getRegex("\"(https?://(www\\.)?up\\-loading\\.net/[A-Za-z0-9]+\\?download_token=[^<>\"]*?)\"").getMatch(0);
            }
