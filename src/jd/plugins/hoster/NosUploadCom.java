...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "nosupload.com" }, urls = { "https?://(www\\.)?nosupload\\.com/(\\?d=)?[a-z0-9]{12}" }, flags = { 0 })
public class NosUploadCom extends PluginForHost {
    ...
    // mods: waitTime, null referrer download2
    ...
    public void correctDownloadLink(DownloadLink link) {
        link.setUrlDownload(link.getDownloadURL().replace("https://", "http://").replace("/?d=", "/"));
    }
    ...
    public NosUploadCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            dllink = new Regex(correctedBR, "dirtyOrangeButton.*?href=\"(https?://.*?)\">Download").getMatch(0);
        }
        if (dllink == null) {
