...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "filesharing88.net" }, urls = { "http://(www\\.)?filesharing88\\.net/public/(?!(faq|register|login|terms|report_file|aboutus|contact|forgot|help|inthenews|privacy|upgrade)\\.html)[a-z0-9]+(?!~|%7Ef)" }, flags = { 2 })
public class FileSharing88Net extends PluginForHost {

    public FileSharing88Net(PluginWrapper wrapper) {
        super(wrapper);
        this.enablePremium(MAINPAGE + "/public/register.html");
    }
    ...
    private final String  MAINPAGE  = "http://filesharing88.net";
    ...
    private void login(Account account, boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                br.setFollowRedirects(true);
                br.postPage("http://www." + this.getHost() + "/public/login.html", "submit=Login&submitme=1&loginUsername=" + Encoding.urlEncode(account.getUser()) + "&loginPassword=" + Encoding.urlEncode(account.getPass()));
                ...
                if (br.containsHTML("/upgrade\\.html\">upgrade account</a>") || !br.containsHTML("/upgrade\\.html\">extend account</a>")) {
    ...
    public AccountInfo fetchAccountInfo(Account account) throws Exception {
        ...
        br.getPage("http://www." + this.getHost() + "/public/upgrade.html");
