...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "filepile.eu" }, urls = { "https?://(www\\.)?filepile\\.eu/[A-Za-z0-9]+" }, flags = { 2 })
public class FilePileEu extends PluginForHost {

    public FilePileEu(PluginWrapper wrapper) {
        super(wrapper);
        this.enablePremium(mainpage + "/upgrade." + type);
    }

    // For sites which use this script: http://www.yetishare.com/
    // YetiShareBasic Version 0.5.0-psp
    // mods:
    // limit-info: untested, finallinks timed out --> Set default limits
    // protocol: no https
    // captchatype: null
    // other: old domain: redfile.eu (XFS script)
    ...
    /* Basic constants */
    private final String         mainpage                                     = "http://filepile.eu";
    private final String         domains                                      = "(filepile\\.eu)";
        ...
        synchronized (LOCK) {
            try {
                ...
                if (useOldLoginMethod) {
                    br.postPage("http://" + this.getProtocol() + "/login." + type, "submit=Login&submitme=1&loginUsername=" + Encoding.urlEncode(account.getUser()) + "&loginPassword=" + Encoding.urlEncode(account.getPass()));
