...
@HostPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "nosvideo.com" }, urls = { "https?://(?:www\\.)?(?:nosvideo|noslocker)\\.com/(?:embed\\-)?[a-z0-9]{12,}" }, flags = { 0 })
public class NosVideoCom extends PluginForHost {
    ...
    private static final String            COOKIE_HOST                        = "http://nosvideo.com";
    private static final String            NICE_HOST                          = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String            NICE_HOSTproperty                  = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    /* domain names used within download links */
    private static final String            DOMAINS                            = "(nosvideo\\.com|noslocker\\.com)";
     ...
     * protocol: no https, using rtmp for download<br />
     * captchatype: null<br />
     * other:<br />
     */
    ...
    private void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink.contains(".smil")) {
            getPage(dllink);
            final String base = new Regex(correctedBR, "base=\"(rtmp://[^<>\"]+)\"").getMatch(0);
            final String playpath = new Regex(correctedBR, "(mp4:[^<>\"]+)\"").getMatch(0);
            if (base == null || playpath == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
            dl = new RTMPDownload(this, downloadLink, base);
            /* Setup rtmp connection */
            final jd.network.rtmp.url.RtmpUrlConnection rtmp = ((RTMPDownload) dl).getRtmpConnection();
            rtmp.setUrl(base);
            rtmp.setPlayPath(playpath);
            rtmp.setApp("vod_yeti/");
