...
public class SockShareCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (br.getURL().contains("sockshare.com/?404") || br.containsHTML("(>404 Not Found<|>This file doesn\\'t exist, or has been removed|<title>Share Files Easily on SockShare</title>)")) {
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
        Regex fileInfo = br.getRegex("<h1>(.*?)<strong>\\( (.*?) \\)</strong></h1>");
        String filename = fileInfo.getMatch(0);
        if (filename == null) {
            filename = br.getRegex("<title>(.*?) \\| SockShare</title>").getMatch(0);
        }
        String filesize = fileInfo.getMatch(1);
        if (filename == null || filesize == null) {
            if (br.containsHTML(">You have exceeded the daily stream limit for your country")) {
                return AvailableStatus.UNCHECKABLE;
            }
            if (br.containsHTML("You can wait until tomorrow, or get a")) {
                return AvailableStatus.UNCHECKABLE;
            }
            throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
        }
        if (!new Regex(filename, "\\.[0-9a-z]{0,4}$").matches()) {
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        ...
        if (freeform.containsHTML("/include/captcha")) {
            ...
            if (captcha != null) {
                freeform.put("captcha_code", Encoding.urlEncode(captcha));
            }
        }
        ...
        synchronized (LOCK) {
            try {
                ...
                if (br.containsHTML("captcha.php\\?")) {
                    ...
                    if (captcha != null) {
                        login.put("captcha_code", Encoding.urlEncode(captcha));
                    }
                }
                ...
                br.getPage("http://www.sockshare.com/profile.php?pro");
                proActive = br.getRegex("Pro  ?Status</?[^>]+>[\r\n\t ]+<[^>]+>(Active)").getMatch(0);
                if (br.containsHTML("<td>Free Account \\- <strong><a href=\"/gopro\\.php\\?upgrade\"")) {
