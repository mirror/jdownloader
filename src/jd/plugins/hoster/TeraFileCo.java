...
public class TeraFileCo extends PluginForHost {
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        // at this stage they have commented out the submit button for free download && we add it back, get captcha, download servers never
        // work. see https://board.jdownloader.org/showthread.php?t=63452
        if (true) {
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                if (!new Regex(correctedBR, "class=\"user\">Premium: <a").matches()) {
        ...
        try {
            ...
            if (config.getBooleanProperty("premAdShown", Boolean.FALSE) == false) {
                if (config.getProperty("premAdShown2") == null) {
                    File checkFile = JDUtilities.getResourceFile("tmp/terraf");
                    if (!checkFile.exists()) {
                        checkFile.mkdirs();
                        showFreeDialog("terafile.co");
                    }
                } else {
                    config = null;
                }
