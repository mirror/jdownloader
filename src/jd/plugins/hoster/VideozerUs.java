...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "videozer.us" }, urls = { "https?://(www\\.)?videozer\\.us/files/[A-Za-z0-9]+|http://(www\\.)?videozerdecrypted\\.us/[A-Za-z0-9\\-_]+\\.html" }, flags = { 2 })
public class VideozerUs extends PluginForHost {
    ...
    private static final String  type_filehost                                = "https?://(www\\.)?videozer\\.us/files/[A-Za-z0-9]+";
    private static final String  type_videohost                               = "https?://(www\\.)?videozer\\.us/[A-Za-z0-9\\-_]+\\.html";
    private String               DLLINK                                       = null;

    /* Basic constants */
    private final String         mainpage                                     = "http://videozer.us";
    private final String         domains                                      = "(videozer\\.us)";
    ...
    private static final int     directlinkfound_WAIT_SECONDS                 = 5;
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (link.getDownloadURL().matches(type_filehost)) {
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resume, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        if (downloadLink.getDownloadURL().matches(type_filehost)) {
    ...
    public void handlePremium(final DownloadLink link, final Account account) throws Exception {
        ...
        login(account, false);
        if (account.getBooleanProperty("free", false)) {
            if (link.getDownloadURL().matches(type_filehost) && !available_CHECK_OVER_INFO_PAGE) {
                br.getPage(link.getDownloadURL());
            }
