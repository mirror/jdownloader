...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "thefile.me" }, urls = { "https?://(www\\.)?thefile\\.me/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class TheFileMe extends PluginForHost {
    ...
    private static final String  DOMAINS                      = "(thefile\\.me)";
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resumable, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            if (download1 != null) {
                ...
                sendForm(download1);
                checkErrors(downloadLink, false);
                final String fname = new Regex(correctedBR, "\\&amp;fn=([^<>\"]*?)\\&fs=").getMatch(0);
                if (fname != null) {
    ...
    private void fixFilename(final DownloadLink downloadLink) {
        ...
        if (orgName.equalsIgnoreCase(fuid.toLowerCase())) {
            ...
            FFN = orgName + servExt;
        } else if (inValidate(orgExt) && !inValidate(servExt)) {
            FFN = servNameExt;
        } else {
            FFN = orgNameExt;
        }
    ...
    private static boolean checkSsl() {
        return SubConfiguration.getConfig("thefile.me").getBooleanProperty(SSL_CONNECTION, false);
    }

    private void setConfigElements() {
        getConfig().addEntry(new ConfigEntry(ConfigContainer.TYPE_CHECKBOX, this.getPluginConfig(), SSL_CONNECTION, JDL.L("plugins.hoster.TheFileMe.preferSSL", "Use Secure Communication over SSL (HTTPS://)")).setDefaultValue(false));
    }
