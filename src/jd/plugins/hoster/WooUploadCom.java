...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "wooupload.com" }, urls = { "https?://(www\\.)?wooupload\\.com/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class WooUploadCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://wooupload.com";
    // domain names used within download links.
    private final String               DOMAINS                      = "(wooupload\\.com)";
    ...
    // other: no redirects
    // mods: download1 manual input, free detection (free accounts have expire time, go figure)
     ...
     * */
    public WooUploadCom(PluginWrapper wrapper) {
        super(wrapper);
        setConfigElements();
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        String availabletraffic = cbr.getRegex("Traffic available.*?:</TD.*?><b>([^<>\"']+)</b>").getMatch(0);
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                if (cbr.containsHTML("Upgrade to premium</a></li>") || !cbr.containsHTML("(Premium(\\-| )Account expire|>Renew premium<)")) {
