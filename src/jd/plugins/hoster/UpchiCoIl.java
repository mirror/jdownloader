...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "upchi.co.il" }, urls = { "https?://(www\\.)?upchi\\.co\\.il/[a-z0-9]{12}" }, flags = { 0 })
public class UpchiCoIl extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        doSomething();
        if (new Regex(correctedBR, Pattern.compile("(No such file|>File Not Found<|>The file was removed by|Reason (of|for) deletion:\n|Possible causes of this error could be:)", Pattern.CASE_INSENSITIVE)).matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
