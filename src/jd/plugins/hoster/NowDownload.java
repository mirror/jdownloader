...
@HostPlugin(revision = "$Revision: 20521 $", interfaceVersion = 2, names = { "NowDownload" }, urls = { "http://(www\\.)?nowdownload\\.(eu|co|ch)/dl(\\d+)?/[a-z0-9]+" }, flags = { 0 })
public class NowDownload extends PluginForHost {
    ...
    public boolean testDomain(DownloadLink link) {
        try {
            ...
            if (br.containsHTML(">This file does not exist")) return false;
    ...
    public void correctDownloadLink(DownloadLink link) {
        ...
        link.setUrlDownload(link.getDownloadURL().replace("nowdownload.ch/", "nowdownload.co/"));
        ...
        if (!testDomain(link)) {
            ...
            if (!testDomain(link)) {
                link.setUrlDownload(link.getDownloadURL().replace("nowdownload.eu/", "nowdownload.ch/"));
