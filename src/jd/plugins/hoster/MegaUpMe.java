...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "megaup.me" }, urls = { "http://(www\\.)?megaup\\.me/\\?d=[A-Z0-9]+" }, flags = { 2 })
public class MegaUpMe extends PluginForHost {

    public MegaUpMe(PluginWrapper wrapper) {
        super(wrapper);
        this.enablePremium(COOKIE_HOST + "/en/register.php?g=3");
    }
    ...
    public AvailableStatus requestFileInformation(DownloadLink parameter) throws Exception {
        ...
        String filename = br.getRegex("class=\"float\\-left_dl\">[\t\n\r ]+<h2  style=\"font\\-size: 25pt; font\\-weight: bold;\">([^<>\"]*?)</h2>").getMatch(0);
        String filesize = br.getRegex("style=\"background\\-color: #CCC;\" />([^<>\"]*?)</p>").getMatch(0);
    ...
    private String findLink(final Browser br) throws Exception {
        String finalLink = br.getRegex("(http://[a-z0-9\\-\\.]{5,30}/getfile\\.php\\?id=\\d+[^<>\"\\']*?)(\"|\\')").getMatch(0);
        if (finalLink == null) {
    ...
    public void login(Account account, boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                final Form form = br.getFormbyProperty("name", "loginfrm");
