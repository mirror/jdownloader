...
public class HulkFileCom extends PluginForHost {
    ...
    private final String               COOKIE_HOST                  = "http://hulkfile.eu";
    // domain names used within download links.
    private final String               DOMAINS                      = "(hulkfile\\.(com|eu))";
    ...
    // other: no redirects
    // mods: linkcheck(scaninfo && requestfileinfo) && waititme
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws Exception {
        ...
        getPage(downloadLink.getDownloadURL() + ".html");
        ...
        if (cbr.containsHTML("(\\$\\(\\'#not\\-found\\'\\)|No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|<li>The file (expired|deleted by (its owner|administration))|window\\.location = \"https?://(w(ww)?\\.)?" + DOMAINS + "/a-z0-9]{12}\\.html\")")) {
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            fileInfo[0] = cbr.getRegex("var filename = \"(.*?)\"").getMatch(0);
            if (inValidate(fileInfo[0])) {
        ...
        if (inValidate(fileInfo[1])) {
            fileInfo[1] = cbr.getRegex("var filesize = \"\\(?(.*?)\\)?\"").getMatch(0);
            if (inValidate(fileInfo[1])) {
    ...
    private void waitTime(final long timeBefore, final DownloadLink downloadLink) throws PluginException {
        /** Ticket Time */
        String ttt = cbr.getRegex("<span[^>]+id=\"[a-z0-9]+\">(\\d+)</span>").getMatch(0);
        if (inValidate(ttt)) {
