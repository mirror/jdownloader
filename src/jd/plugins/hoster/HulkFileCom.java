...
public class HulkFileCom extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, Pattern.compile("(No such file|>File Not Found<|>The file was removed by|Reason (of|for) deletion:\n|>This file is not found)", Pattern.CASE_INSENSITIVE)).matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        if (br.containsHTML("window\\.location = \"http://hulkfile\\.eu/[a-z0-9]{12}\\.html\"")) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        if (correctedBR.contains(MAINTENANCE)) {
            link.getLinkStatus().setStatusText(JDL.L("plugins.hoster.xfilesharingprobasic.undermaintenance", MAINTENANCEUSERTEXT));
            return AvailableStatus.TRUE;
        }
        String filename = new Regex(correctedBR, "var filename = \"(.*?)\"").getMatch(0);
        if (filename == null) filename = new Regex(correctedBR, "You have requested.*?https?://(www\\.)?" + this.getHost() + "/[A-Za-z0-9]{12}/(.*?)</font>").getMatch(1);
        if (filename == null) {
        ...
        String filesize = new Regex(correctedBR, "var filesize = \"(.*?)\"").getMatch(0);
        if (filesize == null) filesize = new Regex(correctedBR, "\\(([0-9]+ bytes)\\)").getMatch(0);
        if (filesize == null) {
    ...
    public void correctBR() throws NumberFormatException, PluginException {
        ...
        regexStuff.add("(<script type=\"text/javascript\">[\r\n\t ]+var from = 1;.*?else)");
