...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "ravishare.com" }, urls = { "http://(www\\.)?ravishare\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class RaviShareCom extends PluginForHost {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, boolean loggedIn) throws Exception, PluginException {
        ...
        if (freeform != null) {
            br.postPage(downloadLink.getDownloadURL(), "op=download1&usr_login=&id=" + new Regex(downloadLink.getDownloadURL(), "ravishare\\.com/([a-z0-9]{12})").getMatch(0) + "&fname=" + downloadLink.getName() + "&referer=&method_free=Free+Download");
            doSomething();
        }
    ...
    public void handleFree(DownloadLink downloadLink) throws Exception, PluginException {
        ...
        // More connections are possible but cause timeouts (for single
        // connections)
        doFree(downloadLink, true, -4, false);
    }
