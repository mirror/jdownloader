...
@HostPlugin(revision = "$Revision: 19496 $", interfaceVersion = 2, names = { "sanshare.com" }, urls = { "https?://(www\\.)?sanshare\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 0 })
public class SanShareCom extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private static final String        COOKIE_HOST                  = "http://sanshare.com";
    // domain names used within download links.
    private static final String        DOMAINS                      = "(sanshare\\.com)";
    ...
    private void decodeDownloadLink(final String s) {
        ...
        if (decoded != null) {
            dllink = new Regex(decoded, "name=\"src\"value=\"(.*?)\"").getMatch(0);
            if (dllink == null) {
                dllink = new Regex(decoded, "type=\"video/divx\"src=\"(.*?)\"").getMatch(0);
                if (dllink == null) {
                    dllink = new Regex(decoded, "\\.addVariable\\(\\'file\\',\\'(http://.*?)\\'\\)").getMatch(0);
                }
            }
        }
    }
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        if (ttt == null) {
            ttt = new Regex(correctedBR, "id=\"countdown_str\"[^>]+>Wait[^>]+>(\\d+)\\s?+</span>").getMatch(0);
            if (ttt == null) {
                ttt = new Regex(br, "id=\"countdown_str\">Please wait.+\">(\\d+)</span>").getMatch(0);
            }
        }
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        if (account.getBooleanProperty("nopremium")) {
            ...
            } else if (expireDay != null && !useAlternativeExpire) {
