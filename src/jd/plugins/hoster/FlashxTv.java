...
public class FlashxTv extends PluginForHost {
    ...
    public void correctDownloadLink(DownloadLink link) {
        if (link.getDownloadURL().matches(".+play\\.flashx\\.tv/player.+")) {
            ...
            if (hash != null) {
                link.setUrlDownload(link.getDownloadURL().replaceAll("http://play.flashx.tv/player/.+", "http://www.flashx.tv/video/" + hash + "/"));
            }
        }
    }
    ...
    private String getPlainData(String regex) {
        ...
        if (encrypted != null) {
            ...
            if (result != null) {
                decrypted = new Regex(result.toString(), regex).getMatch(0);
            }
        }
        return decrypted;
    }
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        ...
        if (next == null) {
            ...
            if (seclinkgk == null) {
                seclinkgk = br.getRegex(fx(0)).getMatch(1);
            }
            if (seclinkgk == null) {
                seclinkgk = getPlainData(fx(0));
            }
            if (seclinkgk == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
        ...
        if (thirdLink == null) {
            thirdLink = getPlainData(regex);
        }
        if (thirdLink == null || fp == null) {
            throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
        }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (br.containsHTML("(>Requested page not found|>404 Error<|>Video not found, deleted or abused, sorry\\!<|>Video not found or deleted<)")) {
