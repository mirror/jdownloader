...
public class UpfileMobi extends PluginForHost {
    ...
    public void handleFree(final DownloadLink downloadLink) throws Exception, PluginException {
        ...
        if (ad_sht != null) br.cloneBrowser().openGetConnection("http://upfile.mobi/" + ad_sht);
        String dllink = br.getRegex("\"(index\\.php\\?page=download[^<>\"]*?)\"").getMatch(0);
        if (dllink == null) dllink = br.getRegex("href=\"(index[^<>\"]*?)\">Download File</a>").getMatch(0);
        ...
        if (dl.getConnection().getContentType().contains("html")) {
            br.followConnection();
            if (br.containsHTML("Download key is incorrect")) throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error: 'Download key is incorrect'", 10 * 30 * 1000l);
