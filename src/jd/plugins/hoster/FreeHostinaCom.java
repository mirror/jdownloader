...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "freehostina.com" }, urls = { "https?://(www\\.)?freehostina\\.com/(vidembed\\-)?[a-z0-9]{12}" }, flags = { 2 })
public class FreeHostinaCom extends PluginForHost {

    private String               correctedBR                  = "";
    private String               passCode                     = null;
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    // primary website url, take note of redirects
    private static final String  COOKIE_HOST                  = "http://freehostina.com";
    private static final String  NICE_HOST                    = COOKIE_HOST.replaceAll("(https://|http://)", "");
    private static final String  NICE_HOSTproperty            = COOKIE_HOST.replaceAll("(https://|http://|\\.|\\-)", "");
    // domain names used within download links.
    private static final String  DOMAINS                      = "(freehostina\\.com)";
    ...
    // mods: scanInfo [Changed last filename regex]
    ...
    public FreeHostinaCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "(No such file|>File Not Found<|>The file was removed by|Reason for deletion:\n|files\\&nbsp;</h1></td>)").matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
                        ...
                        if (fileInfo[0] == null) {
                            ...
                            if (fileInfo[0] == null) {
                                ...
                                if (fileInfo[0] == null) {
                                    ...
                                    fileInfo[0] = new Regex(correctedBR, "</span>File Name: ([^<>\"]*?)</div>").getMatch(0);
                                }
                            }
                        }
                    }
    ...
    private void waitTime(long timeBefore, final DownloadLink downloadLink) throws PluginException {
        ...
        /** Ticket Time */
        final String ttt = new Regex(correctedBR, "id=\"countdown\"> (\\d+) </span> <span class=\"style\\d+\">seconds</span>").getMatch(0);
        if (ttt != null) {
    ...
    private void login(final Account account, final boolean force) throws Exception {
        synchronized (LOCK) {
            try {
                ...
                // loginform.put("image.x", "");
                sendForm(loginform);
                if (br.getCookie(COOKIE_HOST, "xfss") == null || br.getCookie(COOKIE_HOST, "xfss").equals("")) {
                    if ("de".equalsIgnoreCase(lang)) {
