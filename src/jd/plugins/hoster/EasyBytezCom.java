...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "easybytez.com" }, urls = { "http://(www\\.)?easybytez\\.com/(?!users)[a-z0-9]{12}" }, flags = { 2 })
public class EasyBytezCom extends PluginForHost {

    private String              BRBEFORE            = "";
    private static final String PASSWORDTEXT        = "(<br><b>Password:</b> <input|<br><b>Passwort:</b> <input)";
    private static final String COOKIE_HOST         = "http://easybytez.com";
    ...
    public void checkErrors(DownloadLink theLink, boolean checkAll, String passCode) throws NumberFormatException, PluginException {
        ...
        if (new Regex(BRBEFORE, "( can download files up to |Upgrade your account to download bigger files|>This file is available for Premium Users only|>Upgrade your account to download larger files|>The file You requested  reached max downloads limit for Free Users|Please Buy Premium To download this file<|This file reached max downloads limit)").matches()) {
        ...
        try {
            ...
            if (config.getBooleanProperty("premAdShown", Boolean.FALSE) == false) {
                if (config.getProperty("premAdShown2") == null) {
                    File checkFile = JDUtilities.getResourceFile("tmp/ebtmp");
                    if (!checkFile.exists()) {
                        checkFile.mkdirs();
                        showFreeDialog("easybytez.com");
                    }
                } else {
                    config = null;
                }
