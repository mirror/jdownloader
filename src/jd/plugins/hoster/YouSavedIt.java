...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "yousaved.it" }, urls = { "http://(www\\.)?yousaved\\.it/(?!faq|register|login|terms|report_file|index)[a-z0-9]+" }, flags = { 0 })
public class YouSavedIt extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (br.getURL().contains("/error." + TYPE) || br.getURL().contains("/index." + TYPE)) {
            throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        }
