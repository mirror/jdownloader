...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "speed4up.com" }, urls = { "https?://(?:www\\.)?speed4up\\.com/[A-Za-z0-9]+" })
public class Speed4upCom extends antiDDoSForHost {
    public Speed4upCom(PluginWrapper wrapper) {
        super(wrapper);
     ...
     * mods: getContinueLink<br />
     * limit-info: 2018-08-30: No limits<br />
    ...
    public void handleDownload(final DownloadLink link, final Account acc, final boolean resume, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (continue_link != null) {
            ...
            for (int i = 1; i <= 5; i++) {
                ...
                /* 2018-08-30: Special: Hidden pre-download-form */
                final String pt_value = br.getRegex("hidden name=\\'pt\\' value=\\'([^<>\"\\']+)\\'").getMatch(0);
                if (pt_value != null) {
                    continue_form = new Form();
                    continue_form.setMethod(MethodType.POST);
                    continue_form.setAction(this.br.getURL());
                    continue_form.put("pt", Encoding.urlEncode(pt_value));
                } else if (!StringUtils.isEmpty(continue_link)) {
                    continue_form = new Form();
                ...
                if (isDownloadlink(continue_link)) {
                    ...
                    waitTime(link, timeBeforeCaptchaInput, skipWaittime);
                    continue_form.put("recaptcha_challenge_field", rc.getChallenge());
                    continue_form.put("recaptcha_response_field", c);
                    dl = jd.plugins.BrowserAdapter.openDownload(br, link, continue_form, resume, maxchunks);
    ...
    private String getContinueLink() {
        ...
        /* 2018-08-30: Special: Ads/URL-Shortener which is skippable */
        final String urlpart = br.getRegex("api=[a-f0-9]+\\&url=(speed4up\\.com/[^<>\"\\']+)\\'").getMatch(0);
        if (urlpart != null) {
            /* Add 'download_token=' for function isDownloadlink */
            continue_link = "http://" + urlpart + "download_token=";
        }
