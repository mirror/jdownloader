...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "shareblue.eu" }, urls = { "https?://(www\\.)?shareblue\\.eu/((vid)?embed-)?[a-z0-9]{12}" }, flags = { 2 })
@SuppressWarnings("deprecation")
public class ShareBlueEu extends PluginForHost {

    // Site Setters
    // primary website url, take note of redirects
    private final String               COOKIE_HOST                  = "http://shareblue.eu";
    // domain names used within download links.
    private final String               DOMAINS                      = "(shareblue\\.eu)";
    ...
    private String[] scanInfo(final DownloadLink downloadLink, final String[] fileInfo) {
        ...
        if (inValidate(fileInfo[0])) {
            fileInfo[0] = cbr.getRegex(">File: <font style=\"[^<>\"/]+\">([^<>\"]*?)</font>").getMatch(0);
        }
        if (inValidate(fileInfo[1])) {
