...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "gsinfinite.com", "shareblue.eu" }, urls = { "https?://(www\\.)?gsinfinite\\.com/[A-Za-z0-9]+", "REGEX_NOT_POSSIBLE_RANDOM-asdfasdfsadfsdgfd32423" }, flags = { 2, 0 })
public class ShareBlueEu extends PluginForHost {

    public ShareBlueEu(PluginWrapper wrapper) {
        super(wrapper);
    ...
    // mods: added handling for directlinks
    // limit-info:
    // protocol: no https
    // captchatype: null recaptcha solvemedia
    // other:

    @Override
    public String rewriteHost(String host) {
        if ("shareblue.eu".equals(getHost())) {
            if (host == null || "shareblue.eu".equals(host)) {
                return "gsinfinite.com";
            }
        }
    ...
    /* Basic constants */
    private final String         mainpage                                     = "http://gsinfinite.com";
    private final String         domains                                      = "(gsinfinite\\.com)";
    ...
    public void doFree(final DownloadLink downloadLink, final boolean resume, final int maxchunks, final String directlinkproperty) throws Exception, PluginException {
        ...
        if (continue_link != null) {
            ...
            if (available_CHECK_OVER_INFO_PAGE) {
                /* Directlink */
                continue_link = downloadLink.getDownloadURL();
            }
            if (continue_link != null) {
                dl = jd.plugins.BrowserAdapter.openDownload(br, downloadLink, continue_link, resume, maxchunks);
            } else {
