...
//TODO Put this plugin with the .com version to have not duplicate code but this change will have a lot of impact because of the MAINPAGE value that is set at the beginning, it should be change depends on the link
@HostPlugin(revision = "$Revision: 19190 $", interfaceVersion = 2, names = { "depositfiles.org" }, urls = { "https?://(www\\.)?(depositfiles\\.org|dfiles\\.(eu|ru))(/\\w{1,3})?/files/[\\w]+" }, flags = { 2 })
public class DepositFilesOrg extends PluginForHost {
    ...
    public static final String   DOMAINS                  = "(depositfiles\\.org|dfiles\\.(eu|ru))";
    ...
    static {
        ...
        if (MAINPAGE == null) {
            ...
            try {
                ...
                testBr.getPage("http://depositfiles.org");
    ...
    public DepositFilesOrg(final PluginWrapper wrapper) {
        super(wrapper);
    ...
    public void correctDownloadLink(final DownloadLink link) {
        if (!link.getDownloadURL().contains(MAINPAGE.replaceAll("https?://(www\\.)?", ""))) {
            ...
            link.setUrlDownload(link.getDownloadURL().replaceAll("(dfiles\\.(eu|ru)|depositfiles\\.org)(/.*?)?/files", MAINPAGE.replaceAll("https?://(www\\.)?", "") + "/de/files"));
        }
    }
        ...
        try {
            ...
            if (config.getBooleanProperty("premAdShown", Boolean.FALSE) == false) {
                if (config.getProperty("premAdShown2") == null) {
                    ...
                    if (!checkFile.exists()) {
                        checkFile.mkdirs();
                        showFreeDialog("depositfiles.org");
                    }
                } else {
                    config = null;
                }
    ...
    public void checkErrors() throws NumberFormatException, PluginException {
        ...
        if (br.containsHTML("but all downloading slots for your country")) {
            ...
            throw new PluginException(LinkStatus.ERROR_HOSTER_TEMPORARILY_UNAVAILABLE, JDL.L("plugins.hoster.depositfilesorg.errors.allslotsbusy", "All download slots for your country are busy"), 1 * 60 * 1000l);
        }
        /* already loading */
    ...
    public AccountInfo fetchAccountInfo(final Account account) throws Exception {
        ...
        synchronized (LOCK) {
            try {
                ...
                ai.setStatus(JDL.L("plugins.hoster.depositfilesorg.accountbad", "Account expired or not valid."));
            ...
            if (isFreeAccount(account, true)) {
                ...
                ai.setStatus(JDL.L("plugins.hoster.depositfilesorg.accountokfree", "Free Account is ok"));
            ...
            ai.setStatus(JDL.L("plugins.hoster.depositfilesorg.accountok", "Premium Account is ok"));
            Date date;
            account.setValid(true);
            try {
    ...
    private String getDonloadManagerVersion() {
        ...
        try {
            ...
            brc.getPage("http://system.depositfiles.org/api/get_downloader_version.php");
        } catch (Throwable e) {
            return null;
        }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink downloadLink) throws IOException, PluginException {
        ...
        if (br.containsHTML("<strong>Achtung! Sie haben ein Limit")) {
            downloadLink.getLinkStatus().setStatusText(JDL.L("plugins.hoster.depositfilesorg.errors.limitreached", "Download limit reached"));
