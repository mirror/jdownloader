...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "flashdrive.it" }, urls = { "https?://(www\\.)?flashdrive\\.it/(upload/)?[a-z0-9]{12}" }, flags = { 0 })
public class FlashDriveIt extends PluginForHost {

    private String               correctedBR                  = "";
    private static final String  PASSWORDTEXT                 = "<br><b>Passwor(d|t):</b> <input";
    private final String         COOKIE_HOST                  = "http://flashdrive.it";
    ...
    // premium account:
    // protocol: no https
    // captchatype: 4dignum(manic)
    // other: redirects to www.

    @Override
    public void correctDownloadLink(DownloadLink link) {
        link.setUrlDownload(link.getDownloadURL().replaceAll("https?://(www\\.)?flashdrive\\.it", "http://www.flashdrive.it"));
        if (link.getDownloadURL().matches(".+flashdrive\\.it/[a-z0-9]{12}")) {
            String url = new Regex(link.getDownloadURL(), "(https?://[^/]+)").getMatch(0);
            String replace = new Regex(link.getDownloadURL(), "/([a-z0-9]{12})").getMatch(0);
            link.setUrlDownload(link.getDownloadURL().replaceAll(link.getDownloadURL(), url + "/upload/" + replace));
        }
    }
    ...
    public FlashDriveIt(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        if (new Regex(correctedBR, "The file you were looking for could not be found, sorry for any inconvenience|Possible causes of this error could be|Reason for deletion").matches()) throw new PluginException(LinkStatus.ERROR_FILE_NOT_FOUND);
        ...
        filename = filename.replaceAll("(</b>|<b>|\\.html|\\[Flashdrive\\.it\\])", "");
