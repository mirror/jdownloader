...
public class TinyfilesCom extends antiDDoSForHost {
    public TinyfilesCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public static String[] domains = new String[] { "tiny-files.com" };
        ...
        if (dllink == null) {
            ...
            for (int i = 0; i <= repeat; i++) {
                ...
                if (dl.getConnection().getContentType().contains("html")) {
                    ...
                    logger.warning("Form download failed");
                    br.followConnection();
                    correctBR();
                    checkServerErrors();
                } else {
    ...
    private String getFUIDFromURL(final DownloadLink dl) {
        try {
            /* 2019-01-30: Special */
            final String result = new Regex(new URL(dl.getPluginPatternMatcher()).getPath(), "/(?:embed\\-)?(.+)").getMatch(0);
            return result;
