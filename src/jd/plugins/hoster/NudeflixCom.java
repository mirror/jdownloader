...
public class NudeflixCom extends PluginForHost {
    ...
    public String getAGBLink() {
        return "http://www.nudeflix.com/terms-of-use";
    }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        final String url_filename = new Regex(link.getDownloadURL(), "nudeflix\\.com/(.+)").getMatch(0).replace("-", " ").replace("/", "_");
        String filename = br.getRegex("<title>([^<>\"]+)\\- Nudeflix[\t\n\r ]*?</title>").getMatch(0);
        if (filename == null) {
            filename = url_filename;
        }
        dllink = br.getRegex("<video data\\-src=\"(http[^<>\"]+)\"></video>").getMatch(0);
        if (dllink == null) {
