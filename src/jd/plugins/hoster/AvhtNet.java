...
@HostPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "avht.net" }, urls = { "https?://(?:www\\.)?(?:avht|tosix)\\.net/[A-Za-z0-9]+" }, flags = { 0 })
public class AvhtNet extends PluginForHost {

    public AvhtNet(PluginWrapper wrapper) {
        super(wrapper);
    ...
    /* Basic constants */
    private final String         mainpage                                     = "http://www.avht.net";
    private final String         domains                                      = "(avht\\.net|tosix\\.net)";
    ...
    public void correctDownloadLink(final DownloadLink link) {
        ...
        link.setUrlDownload(link.getDownloadURL().replace("tosix.net/", "avht.net/"));
    }
    ...
    public AvailableStatus requestFileInformation(final DownloadLink link) throws IOException, PluginException {
        ...
        if (available_CHECK_OVER_INFO_PAGE) {
            ...
            if (filename == null) {
                filename = this.br.getRegex("(?:Filename|Dateiname):[\t\n\r ]*?</td>[\t\n\r ]*?<td(?: class=\"responsiveInfoTable\")?>([^<>\"]*?)<").getMatch(0);
            }
