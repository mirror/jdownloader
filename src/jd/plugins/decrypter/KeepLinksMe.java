...
@DecrypterPlugin(revision = "$Revision$", interfaceVersion = 3, names = { "keeplinks.me", "safemy.link", "linkpacking.net" }, urls = { "https?://(www\\.)?keeplinks\\.(me|eu)/(p\\d*|d)/[a-z0-9]+", "http://(www\\.)?safemy\\.link/(p|d)/[a-z0-9]+", "http://(www\\.)?linkpacking\\.net/(p|d)/[a-z0-9]+" }, flags = { 0, 0, 0 })
