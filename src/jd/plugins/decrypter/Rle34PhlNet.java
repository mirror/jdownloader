...
public class Rle34PhlNet extends PluginForDecrypt {
	...
	public ArrayList<DownloadLink> decryptIt(CryptedLink param,
		...
		String[] links = br.getRegex("<br><a href=\\'(http://.*?)\\'>")
				.getColumn(0);
		if (links == null || links.length == 0)
			links = br
					.getRegex(
							"\\'(http://rule34\\-images\\.paheal\\.net/_images/[a-z9-8]+/.*?)\\'")
					.getColumn(0);
		if (links == null || links.length == 0)
			return null;
		for (String dl : links)
		...
		fp.setName(new Regex(parameter,
				"rule34\\.paheal\\.net/post/list/(.*?)/\\d+").getMatch(0));
