...
public class BangbrosCom extends PluginForDecrypt {
    ...
    protected DownloadLink createDownloadlink(String url, final String fid, final String quality) {
        ...
        return super.createDownloadlink(url, true);
    }
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        for (final String html_videourl : htmls_videourls) {
            ...
            final DownloadLink dl = this.createDownloadlink(videourl, fid, quality_url);
            dl.setName(title + "_" + quality_url + ext);
            decryptedLinks.add(dl);
        }
        if (cfg.getBooleanProperty("GRAB_photos", false)) {
            final String quality = "pictures";
            final DownloadLink dl = this.createDownloadlink(directurl_photos, fid, quality);
            dl.setName(title + "_" + quality + ".zip");
            decryptedLinks.add(dl);
        }
        if (cfg.getBooleanProperty("GRAB_screencaps", false)) {
            final String quality = "screencaps";
            final DownloadLink dl = this.createDownloadlink(directurl_screencaps, fid, quality);
            dl.setName(title + "_" + quality + ".zip");
            decryptedLinks.add(dl);
        }
