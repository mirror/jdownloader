...
@DecrypterPlugin(revision = "$Revision: 7139 $", interfaceVersion = 2, names = { "best-movies.us" }, urls = { "http://crypt\\.(best-movies\\.us|capcrypt\\.info)/go\\.php\\?id\\=\\d+"}, flags = { 0 })


public class BstMvs extends PluginForDecrypt {
    ...
    public BstMvs(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        for (int retrycounter = 1; (decryptedLinks.isEmpty()) && (retrycounter <= 3); retrycounter++) {
            ...
            if (br.getRegex(patternCaptcha_Needed).matches()) {
                ...
                if (time == null) {
                    UserIO.getInstance().requestConfirmDialog(0, JDL.L("jd.plugins.decrypt.BstMvs.CaptchaInputWrong", "Wrong Input, please enter time like this: 4:30"));
