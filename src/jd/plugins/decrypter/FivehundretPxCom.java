...
public class FivehundretPxCom extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink parameter, ProgressController progress) throws Exception {
        ...
        final String username = new Regex(parameter.toString(), this.getSupportedLinks()).getMatch(0);
        if (parameter.toString().matches(".+/galleries/.+")) {
            /* Crawl a gallery of a user */
            if (true) {
                /* TODO: 2020-09-09: Broken */
            ...
            br.getPage("https://api.500px.com/v1/users/" + curatorID + "/galleries/" + galleryName + "?include_user=true&include_cover=1&cover_size=2048");
            final String galleryID = br.getRegex("\"id\"\\s*:\\s*(\\d+)").getMatch(0);
            if (galleryID == null) {
                throw new PluginException(LinkStatus.ERROR_PLUGIN_DEFECT);
            }
            final String name = br.getRegex("\"name\"\\s*:\\s*\"(.*?)\"").getMatch(0);
            ...
            while (!isAbort()) {
                ...
                /* 2020-09-09: They're using a graphql API but we can keep using their "old" API! */
                ...
                for (final Map<String, Object> photo : photos) {
                    final Number id = (Number) photo.get("id");
                    String url = photo.get("url") != null ? (String) photo.get("url") : null;
                    if (StringUtils.isEmpty(url)) {
                        url = "/photo/" + id.intValue();
                    }
                    if (url.startsWith("/")) {
                    ...
                    if (StringUtils.isNotEmpty(photoName)) {
                        if (StringUtils.isNotEmpty(username)) {
                            dl.setName(username + " - " + photoName + ".jpg");
                        } else {
                            dl.setName(photoName + ".jpg");
                        }
                    }
