            ...
            if (ID != null) {
                ...
                for (String streams[] : br.getRegex("<formitaet basetype=\"([^\"]+)\" isDownload=\"[^\"]+\">(.*?)</formitaet>").getMatches()) {
                    ...
                    for (String stream[] : new Regex(streams[1], "<quality>([^<]+)</quality>.*?<url>([^<]+)<.*?<filesize>(\\d+)<").getMatches()) {
                        ...
                        if (stream[1].endsWith("inf.meta") && stream[1].contains("geostreaming") && stream[1].startsWith("http")) {
                            br.getPage(stream[1]);
                            stream[1] = br.getRegex("<default\\-stream\\-url>(.*?)</default\\-stream\\-url>").getMatch(0);
