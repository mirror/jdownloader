 ...
 * @author raztoki
 */
@DecrypterPlugin(revision = "$Revision: 20515 $", interfaceVersion = 3, names = { "paste.my.id" }, urls = { "https?://(?:www\\.)?paste\\.my\\.id/(?-i)[a-zA-Z0-9]{8}" }, flags = { 0 })
public class PasteMyId extends PluginForDecrypt {

    public PasteMyId(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        String plaintxt = br.getRegex("<div class=\"span8\">.*?</div>\\s*</div>\\s*</div>").getMatch(-1);
        if (plaintxt == null) {
