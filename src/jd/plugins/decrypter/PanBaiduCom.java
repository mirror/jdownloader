...
//You should have received a copy of the GNU General Public License
//along with this program.  If not, see <http://www.gnu.org/licenses/>.
@DecrypterPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "pan.baidu.com" }, urls = { "http://(www\\.)?pan\\.baidu\\.com/(netdisk/(extractpublic\\?username=\\d+|singlepublic\\?fid=\\d+_\\d+)|share/link\\?shareid=\\d+\\&uk=\\d+)" }, flags = { 0 })
public class PanBaiduCom extends PluginForDecrypt {
    ...
    private void decryptData(ArrayList<DownloadLink> decryptedLinks, String correctedBR, final String parameter, final String dirName) {
        if (parameter.matches(OTHERTYPE)) {
            final String[][] linkInfo0 = new Regex(correctedBR, "\"server_filename\":\"([^<>\"]*?)\",\"size\":\"(\\d+)\"").getMatches();
