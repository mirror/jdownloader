...
@DecrypterPlugin(revision = "$Revision: 13393 $", interfaceVersion = 2, names = { "ex.ua" }, urls = { "http://(www\\.)?ex\\.ua/(view_storage|get)/.+" }, flags = { 0 })
public class ExUa extends PluginForDecrypt {

    public ExUa(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (parameter.contains("/get/")) {
            br.getPage(parameter);
            decryptedLinks.add(createDownloadlink("directhttp://" + parameter));
            return decryptedLinks;
        }
        //TODO add support for "/storage" URLs
        String host = new Regex(parameter, "(.+)/view_storage").getMatch(0);
        br.getPage(parameter);
        /* Error handling */
        if (br.containsHTML("Object with such key is not found|Sorry, you can't download files")) {
        ...
        String[] redirectLinks = br.getRegex("(/get/[0-9A-Z]+/[a-z0-9]+)").getColumn(0);
        ...
        for (String singlelink : redirectLinks) {
            String dllink = null;
            if (singlelink.contains("/get/")) {
                br.getPage(host + singlelink);
                dllink = "directhttp://" + host + singlelink;
            }
