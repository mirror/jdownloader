...
@DecrypterPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "ddl-music.to" }, urls = { "https?://(?:www\\.)?ddl-music\\.(?:org|to)/(download/\\d+/.*?/|download/links/[a-f0-9]{32}/(mirror/\\d+/)?)" })
public class DDLMscrg extends antiDDoSForDecrypt {
    private static final String DECRYPTER_DDLMSC_MAIN  = "https?://[^/]+/download/\\d+/.*?/";
    private static final String DECRYPTER_DDLMSC_CRYPT = "https?://[^/]+/download/links/[a-z0-9]+/(mirror/\\d+/)?";
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (parameter.matches(DECRYPTER_DDLMSC_CRYPT)) {
            ...
            for (int i = 1; i < 10; i++) {
                ...
                getPage(parameter);
                if (br.getURL().matches(DECRYPTER_DDLMSC_MAIN)) {
                    /* 2020-12-16: Redirect to main --> Referer was needed but missing -> It is now given -> Try again */
                    getPage(parameter);
                }
            ...
            String[] allLinks = br.getRegex("<a href=\"(https://[^\"]+)\" target=\"_blank\"[^>]*style=\"border:0px;\"").getColumn(0);
            if (allLinks.length == 0) {
                /* Fallback - just grab all existing URLs */
                allLinks = HTMLParser.getHttpLinks(br.toString(), br.getURL());
            }
            ...
            for (final String aLink : allLinks) {
                if (this.canHandle(aLink)) {
                    /* Skip URLs which this crawler can handle */
                    continue;
                } else {
