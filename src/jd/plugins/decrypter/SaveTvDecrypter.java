...
public class SaveTvDecrypter extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        try {
            ...
            try {
                for (int i = 1; i <= maxPage; i++) {
                    ...
                    if (dlInfo != null && dlInfo.length != 0) {
                        logger.info("save.tv: Page: " + i + "of " + maxPage + ": Found " + dlInfo.length + " series to expand and decrypt!");
                        int seriesCounter = 1;
                        for (final String[] dInfo : dlInfo) {
                            logger.info("save.tv: Page: " + i + " of " + maxPage + ": Decrypting series " + seriesCounter + " of " + dlInfo.length);
                            try {
                                if (this.isAbort()) {
            ...
            } else if (decryptedLinks.size() < totalLinksNum && !crawlerInfiDialogsDisabled) {
                try {
                    SwingUtilities.invokeAndWait(new Runnable() {
                        @Override
                        public void run() {
            ...
            try {
                final int totalLinksNumTry = totalLinksNum2;
                SwingUtilities.invokeAndWait(new Runnable() {
                    @Override
                    public void run() {
                        try {
                            ...
                            message += "Es wird empfohlen, es zu einem späteren Zeitpunkt nochmals zu versuchen.\r\n";
                            message += "Es wurden nur " + decryptedLinks.size() + " von " + totalLinksNumTry + " Links (telecastIDs) gefunden!";
                            message += "\r\n\r\n";
                            message += "Genervt von diesen Info-Dialogen? In den Plugin Einstellung kannst du sie deaktivieren ;)";
                            JOptionPane.showConfirmDialog(jd.gui.swing.jdgui.JDGui.getInstance().getMainFrame(), message, title, JOptionPane.PLAIN_MESSAGE, JOptionPane.ERROR_MESSAGE, null);
                        } catch (Throwable e) {
