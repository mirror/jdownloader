...
@DecrypterPlugin(revision = "$Revision: 25359 $", interfaceVersion = 2, names = { "xxx-4-free.net", "chili-warez.net" }, urls = { "http://(www\\.)?xxx\\-4\\-free\\.net/.*?/.*?\\.html", "http://(www\\.)?chili\\-warez\\.net/[^<>\"]+\\d+\\.html" }, flags = { 0, 0 })
public class X4fChWa extends PluginForDecrypt {

    // DEV NOTES
    // variant of MvWrldNt

    public X4fChWa(final PluginWrapper wrapper) {
        super(wrapper);
    }

    private static final String UNSUPPORTEDLINKS         = "http://(www\\.)?(xxx\\-4\\-free\\.net|chili\\-warez\\.net)//?(news/|topliste/|premium_zugang|suche/|faq|pics/index|clips/index|movies/index|movies/seite|streams/index|stories/index|partner/anmelden|kontakt).*?\\.html";
    ...
    public ArrayList<DownloadLink> decryptIt(final CryptedLink param, final ProgressController progress) throws Exception {
        ...
        for (int i = 0; i <= 5; i++) {
            if (i > 0) {
                ...
                code = getCaptchaCode(this.getHost(), captchaFile, param);
            }
        ...
        if (links == null || links.length == 0) {
            logger.warning(this.getHost() + ": The content of this link is completly offline");
            logger.warning(this.getHost() + ": Please confirm via browser, and report any bugs to developement team. :" + parameter);
            return decryptedLinks;
        }
