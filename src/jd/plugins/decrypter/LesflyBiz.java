...
public class LesflyBiz extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (br.getHttpConnection().getResponseCode() == 404 || this.br.getURL().contains("error.html")) {
            decryptedLinks.add(this.createOfflinelink(parameter));
            return decryptedLinks;
        }
        final String finallink = this.br.getRegex("window\\.location[\t\n\r ]*?=[\t\n\r ]*?\"(http[^<>\"]+)\"").getMatch(0);
        if (finallink == null || finallink.matches(".+lesfly\\.biz.+")) {
            return null;
        }
