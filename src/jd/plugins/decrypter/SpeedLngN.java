...
@DecrypterPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "speedlounge.in" }, urls = { "http://[\\w\\.]*?speedlounge\\.in/detail\\.php\\?cat=.*?\\&id=[0-9]+" }, flags = { 0 })
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        for (String cryptedlink : links) {
            if (cryptedlink.contains("=")) {
                String finallink = "http://linksave.in/" + Encoding.Base64Decode(cryptedlink.replace("&amp", ""));
