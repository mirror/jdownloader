...
public class UnixMangaCom extends PluginForDecrypt {
	...
	public ArrayList<DownloadLink> decryptIt(CryptedLink parameter,
		...
		String[][] hrefArray = br
				.getRegex(
						"<A[^>]*?\"td2\"[^>]*?HREF[^>]*?\"([^>]*?)\">[^>]*?\\d+[^>]*?\\.(png|jpg|jpeg|gif)[^>]*?</A")
				.getMatches();
		int numberOfPages = hrefArray.length;
		String format = "%02d";
		if (numberOfPages > 0) {
			format = String.format("%%0%dd",
					(int) Math.log10(numberOfPages) + 1);
		}
		...
		for (int i = 0; i < numberOfPages; i++) {
			...
			String[][] unformattedSource = br.getRegex(
					"<IMG.*?SRC.*?\"(.*?)\\.(png|jpg|jpeg|gif)\"").getMatches();
			...
			link.setFinalFileName(title + "–page_" + pageNumber + "."
					+ extension);
			fp.add(link);
			try {
				distribute(link);
