...
public class Serienjunkies extends PluginForDecrypt {
	...
	private static String[] mirrorManagement = new String[] {
			JDL.L("plugins.decrypt.serienjunkies.usePremiumLinks",
					"use premiumlinks if possible"),
			JDL.L("plugins.decrypt.serienjunkies.automaticMirrorManagment",
					"automatic mirror managment"),
			JDL.L("plugins.decrypt.serienjunkies.noMirrorManagment",
					"no mirror managment"),
			JDL.L("plugins.decrypt.serienjunkies.RsComOnly",
					"nur Rapidshare.com"),
			JDL
					.L("plugins.decrypt.serienjunkies.RsDeOnly",
							"nur Rapidshare.de"),
			JDL
					.L("plugins.decrypt.serienjunkies.NetloadOnly",
							"nur Netload.in"),
			JDL.L("plugins.decrypt.serienjunkies.UlOnly", "nur Uploaded.to"),
			JDL
					.L("plugins.decrypt.serienjunkies.FFOnly",
							"nur FileFactory.com") };

	private static String mirror = mirrorManagement[0];
	private final Pattern patternCaptcha = Pattern
			.compile("(?s)<FORM ACTION=\".*?\" METHOD=\"post\".*?<INPUT TYPE=\"HIDDEN\" NAME=\"s\" VALUE=\"(.*?)\">.*?<IMG SRC=\"([^\"]*)\"");
	...
	private static URLConnectionAdapter openGetConnection(Browser capbr,
			String captchaAdress) throws IOException {
		while (rc) {
			try {
				Thread.sleep(200);
	...
	private static String postPage(Browser br3, String url, String string)
			throws IOException {
		while (rc) {
			try {
				Thread.sleep(200);
	 ...
	 */
	private ArrayList<String> containerLinks(String url,
			CryptedLink downloadLink) throws DecrypterException {
		final ArrayList<String> links = new ArrayList<String>();
		final Browser br3 = getBrowser();
		if (url.matches("http://[\\w\\.]*?.serienjunkies.org/..\\-.*")) {
			url = url.replaceFirst("serienjunkies.org",
					"serienjunkies.org/frame");
		}
		...
		try {
			...
			while (true) {
				...
				if (captchaRegex.matches()) {
					...
					if (con.getLongContentLength() < 1000) {
						con.disconnect();
						logger
								.info("Sj Downloadlimit(decryptlimit) reached. Wait for reconnect(max 2 min)");
						progress
								.setStatusText(JDL
										.L(
												"plugins.decrypt.serienjunkies.progress.decryptlimit",
												"SJ Downloadlimit(decryptlimit) reached. Wait for reconnect(max 2 min)"));
						...
						if (!Reconnecter.waitForNewIP(2 * 60 * 1000l)) {
							...
							progress
									.setStatusText(JDL
											.L(
													"plugins.decrypt.serienjunkies.progress.downloadlimit",
													"Error: SerienJunkies Downloadlimit"));
							for (int i = 0; i < 100; i++) {
								try {
									Thread.sleep(100);
								} catch (InterruptedException e) {
					...
					if (capTxt == null || capTxt.length() != 3)
						continue;
					htmlcode = postPage(br3, url, "s="
							+ captchaRegex.getMatch(0) + "&c=" + capTxt
							+ "&action=Download");
			...
			final Browser[] br2 = new Browser[] { br3.cloneBrowser(),
					br3.cloneBrowser(), br3.cloneBrowser(), br3.cloneBrowser() };
			progress.setStatusText(JDL.L(
					"plugins.decrypt.serienjunkies.progress.getLinks",
					"get links"));
			progress.setStatus(0);

			ArrayList<String> actions = new ArrayList<String>();
			for (Form form : forms) {
				if (form.getAction().contains("download.serienjunkies.org")
						&& !form.getAction().contains("firstload")
						&& !form.getAction().equals(
								"http://mirror.serienjunkies.org")) {
	 ...
	 */
	private String einzelLinks(String url, CryptedLink downloadLink)
		...
		try {
			...
			while (true) {
				...
				if (captchaRegex.matches()) {
					...
					active++;
					try {
						capTxt = getCaptchaCode(
								"einzellinks.serienjunkies.org", captchaFile,
								downloadLink);
					} catch (DecrypterException e) {
						active--;
						throw e;
	...
	private ArrayList<DownloadLink> getDLinks(String parameter,
			CryptedLink cryptedLink) throws DecrypterException {
		...
		try {
			...
			subdomain = new Regex(parameter, "http://(.*?)serienjunkies.org.*")
					.getMatch(0);
			...
			String furl = br3.getRegex(
					Pattern.compile("<FRAME SRC=\"(.*?)"
							+ modifiedURL.replaceAll("[^0-1a-zA-Z]", ".")
			...
			if (isEinzelLink(parameter)) {
				...
				decryptedLinks.add(createDownloadlink(Encoding
						.htmlDecode(einzelLinks(parameter, cryptedLink))));
			} else if (parameter.indexOf(subdomain + "serienjunkies.org") >= 0
					|| parameter.indexOf("/sjsafe/") >= 0) {
				...
				for (String link : links) {
					if (link.indexOf("/safe/") >= 0) {
						decryptedLinks.add(createDownloadlink(Encoding
								.htmlDecode(einzelLinks(link, cryptedLink))));
	...
	private SerienjunkiesLinks createdl(String parameter, String[] info) {
		...
		if (title == null)
			title = "";
		try {
			linkName = ((title.length() > 10 ? title.substring(0, 10) : title)
		...
		if (linkName == null
				|| parameter.matches("http://serienjunkies.org/sa[fv]e/.*")
				|| parameter
						.matches("http://download.serienjunkies.org/..\\-.*")) {
			size = 100;
			linkName = parameter.replaceFirst(".*/..[\\_\\-]", "")
					.replaceFirst("\\.html?", "");
		}
		String hostname = getHostname(parameter);
		SerienjunkiesLinks dlink = new SerienjunkiesLinks(linkName, hostname,
				size * 1024 * 1024, parameter, mirrors);
		return dlink;
	}
	...
	public ArrayList<DownloadLink> decryptIt(CryptedLink param,
		...
		Browser
				.setRequestIntervalLimitGlobal("download.serienjunkies.org",
						400);
	...
	private ArrayList<SerienjunkiesLinks> decryptItMain(CryptedLink param)
			throws Exception {
		...
		if (parameter.contains("/\\?cat=")) {
			if (getSerienJunkiesCat() == sCatNoThing)
				return new ArrayList<SerienjunkiesLinks>();
		}
		...
		if (parameter.matches(".*\\?(cat|p)=.*")) {
			...
			if (catst == sCatNoThing)
				return new ArrayList<SerienjunkiesLinks>();

			int cat = Integer.parseInt(parameter.replaceFirst(".*\\?(cat|p)=",
					"").replaceFirst("[^\\d].*", ""));
			if (catst == sCatNewestDownload) {
				getPage(br, "http://serienjunkies.org/");

				String[] linkss = br.getRegex(
						"<a href=\"http://serienjunkies\\.org/\\?cat=" + cat
								+ "\">(.*?)</a><br").getColumn(0);
				if (linkss.length == 0)
					return decryptedLinks;
					...
					if (wh != null) {
						for (String element : links) {
							if (getHostname(element).equals(wh)) {
								String[] info = getLinkName(element,
										lastHtmlCode);
						...
						for (String element : links) {
							...
							if (JDUtilities.getPluginForHost(
									getHostname(element))
									.getMaxSimultanDownloadNum(null) > 1) {
						...
						if (!got) {
							for (String element : links) {
								String[] info = getLinkName(element,
										lastHtmlCode);
								SerienjunkiesLinks dl_link = createdl(element,
										info);
								decryptedLinks.add(dl_link);
								break;
				...
				if (parameter.contains("/?p=")) {
					...
					htmlcode = br.toString();
					try {
						int pages = Integer.parseInt(br.getRegex(
								"<p align=\"center\">  Pages \\(([\\d]+)\\):")
								.getMatch(0));
						for (int i = 2; i < pages + 1; i++) {
							htmlcode += "\n"
									+ getPage(br,
											"http://serienjunkies.org/?cat="
													+ cat + "&paged=" + i);
						}
					} catch (Exception e) {
					}
				...
				String wh = getHostWishedHost();
				String[] titles = htmlcode
						.replaceFirst(
								"(?is).*?(<h2><a href=\"http://serienjunkies.org/[^\"]*\" rel=\"bookmark\"[^>]*>)",
								"$1")
						.split(
								"<h2><a href=\"http://serienjunkies.org/[^\"]*\" rel=\"bookmark\"[^>]*?>");
				for (String element : titles) {

					String title = new Regex(element, "([^><]*?)</a>")
							.getMatch(0);
					String[] sp = element
							.split("(?is)<strong>Gr(ö|oe)(ß|ss)e:?</strong>:?[\\s]*");
					for (String element2 : sp) {
						...
						for (String[] element3 : links) {
							String[] sp2 = element3[1]
									.split("<strong>.*?</strong>");

							if (wh != null) {

								outer: for (String bb : sp2) {
									String[] links2 = HTMLParser.getHttpLinks(
											bb, parameter);
									for (String element4 : links2) {
										if (canHandle(element4)
												&& getHostname(element4)
														.equals(wh)) {
											SerienjunkiesLinks dl = createdl(
													element4,
													new String[] { size,
															element3[0],
															element3[1], title });
								...
								if (mirror.equals(mirrorManagement[0])) {
									for (String bb : sp2) {
										String[] links2 = HTMLParser
												.getHttpLinks(bb, parameter);
										for (String element4 : links2) {
											if (canHandle(element4)) {
												SerienjunkiesLinks dl = createdl(
														element4, new String[] {
	...
	private String[] getLinkName(String link, String htmlcode) {
		...
		for (String element : titles) {
			...
			for (String element2 : sp) {
				...
				String[][] links = new Regex(element2.replaceAll(
						"<a href=\"http://vote.serienjunkies.org.*?</a>", ""),
						"<p><strong>(.*?)</strong>(.*?)</p>").getMatches();

				for (String[] element3 : links) {
					try {
						if (element3[1].toLowerCase().contains(
								Encoding.UTF8Decode(link).toLowerCase()))
							return new String[] { size, element3[0],
									element3[1], title };
					} catch (Exception e) {
						logger.log(Level.SEVERE, "Exception occurred", e);
					}
	...
	private class SerienjunkiesThread extends Thread {
		...
		public SerienjunkiesThread(SerienjunkiesLinks downloadLink,
				CryptedLink cryptedLink) {
			...
			try {
				...
				if (dls != null && dls.size() > 0) {
					...
					for (DownloadLink dls2 : dls) {
						...
						DistributeData distributeData = new DistributeData(dls2
								.getDownloadURL());
						finaldls.addAll(distributeData.findLinks());
					}
					if (finaldls.size() > 0) {
						try {
							DownloadLink[] linksar = finaldls
									.toArray(new DownloadLink[finaldls.size()]);
							progress
									.setStatusText(JDL
											.L(
													"plugins.decrypt.serienjunkies.progress.checkLinks",
													"check links"));
							progress.setStatus(0);
					...
					if (finaldls == null) {
						if (mirrors == null) {
							...
							getPage(br2k, "http://serienjunkies.org/?s="
									+ cryptedLink.getCryptedUrl().replaceFirst(
											".*/", "").replaceFirst(
											"\\.html?$", ""));
							String[] info = getLinkName(cryptedLink
									.getCryptedUrl(), br2k.toString());
							logger.warning("use Mirror");
							try {
								mirrors = getMirrors(cryptedLink
										.getCryptedUrl(), info[2]);
							} catch (Exception e) {
								...
								try {
									...
									for (DownloadLink dls2 : dls) {
										DistributeData distributeData = new DistributeData(
												dls2.getDownloadURL());
										finaldls.addAll(distributeData
												.findLinks());
									}
									if (finaldls.size() > 0) {
										try {
											DownloadLink[] linksar = finaldls
													.toArray(new DownloadLink[finaldls
															.size()]);
											progress
													.setStatusText(JDL
															.L(
																	"plugins.decrypt.serienjunkies.progress.checkMirror",
																	"check mirror"));
											progress.setStatus(0);
											int inc = 100 / linksar.length;
											linksar[0].getPlugin().checkLinks(
													linksar);
											for (DownloadLink downloadLink2 : linksar) {
												if (!downloadLink2
														.isAvailable()) {
					...
					if (finaldls != null && finaldls.size() > 0) {
						for (DownloadLink downloadLink2 : finaldls) {
							downloadLink2
									.addSourcePluginPasswordList(passwords);
						}
						result = finaldls;
					} else {
						result = down;
	...
	private class SerienjunkiesLinks {
		...
		public SerienjunkiesLinks(String name, String host, long size,
				String link, String[] mirrors) {
			this.name = name;
			this.host = host;
	...
	private class SerienjunkiesCatDialog extends JDialog {
		...
		private void initGUI() {
			...
			meths[0] = new SerienjunkiesMeth(JDL.L(
					"plugins.SerienJunkies.CatDialog.sCatNoThing",
					"Kategorie nicht hinzufügen"), sCatNoThing);
			meths[1] = new SerienjunkiesMeth(JDL.L(
					"plugins.SerienJunkies.CatDialog.sCatGrabb",
					"Alle Serien in dieser Kategorie hinzufügen"), sCatGrabb);
			meths[2] = new SerienjunkiesMeth(JDL.L(
					"plugins.SerienJunkies.CatDialog.sCatNewestDownload",
			...
			final JCheckBox checkScat = new JCheckBox(JDL.L(
					"plugins.SerienJunkies.CatDialog.sCatSave",
					"Einstellungen für diese Sitzung beibehalten?"));
			JButton btnOK = new JButton(JDL.L("gui.btn_ok", "OK"));
			btnOK.addActionListener(new ActionListener() {

				public void actionPerformed(ActionEvent e) {
					useScat = new int[] {
							((SerienjunkiesMeth) methods.getSelectedItem()).var,
			...
			setModal(true);
			setTitle(JDL.L("plugins.SerienJunkies.CatDialog.title",
					"SerienJunkies ::CAT::"));
			setAlwaysOnTop(true);
			...
			add(new JLabel(JDL.L("plugins.SerienJunkies.CatDialog.action",
					"Wählen sie eine Aktion aus:")));
			add(methods);
			add(new JLabel(JDL.L("plugins.SerienJunkies.CatDialog.mirror",
					"Wählen sie eine Mirrorverwalung:")));
			add(settings);
			add(checkScat);
	...
	private class SerienjunkiesSJTable extends JDialog {
		...
		private void initGUI() {
			JLabel m_title = new JLabel(JDL.L(
					"plugin.serienjunkies.manager.dllinks",
					"Unerwünschte Links einfach löschen"));
			m_title.setIcon(new ImageIcon(JDImage.getImage(JDTheme
					.V("gui.images.config.addons"))));
			...
			JButton deleteButton = new JButton(JDL.L(
			...
			final JButton insertButton = new JButton(JDL.L(
			...
			final JButton closeButton = new JButton(JDL.L("gui.btn_cancel",
					"Cancel"));
			...
			setTitle(JDL.L("plugin.serienjunkies.manager.title",
					"SerienJunkies Linkverwaltung"));
			setModal(true);
		...
		private class SerienjunkiesTM extends AbstractTableModel {
			...
			private String m_columns[] = {
					JDL.L("gui.packageinfo.name", "Name"),
					JDL.L("gui.treetable.header_3.hoster", "Anbieter"),
					JDL.L("gui.linkgrabber.packagetab.table.column.size",
							"Größe") };
