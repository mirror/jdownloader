...
@DecrypterPlugin(revision = "$Revision$", interfaceVersion = 2, names = { "szort.pl" }, urls = { "http://(www\\.)?szort\\.pl/(?!style\\.css).{3,}" }) 
public class SzrtPl extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        while (true) {
            ...
            if (link == null) link = br.getRegex("<frame name=\"strona\" src=\"([^<>\"]*?)\">").getMatch(0);
            if (link == null) {
