...
public class Antena3ComSalon extends PluginForDecrypt {
	...
	@Override
	public ArrayList<DownloadLink> decryptIt(CryptedLink link,
			ProgressController progress) throws Exception {

		String html = br.getPage(link.toString());
		if (html.contains("<h1>¡Uy! No encontramos la página que buscas.</h1>"))
			throw new DecrypterException(
					JDL.L("plugins.decrypt.errormsg.unavailable",
							"Perhaps wrong URL or the download is not available anymore."));

		String xmlURL = "http://www.antena3.com"
				+ new Regex(html, "player_capitulo.xml='(.*?)';").getMatch(0);
		br.getPage(xmlURL);
		...
		for (String sdl : links) {
			if (sdl.contains(".mp4")) {
				sdl = "http://desprogresiva.antena3.com/"
						+ sdl.replace("<![CDATA[", "").replace("]]>", "");
				DownloadLink dl = createDownloadlink(sdl);
				dl.setName(br.getRegex("<descripcion>(.*?)</descripcion>")
						.getMatch(0).replace("<![CDATA[", "")
						.replace("]]>", "")
						+ " - " + dl.getName());
