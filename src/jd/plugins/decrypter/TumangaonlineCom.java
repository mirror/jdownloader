...
public class TumangaonlineCom extends antiDDoSForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        br.setHeader("User-Agent", "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.110 Safari/537.36");
        br.getHeaders().put("Accept", "application/json, text/plain, */*");
        br.getHeaders().put("X-Requested-With", "XMLHttpRequest");
        br.getHeaders().put("Cache-Control", "no-cache");
        br.getHeaders().put("Cache-Mode", "no-cache");
        br.getHeaders().put("Pragma", "no-cache");
        br.setCookie(getHost(), "bbl", "1");
        br.setCookie(getHost(), "BB_plg", "pm");
        br.setCookie(getHost(), "TMOVisto_" + url_idManga + "_" + url_numeroCapitulo + "_" + url_idScanlation, "true");
        getPage(String.format("https://www.tumangaonline.com/api/v1/imagenes?idManga=%s&idScanlation=%s&numeroCapitulo=%s&visto=false", url_idManga, url_idScanlation, url_numeroCapitulo));
