...
public class SharedZillacom extends PluginForHost {
    ...
    public PluginStep doStep(PluginStep step, DownloadLink downloadLink) {
        ...
        try {
            ...
            requestInfo = HTTP.postRequestWithoutHtmlCode(new URL("http://sharedzilla.com/en/downloaddo"), requestInfo.getCookie(), downloadLink.getFileInfomationString(), "id="+id, false);
            if (requestInfo.getLocation()==null){
                downloadLink.setStatus(DownloadLink.STATUS_ERROR_TEMPORARILY_UNAVAILABLE);
                step.setStatus(PluginStep.STATUS_ERROR);
                return step;
            }
            requestInfo = HTTP.getRequestWithoutHtmlCode(new URL(requestInfo.getLocation()), requestInfo.getCookie(), requestInfo.getConnection().getURL().toString(), false);
            /* Datei herunterladen */
            ...
            dl.setChunkNum(1);/*bei dem speed lohnen mehrere chunks nicht, da es nicht schneller wird*/
            dl.setResume(true);
