...
public class BadOngo extends PluginForHost {
    ...
    public PluginStep doStep(PluginStep step, DownloadLink downloadLink)
		...
		case PluginStep.STEP_WAIT_TIME:
            ...
            requestInfo = getRequest(new URL(downloadLink
                    .getDownloadURL().replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}file", "http://www.badongo.com/de/file").replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}vid", "http://www.badongo.com/de/vid")));
			...
			if(cookie==null)
			{
	            ...
	            requestInfo = getRequest(new URL(downloadLink
	                    .getDownloadURL().replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}file", "http://www.badongo.com/en/file").replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}vid", "http://www.badongo.com/en/vid")+"?rs=refreshImage&rst=&rsrnd="+rsrnd+"&rsargs[]=0"), cookie, requestInfo.getConnection().getURL().toString(), true);
				...
				if(code==null||code=="")
				{
					...
					downloadLink
							.setStatus(DownloadLink.STATUS_ERROR_CAPTCHA_WRONG);
					downloadLink.saveObjects.addFirst(downloadLink.getDownloadURL());
					step.setStatus(PluginStep.STATUS_ERROR);
			...
			{
	            requestInfo = getRequest(new URL(downloadLink
	                    .getDownloadURL().replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}file", "http://www.badongo.com/de/cfile").replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}vid", "http://www.badongo.com/de/cfile")), cookie, requestInfo.getConnection().getURL().toString(), false);
				cookie += "; "+requestInfo.getCookie();
			}
			return step;
		...
		case PluginStep.STEP_DOWNLOAD:
            ...
            for (int i = 0; i < 10; i++) {
            	try {
                    requestInfo = getRequest(new URL(downloadLink
                            .getDownloadURL().replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}(vid|file)", "http://www.badongo.com/de/cfile")+"?rs=getFileLink&rst=&rsrnd="+rsrnd+"&rsargs[]=0"),cookie,requestInfo.getConnection().getURL().toString(), true);
                    form=requestInfo.getForm();
				} catch (Exception e) {
    ...
    public boolean getFileInformation(DownloadLink downloadLink) {
        try {
            requestInfo = getRequest(new URL(downloadLink
                    .getDownloadURL().replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}file", "http://www.badongo.com/de/file").replaceFirst("http://.*?badongo\\.com/[a-zA-Z/]{0,5}vid", "http://www.badongo.com/de/vid")));
            String[] fileInfo = new Regexp(requestInfo.getHtmlCode(),
                    "<td valign=\"top\"><b>Datei.</b></td>.*?<td>.nbsp.(.*?)</td>.*?<td valign=\"top\"><b>Dateigr.sse.</b></td>.*?<td>.nbsp.([0-9\\.]*)(.*?)</td>")
                    .getMatches()[0];
