...
public class Awesome extends Observable {
	...
	/**
	 * @param listener the listener which should listen to incoming AwesomeProposalRequests
	 */
	...
	/**
	 * This function requests proposals for the given input string.
	 * For a notification of each returned proposal use {@link #addObserver(Observer)}.
	 * In order to get all returned proposals use {@link #getProposals()}, 
	 * but please take care of locking the readLock with {@link #returnedProposalsLock}.
	...
	public synchronized void requestProposal(String in) {
		...
		/*
		 * If both params and command are empty, delete returned proposals.
		 * This wouldn't be necessary of course, but it is a cosmetic fix.
		 * Also, it works as a workaround for changed commands with unchanged params
		 * because usually the first char changes and is removed as last part of the command:
		 * (input one: "command-alias-1 param" returnedProposal: "command-alias-1 param")
		 * (input two: "command-alias-2 param" returnedProposal: still "command-alias-1 param")
		 * (working well anyway, just displaying wrong keyword)
		 */
	...
	/**
	 * Adds a proposal to the list of the returned proposals as long as it is not outdated (= not interrupted).
		...
		try {
			...
			try {
				...
				while (iterator.hasPrevious()) {
					if (proposal.compareTo(iterator.previous()) <= 0)
						break;
				};
			} finally {
				commandLock.readLock().unlock();
