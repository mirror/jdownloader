...
public class JDFeedMeFeed implements Serializable {
    ...
    public synchronized static void saveXML(ArrayList<JDFeedMeFeed> settings, String location)
    {
    	...
    	JDIO.saveObject(null, settings, JDUtilities.getResourceFile(location), null, null, true);
    	...
    	try
    	{
    		String path = JDUtilities.getJDHomeDirectoryFromEnvironment().getAbsolutePath() + "/" + location;
    		File f = new File(path);
    		f.mkdirs();
    		
    		XMLEncoder encoder = new XMLEncoder(
                 new BufferedOutputStream(
                     new FileOutputStream(JDUtilities.getJDHomeDirectoryFromEnvironment().getAbsolutePath() + "/" + location)));
	...
	public synchronized static ArrayList<JDFeedMeFeed> loadXML(String location)
    {
    	...
    	try
    	{
    		XMLDecoder decoder = new XMLDecoder(
    			new BufferedInputStream(
    				new FileInputStream(JDUtilities.getJDHomeDirectoryFromEnvironment().getAbsolutePath() + "/" + location)));
    		ArrayList<JDFeedMeSetting> settings = (ArrayList<JDFeedMeSetting>)decoder.readObject();
    		decoder.close();
    		return settings;
    	}
    	catch (Exception e)
    	...
    	// if here then we were unable to load
    	return new ArrayList<JDFeedMeSetting>();
    	*/
    }
}
