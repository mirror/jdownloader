...
public class JDSimpleWebserverTemplateFileRequestHandler {
	 ...
	 *            directory
	 */
	public JDSimpleWebserverTemplateFileRequestHandler(
			JDSimpleWebserverResponseCreator response) {

		this.response = response;
	}

	private void add_all_info(Template t,
		...
		if (requestParameter.containsKey("all_info")) {
			...
			addEntry("packagesize", JDUtilities.formatKbReadable(fp
					.getTotalEstimatedPackageSize())
					+ " " + fp.getTotalEstimatedPackageSize() + " KB");
			addEntry("loaded", JDUtilities.formatKbReadable(fp
					.getTotalKBLoaded())
					+ " " + fp.getTotalKBLoaded() + " KB");
			addEntry("links", "");

			DownloadLink next = null;
			int i = 1;
			for (Iterator<DownloadLink> it = fp.getDownloadLinks().iterator(); it
					.hasNext(); i++) {
				...
				double percent = next.getDownloadCurrent() * 100.0
						/ Math.max(1, next.getDownloadMax());

				h_info.put("info_percent", f.format(percent));
				h_info.put("download_status", Single_Status);
				h_info.put("info_var", i + ". " + next.getName());
				h_info.put("info_value", JDUtilities
						.formatKbReadable((int) next.getDownloadSpeed() / 1024)
						+ "/s "
						+ JDUtilities.getPercent((int) next
								.getDownloadCurrent(), (int) next
								.getDownloadMax())
						+ " | "
						+ next.getDownloadCurrent()
						+ "/"
						+ next.getDownloadMax() + " bytes");
	...
	private void add_single_info(Template t,
			HashMap<String, String> requestParameter) {
		...
		if (requestParameter.containsKey("single_info")) {
			ids = requestParameter.get("single_info").toString()
					.split("[+]", 2);
			package_id = JDUtilities.filterInt(ids[0].toString());
			download_id = JDUtilities.filterInt(ids[1].toString());
			downloadLink = JDUtilities.getController().getPackages().get(
					package_id).getDownloadLinks().get(download_id);

			addEntry("file", new File(downloadLink.getFileOutput()).getName()
					+ " @ " + downloadLink.getHost());
			if (downloadLink.getFilePackage() != null
					&& downloadLink.getFilePackage().getPassword() != null) {
				addEntry(JDLocale
						.L("linkinformation.password.name", "Passwort"),
						downloadLink.getFilePackage().getPassword());
			}
			if (downloadLink.getFilePackage() != null
					&& downloadLink.getFilePackage().getComment() != null) {
				addEntry(JDLocale
						.L("linkinformation.comment.name", "Kommentar"),
						downloadLink.getFilePackage().getComment());
			}
			if (downloadLink.getFilePackage() != null) {
				addEntry(JDLocale.L("linkinformation.package.name", "Packet"),
						downloadLink.getFilePackage().getName());
			}
			if (downloadLink.getDownloadMax() > 0) {
				addEntry(JDLocale.L("linkinformation.filesize.name",
						"Dateigröße"), JDUtilities.formatBytesToMB(downloadLink
						.getDownloadMax()));
			}
			if (downloadLink.isAborted()) {
				addEntry(JDLocale
						.L("linkinformation.download.name", "Download"),
						JDLocale.L("linkinformation.download.aborted",
								"Abgebrochen"));
			}
			if (downloadLink.isAvailabilityChecked()) {
				addEntry(JDLocale.L("linkinformation.available.name",
						"Verfügbar"), downloadLink.isAvailable() ? JDLocale.L(
						"linkinformation.available.ok", "Datei OK") : JDLocale
						.L("linkinformation.available.error", "Fehler!"));
			} else {
				addEntry(JDLocale.L("linkinformation.available.name",
						"Verfügbar"), JDLocale.L(
						"linkinformation.available.notchecked",
						"noch nicht überprüft"));
			}
			if (downloadLink.getDownloadSpeed() > 0) {
				addEntry(JDLocale.L("linkinformation.speed.name",
						"Geschwindigkeit"), downloadLink.getDownloadSpeed()
						/ 1024 + " kb/s");
			}
			if (downloadLink.getFileOutput() != null) {
				addEntry(JDLocale.L("linkinformation.saveto.name",
						"Speichern in"), downloadLink.getFileOutput());
			}
			if (downloadLink.getRemainingWaittime() > 0) {
				addEntry(JDLocale.L("linkinformation.waittime.name",
						"Wartezeit"), downloadLink.getRemainingWaittime()
						+ " sek");
			}
			if (downloadLink.isInProgress()) {
				addEntry(JDLocale
						.L("linkinformation.download.name", "Download"),
						JDLocale.L("linkinformation.download.underway",
								" ist in Bearbeitung"));
			} else {
				addEntry(JDLocale
						.L("linkinformation.download.name", "Download"),
						JDLocale.L("linkinformation.download.notunderway",
								" ist nicht in Bearbeitung"));
			}
			if (!downloadLink.isEnabled()) {
				addEntry(JDLocale
						.L("linkinformation.download.name", "Download"),
						JDLocale.L("linkinformation.download.deactivated",
								" ist deaktiviert"));
			} else {
				addEntry(JDLocale
						.L("linkinformation.download.name", "Download"),
						JDLocale.L("linkinformation.download.activated",
								" ist aktiviert"));
			}
			addEntry(JDLocale.L("linkinformation.download.status", "Status"),
					downloadLink.getStatusText());
			...
			if (downloadLink.isInProgress()
					&& (dl = downloadLink.getDownloadInstance()) != null) {
				addEntry(JDLocale.L("linkinformation.download.chunks.label",
						"Chunks"), "");
				int i = 1;
				for (Iterator<Chunk> it = dl.getChunks().iterator(); it
						.hasNext(); i++) {
					Hashtable<Object, Object> h_info = new Hashtable<Object, Object>();
					Chunk next = it.next();
					double percent = next.getBytesLoaded() * 100.0
							/ Math.max(1, next.getChunkSize());
					h_info.put("download_status", Single_Status);
					h_info.put("info_var", JDLocale.L(
							"download.chunks.connection", "Verbindung")
							+ " " + i);
					h_info.put("info_value",
							JDUtilities.formatKbReadable((int) next
									.getBytesPerSecond() / 1024)
									+ "/s "
									+ JDUtilities.getPercent(next
											.getBytesLoaded(), next
											.getChunkSize()));
					h_info.put("info_percent", f.format(percent));
	 ...
	 * }
	 */
	private void add_status_page(Template t,
			HashMap<String, String> requestParameter) {
		...
		Integer Download_ID;
		Double percent = 0.0;
		for (Package_ID = 0; Package_ID < JDUtilities.getController()
				.getPackages().size(); Package_ID++) {
			filePackage = JDUtilities.getController().getPackages().get(
					Package_ID);
			...
			if (filePackage.getLinksInProgress() > 0) {
				value = filePackage.getLinksInProgress()
						+ "/"
						+ filePackage.size()
						+ " "
						+ JDLocale.L(
			...
			if (filePackage.getTotalDownloadSpeed() > 0)
				value = "["
						+ filePackage.getLinksInProgress()
						+ "/"
						+ filePackage.size()
						+ "] "
						+ "ETA "
						+ JDUtilities.formatSeconds(filePackage.getETA())
						+ " @ "
						+ JDUtilities.formatKbReadable(filePackage
								.getTotalDownloadSpeed() / 1024) + "/s";

			h.put("package_id", Package_ID.toString());
			h.put("download_hoster", value);
			h.put("download_status_text", f.format(percent)
					+ " % ("
					+ JDUtilities.formatKbReadable(filePackage
							.getTotalKBLoaded())
					+ " / "
			...
			for (Download_ID = 0; Download_ID < filePackage.getDownloadLinks()
					.size(); Download_ID++) {
				dLink = filePackage.getDownloadLinks().get(Download_ID);

				/* Download Infos */

				percent = (double) (dLink.getDownloadCurrent() * 100.0 / Math
						.max(1, dLink.getDownloadMax()));
				...
				h2.put("download_status_text", f.format(percent) + "% "
						+ dLink.getStatusText());
				v2.addElement(h2);

			}

			if (status[3] == 1 && status[2] == 0 && status[1] == 0
					&& status[0] == 0) {
		...
		t.setParam("config_current_speed", JDUtilities.getController()
				.getSpeedMeter() / 1024);
		...
		if (JDUtilities.getConfiguration().getBooleanProperty(
				Configuration.PARAM_DISABLE_RECONNECT, false) == true) {
	...
	private void add_linkadder_page(Template t,
			HashMap<String, String> requestParameter) {
		Vector<Object> v, v2 = new Vector<Object>();
		Hashtable<Object, Object> h, h2 = new Hashtable<Object, Object>();
		v = new Vector<Object>();
		...
		for (Package_ID = 0; Package_ID < JDWebinterface.Link_Adder_Packages
				.size(); Package_ID++) {
	...
	private void add_password_list(Template t,
			HashMap<String, String> requestParameter) {
	...
	public void handleRequest(String url,
			HashMap<String, String> requestParameter) {
		try {
			Template t = new Template(JDUtilities.getResourceFile(
					"plugins/webinterface/" + url).getAbsolutePath());

			t.setParam("webinterface_version", JDWebinterface.instance
					.getPluginID());
