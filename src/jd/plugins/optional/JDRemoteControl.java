    ...
    class Serverhandler extends AbstractHandler {
        ...
        public void handle(String target, HttpServletRequest request, HttpServletResponse response, int dispatch) throws IOException, ServletException {
            ...
            if (request.getRequestURI().equals("/help")) {
                ...
                // final String[] desc2={"Get IP","Get Config","Get
                // Version","Get Current Downloads","Get max. sim.
                // Downloads","Get finished Downloads","Get current Speed","Get
                // If Reconnect","Get If ClipBoard enabled","Start DLs","Pause
                // DLs","Stop DLs","Toggle DLs","Do Webupdate","Do
                // Reconnect","Restart JD","Shutdown JD","Set Downloadspeedlimit
                // %X%","Set max sim. Downloads %X%","Open Add Links
                // Dialog","Open Config Dialog","Open Log Dialog","Open
                // Container Dialog","Add Links %X% to Grabber<br />Options:<br
                // />grabber(0|1): Show/Hide LinkGrabber<br />start(0|1): Start
                // DLs after insert<br />Sample<br
                // />/action/add/links/grabber0/start1/http://tinyurl.com/6o73eq
                // http://tinyurl.com/4khvhn<br />Don't forget Space between
                // Links!","Add Container %X%","Set ClipBoard Control","Set
                // Reconnect Enabled"};
            ...
            // Get HappyHour
                ...
                Integer happyguiint = Integer.parseInt(new Regex(request.getRequestURI(), "[\\s\\S]*?/get/happyhour/gui([01]{1})/[\\s\\S]*").getFirstMatch());
                ...
                int sec = 300 - JDUtilities.filterInt(Regex.getLines(ri.getHtmlCode())[3]);

                int lastStart = JDUtilities.filterInt(Regex.getLines(ri.getHtmlCode())[4]);
                int lastEnd = JDUtilities.filterInt(Regex.getLines(ri.getHtmlCode())[5]);
                ...
                if (ri.containsHTML("Hour")) {// HappyHour aktiv
                    int activ = JDUtilities.filterInt(Regex.getLines(ri.getHtmlCode())[1]);
             ...
             * JDUtilities.getController().getDownloadLinks().size(); i++) { if
             * (JDUtilities.getController().getDownloadLinks().get(i).getStatus() ==
             * LinkStatus.DOWNLOAD_IN_PROGRESS) { output = output + "<-
             * name=\""+JDUtilities.getController().getDownloadLinks().get(i).getName()+
             * "\" hoster=\"" +
             * JDUtilities.getController().getDownloadLinks().get(i).getHost() +
             * "\" speed=\"" +
             * JDUtilities.getController().getDownloadLinks().get(i).getDownloadSpeed() +
             * "\" ->"; } } response.getWriter().println(output); }
             */
            ...
            // Do Make Webupdate
                ...
                Integer force = Integer.parseInt(new Regex(request.getRequestURI(), "[\\s\\S]*?/action/update/force([01]{1})/[\\s\\S]*").getFirstMatch());
                if (force == 1) {
            ...
            // Set Downloadlimit
                ...
                Integer newdllimit = Integer.parseInt(new Regex(request.getRequestURI(), "[\\s\\S]*/action/set/download/limit/([0-9]+).*").getFirstMatch());
            ...
            // Set max. sim. Downloads
                ...
                Integer newsimdl = Integer.parseInt(new Regex(request.getRequestURI(), "[\\s\\S]*/action/set/download/max/([0-9]+).*").getFirstMatch());
            ...
            // Add Link(s)
                ...
                String link = new Regex(request.getRequestURI(), "[\\s\\S]*?/action/add/links/grabber[01]{1}/start[01]{1}/([\\s\\S]+)").getFirstMatch();
                // response.getWriter().println(link);
                Integer showgrab = Integer.parseInt(new Regex(request.getRequestURI(), "[\\s\\S]*?/action/add/links/grabber([01]{1})/start[01]{1}/[\\s\\S]+").getFirstMatch());
                ...
                Integer stdl = Integer.parseInt(new Regex(request.getRequestURI(), "[\\s\\S]*?/action/add/links/grabber[01]{1}/start([01]{1})/[\\s\\S]+").getFirstMatch());
            ...
            // Set ReconnectEnabled
                ...
                boolean newrc = Boolean.parseBoolean(new Regex(request.getRequestURI(), "[\\s\\S]*/action/set/reconnectenabled/(.*)").getFirstMatch());
