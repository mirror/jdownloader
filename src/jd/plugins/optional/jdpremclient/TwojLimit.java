...
public class TwojLimit extends PluginForHost implements JDPremInterface {
    ...
    public String getHost() {
        if (plugin == null) return "twojlimit.pl";
    ...
    public String getBuyPremiumUrl() {
        if (plugin == null) return "http://www.twojlimit.pl/";
    ...
    public void handle(final DownloadLink downloadLink, final Account account) throws Exception {
        ...
        /* try twojlimit.pl first */
        if (account == null) {
            if (handleTwojLimit(downloadLink)) return;
    ...
    private boolean handleTwojLimit(DownloadLink link) throws Exception {
        Account acc = null;
        synchronized (LOCK) {
            /* jdpremium enabled */
    ...
    public String getCustomFavIconURL() {
        if (proxyused) return "TwojLimit.pl";
