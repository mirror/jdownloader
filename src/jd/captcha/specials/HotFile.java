...
public class HotFile {
        ...
        for (int x = 0; x < captcha.getWidth(); x++) {
            for (int y = 0; y < captcha.getHeight(); y++) {
                ...
                if (c.getBlue() > 110 && c.getRed() > 110 && c.getGreen() > 110) {
                    if (lastC == null || !((Math.abs(lastpos[0] - x) + Math.abs(lastpos[1] - y)) < 3 && Colors.getColorDifference(lastC.getRGB(), c.getRGB()) < 20))
    ...
    public static Letter[] getLetters(Captcha captcha) {
        ...
        // captcha.cleanByColor(captcha.getAverage(), 9);
        captcha.removeSmallObjects(0.75, 0.75, 6);

        captcha.toBlackAndWhite(0.95);
        // captcha.removeSmallObjects(0.6, 0.6, 3);
