...
public class PixelGrid {
	...
	public void toBlackAndWhite(double contrast) {
		...
		for (int x = 0; x < getWidth(); x++) {
			for (int y = 0; y < getHeight(); y++) {
				...
				setPixelValue(x, y, isElement(getPixelValue(x, y),
						(int) (getMaxPixelValue() * contrast)) ? 0
						: getMaxPixelValue());
	 ...
	 * @param bottomPadding
	 */
	public void crop(int leftPadding, int topPadding, int rightPadding,
			int bottomPadding) {
			...
			for (int y = 0; y < newHeight; y++) {
				...
				for (int gx = 0; gx < faktor; gx++) {
					for (int gy = 0; gy < faktor; gy++) {
						...
						localAVG = UTILITIES.mixColors(localAVG, getPixelValue(
								newX, newY), values, 1);
						values++;
	...
	public void blurIt(int faktor) {
		...
		for (int x = 0; x < getWidth(); x++) {
			for (int y = 0; y < getHeight(); y++) {
				setPixelValue(x, y, newGrid, getAverage(x, y, faktor, faktor),
						owner);
	...
	public void cleanBackgroundByColor(int avg) {
		...
		for (int x = 0; x < getWidth(); x++) {
			for (int y = 0; y < getHeight(); y++) {
				...
				// y)+"_");
				if (dif < (int) (getMaxPixelValue() * owner.getJas().getDouble(
	...
	public Image getFullImage() {
		...
		for (int y = 0; y < getHeight(); y++) {
			for (int x = 0; x < getWidth(); x++) {
				graphics.setColor(new Color(getPixelValue(x, y) == 0 ? 0
						: 0xffffff));
				graphics.fillRect(x, y, 1, 1);
			}
		}
	...
	public Image getImage(int faktor) {
		...
		BufferedImage image = new BufferedImage(getWidth() * faktor,
				getHeight() * faktor, BufferedImage.TYPE_INT_RGB);
		Graphics2D graphics = image.createGraphics();

		for (int y = 0; y < getHeight() * faktor; y += faktor) {
			for (int x = 0; x < getWidth() * faktor; x += faktor) {
				graphics.setColor(new Color(getPixelValue(x / faktor, y
						/ faktor)));
			...
			for (int x = 0; x < getWidth(); x++) {
				...
				if (x == 0 && y == 0 && faktor < 3) {
					...
					if (!isElement(getPixelValue(x, y), (int) (avg * contrast))) {
						setPixelValue(x, y, newGrid, getAverageWithoutPoint(x,
								y, faktor, faktor), this.owner);
					}
				}
			}
		}
		...
		for (int y = 0; y < getHeight(); y++) {
			for (int x = 0; x < getWidth(); x++) {
				...
				if (x == 0 && y == 0 && faktor < 3) {
					...
					if (isElement(getPixelValue(x, y), (int) (avg * contrast))
							&& localAVG >= (contrast * getMaxPixelValue())) {

						setPixelValue(x, y, newGrid, (int) (localAVG),
								this.owner);
					} else {
						setPixelValue(x, y, newGrid, getPixelValue(x, y),
								this.owner);
					}
				}
			}
	 ...
	 * @param pixels Wieviele Pixel Um das Objekt liegen dürfen
	 * @param middel ambesten zwischen 2.1-3
	 */
				...
				if (x < pixels || y < pixels || y > ignorh1 || y < ignorh2) {
					...
					if (isElement(getPixelValue(x, y), avg)) {
						...
						while (c <= pixels && y + i < getHeight()) {
							if (isElement(getPixelValue(x, y + i), avg)
									|| isElement(getPixelValue(x + 1, y + i),
											avg)
									|| isElement(getPixelValue(x - 1, y + i),
											avg)
									|| isElement(
											getPixelValue(x + 1, y + i - 1),
											avg)
									|| isElement(
											getPixelValue(x - 1, y + i - 1),
											avg))
						...
						if (c <= pixels)
							setPixelValue(x, y, newGrid, getMaxPixelValue(),
									this.owner);
						else
							newGrid[x][y] = grid[x][y];
	...
	public void removeSmallObjects(double contrast, double objectContrast) {
		double tmp = owner.getJas().getDouble(
				"objectDetectionMergeSeperatedPartsDistance");
	...
	public Vector<PixelObject> getObjects(double contrast, double objectContrast) {
		...
		if (showdebug)
			w = new ScrollPaneWindow(this.owner);
		if (showdebug)
			...
			for (int y = 0; y < getHeight(); y++) {
				...
				if (getPixelValue(x, y) <= (objectContrast * getMaxPixelValue())) {
					...
					if (lastObject != null) {
						dist = (int) (Math.pow(
								x
										- (lastObject.getXMin() + lastObject
												.getWidth() / 2), 2) + Math
								.pow(y
										- (lastObject.getYMin() + lastObject
												.getHeight() / 2), 2));
					}

					if (lastObject != null
							&& lastObject.getArea() < owner.getJas()
									.getInteger("minimumObjectArea")
							&& dist < Math.pow(owner.getJas().getInteger(
									"minimumLetterWidth") / 2 + 1, 2)) {
						...
						if (JAntiCaptcha.isLoggerActive())
							logger
									.finer("Verfolge weiter Letztes Object: area:"
											+ lastObject.getArea()
											+ " dist: "
											+ dist);

					} else if (owner.getJas().getDouble(
							"objectDetectionMergeSeperatedPartsDistance") > 0.0
							&& lastObject != null
							&& x >= lastObject.getXMin()
							&& x <= (lastObject.getXMin() + lastObject
									.getWidth())) {
						double xDist = Math
								.abs(x
										- (lastObject.getXMin() + lastObject
												.getWidth() / 2));
						double perc = 100.0 * (xDist / (double) (lastObject
								.getWidth() / 2));

						if (perc < owner.getJas().getDouble(
								"objectDetectionMergeSeperatedPartsDistance")) {
	...
	protected Vector<PixelObject> getColorObjects() {
		...
		int running = owner.getJas().getInteger(
				"colorObjectDetectionRunningAverage");
		percent = 5;
		running = 1000;
		...
		logger.info(""
				+ UTILITIES.getColorDifference(new int[] { 0, 0, 204 },
						new int[] { 0, 0, 184 }));
		logger.info(""
				+ UTILITIES.getColorDifference(new int[] { 0, 0, 204 },
						new int[] { 60, 10, 240 }));
			...
			for (int y = 0; y < getHeight(); y++) {
				...
				if (map.get(key) == null) {
					...
					while (iterator.hasNext()) {
						...
						dif = UTILITIES.getColorDifference(bv, UTILITIES
								.hexToRgb(map.get(key2).getAverage()));
						if (dif < bestValue) {
							bestKey = key2;
							bestValue = dif;
		...
		while (iterator.hasNext()) {
			...
			BasicWindow.showImage(map.get(key).toLetter().getImage(3), key + ""
					+ map.get(key).getAverage());
	...
	private void getObject(int x, int y, int[][] tmpGrid, PixelObject object) {
		...
		if (x < 0 || y < 0 || tmpGrid.length <= x || tmpGrid[0].length <= y
				|| tmpGrid[x][y] < 0)
			return;
