...
public class JAntiCaptcha {
	...
	public JAntiCaptcha(String methodsPath, String methodName) {
		methodsPath = UTILITIES.getFullPath(new String[] { methodsPath,
				methodName, "" });
	...
	public static boolean hasMethod(String methodsPath, String methodName) {
		...
		return JDUtilities.getResourceFile(
				methodsPath + "/" + methodName + "/script.jas").exists();
	}
	...
	public void exportDB() {
		...
		while (iter.hasNext()) {
			...
			file = new File(path + "/letterDB_" + this.getMethodName() + "/"
					+ i++ + "_" + letter.getDecodedValue() + ".png");
			file.mkdirs();
			try {
				if (JAntiCaptcha.isLoggerActive())
			...
			if (!(cm instanceof IndexColorModel)) {
				...
				for (int y = 0; y < height; y++) {
					for (int x = 0; x < width; x++) {
						...
						// "+pixel[px]);
						newGrid[x][y] = pixel[px++] < 100 ? 0 : PixelGrid
								.getMaxPixelValue(this);
				...
				letter.setDecodedValue(images[i].getName().split("\\_")[1]
						.split("\\.")[0]);
				letter.clean();
	...
	private void loadMTHFile() {
		File f = JDUtilities.getResourceFile("jd/captcha/methods/"
				+ this.methodDirName + "/" + "letters.mth");
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (childNode.getNodeName().equals("letter")) {
					...
					tmp.setDecodedValue(att.getNamedItem("value")
							.getNodeValue());
					tmp.setBadDetections(Integer.parseInt(UTILITIES
							.getAttribute(childNode, "bad")));
					tmp.setGoodDetections(Integer.parseInt(UTILITIES
							.getAttribute(childNode, "good")));
					letterDB.add(tmp);
					...
					if (JAntiCaptcha.isLoggerActive())
						logger.fine("LetterMap Parsing time: "
								+ (UTILITIES.getTimer() - start2));
				}
			}
		} catch (Exception e) {
			if (JAntiCaptcha.isLoggerActive())
				logger
						.severe("Fehler bein lesen der MTH Datei!!. Methode kann nicht funktionieren!");
	...
	private void getJACInfo() {
		...
		Document doc;
		File f = JDUtilities.getResourceFile("jd/captcha/methods/"
				+ this.methodDirName + "/" + "jacinfo.xml");
	...
	public void showPreparedCaptcha(File captchafile) {
		...
		if (!captchafile.exists()) {
			if (JAntiCaptcha.isLoggerActive())
				logger.severe(captchafile.getAbsolutePath()
						+ " existiert nicht");
			return;
		}
		...
		bw3.add(new ImageComponent(captcha.getImage(1)), UTILITIES.getGBC(0, 2,
				2, 2));
		bw3.add(new JLabel("Farbraum Anpassung"), UTILITIES.getGBC(2, 2, 2, 2));
		jas.executePrepareCommands(captcha);

		bw3.add(new ImageComponent(captcha.getImage(1)), UTILITIES.getGBC(0, 4,
				2, 2));
		bw3.add(new JLabel("Prepare Code ausgeführt"), UTILITIES.getGBC(2, 4,
				2, 2));
		...
		bw3.add(new ImageComponent(captcha.getImageWithGaps(1)), UTILITIES
				.getGBC(0, 6, 2, 2));
		bw3.add(new JLabel("Buchstaben freistellen"), UTILITIES.getGBC(2, 6, 2,
				2));
		bw3.refreshUI();
		if (bw2 != null) {
			bw2.destroy();
		...
		for (int i = 0; i < letters.length; i++) {
			...
			bw2.add(new ImageComponent(letters[i].getImage(jas
					.getInteger("simplifyFaktor"))), UTILITIES.getGBC(
					i * 2 + 2, 0, 2, 2));
		...
		for (int i = 0; i < lcs.length; i++) {
			...
			bw2.add(new ImageComponent(lcs[i].getB().getImage(
					jas.getInteger("simplifyFaktor"))), UTILITIES.getGBC(
					i * 2 + 2, 6, 2, 2));
			bw2.add(new JLabel("Wert:"), UTILITIES.getGBC(0, 8, 2, 2));
			bw2.add(new JLabel(lcs[i].getDecodedValue()), UTILITIES.getGBC(
					i * 2 + 2, 8, 2, 2));
			bw2.add(new JLabel("Proz.:"), UTILITIES.getGBC(0, 10, 2, 2));
			bw2.add(new JLabel(lcs[i].getValityPercent() + "%"), UTILITIES
					.getGBC(i * 2 + 2, 10, 2, 2));
	...
	int trainCaptcha(File captchafile, int letterNum) {
		...
		for (int i = 0; i < letters.length; i++) {
			bw3.setImage(i + 1, 2, letters[i].getImage(jas
					.getInteger("simplifyFaktor")));
		}
		bw3.repack();
		// Decoden. checkCaptcha verwendet dabei die gecachte Erkennung der
		// letters
		...
		if (guess != null /* && guess.length() == getLetterNum() */) {
			...
			for (int i = 0; i < lcs.length; i++) {
				if (lcs[i] != null && lcs[i].getB() != null)
					bw3.setImage(i + 1, 3, lcs[i].getB().getImage(
							jas.getInteger("simplifyFaktor")));
				if (lcs[i] != null && lcs[i].getB() != null)
					bw3.setText(i + 1, 4, lcs[i].getDecodedValue());
				if (lcs[i] != null && lcs[i].getB() != null)
					bw3.setText(i + 1, 5, (double) Math.round(10 * lcs[i]
							.getValityPercent()) / 10.0);
		...
		if (captcha.getValityPercent() >= 0 || true) {
			...
			if (getCodeFromFileName(captchafile.getName(), captchaHash) == null || true) {
				code = UTILITIES.prompt(
						"Bitte Captcha Code eingeben (Press enter to confirm "
								+ guess, guess);
				if (code != null && code.equals(guess))
					code = "";
				else if (code == null) {
					boolean doIt = JOptionPane
							.showConfirmDialog(new JFrame(),
									"Ja (yes) = beenden (close) \t Nein (no) = nächstes Captcha (next captcha)") == JOptionPane.OK_OPTION;
					if (doIt) {
						System.exit(0);
					}
				}
				...
				if (JAntiCaptcha.isLoggerActive())
					logger.warning("captcha code für " + captchaHash
							+ " verwendet: " + code);
		...
		if (code == null) {
			File file = getResourceFile("detectionErrors3/"
					+ (UTILITIES.getTimer()) + "_" + captchafile.getName());
			file.getParentFile().mkdirs();
			captchafile.renameTo(file);
			if (JAntiCaptcha.isLoggerActive())
		...
		if (code.length() != letters.length) {
			File file = getResourceFile("detectionErrors4/"
					+ (UTILITIES.getTimer()) + "_" + captchafile.getName());
			file.getParentFile().mkdirs();
			captchafile.renameTo(file);
			if (JAntiCaptcha.isLoggerActive())
		...
		if (code.indexOf("-") < 0) {
			...
			String newName = captchafile.getParentFile().getAbsolutePath()
					+ UTILITIES.FS + "captcha_" + this.getMethodDirName()
					+ "_code" + code + "." + ext;
	...
	private void sortLetterDB() {
		...
		while (iter.hasNext()) {
			...
			while (iter2.hasNext()) {
				x++;
				if (akt.getDecodedValue().compareToIgnoreCase(
						((Letter) iter2.next()).getDecodedValue()) > 0) {
					ret.add(x, akt);
					akt = null;
					break;
				}
	...
	public Captcha createCaptcha(Image captchaImage) {
		if (captchaImage.getWidth(null) <= 0
				|| captchaImage.getHeight(null) <= 0) {
			if (JAntiCaptcha.isLoggerActive())
				logger
						.severe("Image Dimensionen zu klein. Image hat keinen Inahlt. Pfad/Url prüfen!");
			return null;
		}
	...
	public File getResourceFile(String arg) {
		return JDUtilities.getResourceFile("jd/captcha/methods/"
				+ this.methodDirName + "/" + arg);
	}
	...
	public void saveMTHFile() {
		try {
			...
			if (!UTILITIES.writeLocalFile(this.getResourceFile("letters.mth"),
					xmlString)) {
				if (JAntiCaptcha.isLoggerActive())
	 ...
	 * @return Captchacode
	 */
	public static String getCaptchaCode(Image img, String methodPath,
			String methodname) {
	...
	private boolean isCaptchaInMTH(String captchaHash) {
		...
		Iterator<Letter> iter = letterDB.iterator();
		while (iter.hasNext()) {
			if (((Letter) iter.next()).getSourcehash().equals(captchaHash))
				return true;
		}
	...
	//	LetterComperator[] newLetters = new LetterComperator[letters.length];
		...
		if (letters.length > getLetterNum()) {
			// sortieren
			Collections.sort(newLettersVector,
					new Comparator<LetterComperator>() {
						public int compare(LetterComperator obj1,
								LetterComperator obj2) {

							if (obj1.getValityPercent() < obj2
									.getValityPercent())
								return -1;
							if (obj1.getValityPercent() > obj2
									.getValityPercent())
		...
		captcha.setLetterComperators(newLettersVector
				.toArray(new LetterComperator[] {}));
	...
	private LetterComperator getLetter(Letter letter) {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (Math.abs(tmp.getHeight() - letter.getHeight()) > bvX
						|| Math.abs(tmp.getWidth() - letter.getWidth()) > bvY) {
					// continue;
				}
				...
				if (res == null
						|| lc.getValityPercent() < res.getValityPercent()) {
					if (res != null && res.getValityPercent() < lastPercent) {
						lastPercent = res.getValityPercent();
					}
					res = lc;
					if (jas.getDouble("LetterSearchLimitPerfectPercent") >= lc
							.getValityPercent()) {
						if (JAntiCaptcha.isLoggerActive())
							logger.finer(" Perfect Match: "
									+ res.getB().getDecodedValue()
									+ res.getValityPercent() + " good:"
									+ tmp.getGoodDetections() + " bad: "
									+ tmp.getBadDetections() + " - " + res);
						res
								.setDetectionType(LetterComperator.QUICKSCANPERFECTMATCH);
						res
								.setReliability(lastPercent
										- res.getValityPercent());
						return res;
					}
		...
		if (res != null) {
			if (JAntiCaptcha.isLoggerActive())
				logger.finer(" Normal Match: " + res.getB().getDecodedValue()
						+ " " + res.getValityPercent() + " good:"
						+ res.getB().getGoodDetections() + " bad: "
						+ res.getB().getBadDetections());
			if (JAntiCaptcha.isLoggerActive())
				logger.fine("Letter erkannt in: "
						+ (UTILITIES.getTimer() - startTime) + " ms");
			res.setReliability(lastPercent - res.getValityPercent());
			if (res.getReliability() >= jas
					.getDouble("quickScanReliabilityLimit")
					&& res.getValityPercent() < jas
							.getDouble("quickScanValityLimit")) {
				res.setDetectionType(LetterComperator.QUICKSCANMATCH);
				return res;
			} else {
				if (JAntiCaptcha.isLoggerActive())
					logger
							.warning("Letter nicht ausreichend erkannt. Try Extended "
									+ res.getReliability()
									+ " - "
									+ jas
											.getDouble("quickScanReliabilityLimit")
									...
									+ res.getValityPercent()
									+ "-"
									+ jas.getDouble("quickScanValityLimit"));
	...
	private LetterComperator getLetterExtended(Letter letter) {
		...
		try {
			...
			int line = 0;
			for (angle = UTILITIES.getJumperStart(leftAngle, rightAngle); UTILITIES
					.checkJumper(angle, leftAngle, rightAngle); angle = UTILITIES
					.nextJump(angle, leftAngle, rightAngle, steps)) {
				...
				while (iter.hasNext()) {
					tmp = (Letter) iter.next();
					if (Math.abs(tmp.getHeight() - letter.getHeight()) > jas
							.getInteger("borderVarianceY")
							|| Math.abs(tmp.getWidth() - letter.getWidth()) > jas
									.getInteger("borderVarianceX")) {
						// continue;
					}
					...
					if (this.isShowDebugGui()) {
						...
						w.setImage(3, line, lc.getB().getImage(2));
						w.setText(4, line, lc.getB().getDim());
						w.setImage(5, line, lc.getIntersectionLetter()
								.getImage(2));
						w.setText(6, line, lc.getIntersectionLetter().getDim());
						w.setText(7, line, lc);
						line++;
					...
					if (res == null
						...
						if (jas.getDouble("LetterSearchLimitPerfectPercent") >= lc
							...
							res.setReliability(lastPercent
									- res.getValityPercent());
							if (JAntiCaptcha.isLoggerActive())
								logger.finer(" Perfect Match: "
										+ res.getB().getDecodedValue() + " "
	...
	public void displayLibrary() {
		...
		while (iter.hasPrevious()) {
			...
			w.setText(x, y, tmp.getId() + ": " + tmp.getDecodedValue() + "("
					+ tmp.getGoodDetections() + "/" + tmp.getBadDetections()
					+ ")");
			w
					.setImage(x + 1, y, tmp.getImage(jas
							.getInteger("simplifyFaktor")));
			w.setComponent(x + 3, y, new JButton(new AbstractAction("-" + i++) {
				...
				public void actionPerformed(ActionEvent evt) {
					jac.removeLetterFromLibrary(Integer.parseInt(((JButton) evt
							.getSource()).getText().substring(1)));
					w.destroy();
					jac.displayLibrary();
					jac.saveMTHFile();
				}
	...
	public void runTestMode(File file) {
		...
		BasicWindow.showImage(captcha.getImage(2),
				"Farbtester. Bild sollte Identisch sein");
		...
		Vector<PixelObject> letters = captcha.getBiggestObjects(4, 200, 0.7,
				0.8);
	...
	public void removeBadLetters() {
		...
		while (iter.hasPrevious()) {
			tmp = (Letter) iter.previous();
			if ((tmp.getGoodDetections() == 0 && tmp.getBadDetections() > 0)
					|| ((double) tmp.getBadDetections() / (double) tmp
							.getGoodDetections()) >= jas
							.getDouble("findBadLettersRatio")) {
				if (JAntiCaptcha.isLoggerActive())
					logger.info("bad Letter entfernt: " + tmp.getDecodedValue()
							+ " (" + tmp.getBadDetections() + "/"
							+ tmp.getGoodDetections() + ")");
				iter.remove();
			}
	...
	private String getLetterMapString() {
		...
		for (int x = 0; x < letterMap.length; x++) {
			...
			if (JAntiCaptcha.isLoggerActive())
				logger
						.fine("Db String: " + ((x * 100) / letterDB.size())
								+ "%");
		}
		ret.deleteCharAt(0);
		return ret.toString();
		...
		File[] entries = dir.listFiles(new FileFilter() {
			public boolean accept(File pathname) {
				...
				if (pathname.isDirectory()
						&& new File(pathname.getAbsoluteFile() + UTILITIES.FS
								+ "jacinfo.xml").exists()) {
	...
	public static void testMethod(File file) {
		...
		File captchaDir = UTILITIES.getFullFile(new String[] {
				file.getAbsolutePath(), "captchas" });
		if (JAntiCaptcha.isLoggerActive())
		...
		for (i = 0; i < testNum; i++) {
			...
			if (cap == null) {
				if (JAntiCaptcha.isLoggerActive())
					logger
							.severe("Captcha Bild konnte nicht eingelesen werden");
				continue;
			}
			...
			if (JAntiCaptcha.isLoggerActive())
				logger.info("Erkennung: " + correctLetters + "/" + totalLetters
						+ " = "
						+ UTILITIES.getPercent(correctLetters, totalLetters)
						+ "%");
		}
		w.setText(0, i + 1, "Erkennung: "
				+ UTILITIES.getPercent(correctLetters, totalLetters) + "%");
		w.setText(4, i + 1, "Richtig: " + correctLetters);
		w.setText(5, i + 1, "Falsch: " + (totalLetters - correctLetters));
		UTILITIES.showMessage("Erkennung: " + correctLetters + "/"
				+ totalLetters + " = "
				+ UTILITIES.getPercent(correctLetters, totalLetters) + "%");
	}
