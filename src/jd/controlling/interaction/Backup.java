        ...
        try {
            ...
            if (getBooleanProperty("CONFIG", false)) {
                ...
                for (String element : filenames) {
                    File pass = new File(backuppath + "config/" + element);
                    if (pass.exists()) {
                        pass.delete();
                    }

                    RandomAccessFile datei = new RandomAccessFile(JDUtilities.getJDHomeDirectoryFromEnvironment().getAbsolutePath() + "/config/" + element, "r");
                    RandomAccessFile neudatei = new RandomAccessFile(backuppath + "config/" + element, "rw");
                    while (neudatei.length() < datei.length()) {
                        neudatei.write(datei.read());
                    }
                    datei.close();
