...
class mergeFile extends File {
	...
	public boolean equals(Object obj) {
		if (obj instanceof File) {
			...
			String matcher = name.matches(".*\\.\\a.$") ? name.replaceFirst(
					"\\.a.$", "") : name.replaceFirst("\\.[\\d]+($|\\..*)", "");
			String oMatcher = oName.matches(".*\\.\\a.$") ? oName.replaceFirst(
					"\\.a.$", "") : oName
					.replaceFirst("\\.[\\d]+($|\\..*)", "");
...
public class Unrar extends Interaction implements Serializable {
	...
	private transient static final String NAME = JDLocale.L(
			"interaction.unrar.name", "Extractor");

	public transient static final String PROPERTY_UNRARCOMMAND = "UNRAR_PROPERTY_UNRARCMD",
			PROPERTY_AUTODELETE = "UNRAR_PROPERTY_AUTODELETE",
			PROPERTY_OVERWRITE_FILES = "UNRAR_PROPERTY_OVERWRITE_FILES",
			PROPERTY_ENABLED = "UNRAR_PROPERTY_ENABLED",
			PROPERTY_WAIT_FOR_TERMINATION = "UNRAR_WAIT_FOR_TERMINATION",
			PROPERTY_ENABLE_EXTRACTFOLDER = "UNRAR_PROPERTY_ENABLE_EXTRACTFOLDER",
			PROPERTY_EXTRACTFOLDER = "UNRAR_PROPERTY_EXTRACTFOLDER",
			PROPERTY_DELETE_INFOFILE = "PROPERTY_DELETE_INFOFILE",
			PROPERTY_USE_HJMERGE = "PROPERTY_USE_HJMERGE",
			PROPERTY_DELETE_MERGEDFILES = "PROPERTY_DELETE_MERGEDFILES",
			PROPERTY_AUTOFOLDER_COUNT = "PROPERTY_AUTOFOLDER_COUNT";
	...
	public static Unrar getInstance() {
		if (INSTANCE == null) {
			if (JDUtilities.getConfiguration().getProperty(
					Configuration.PARAM_UNRAR_INSTANCE, null) != null) {
				INSTANCE = (Unrar) JDUtilities.getConfiguration().getProperty(
						Configuration.PARAM_UNRAR_INSTANCE, null);
				return INSTANCE;
			}
			INSTANCE = new Unrar();
		}
	...
	private JUnrar getUnrar() {
		...
		if (lastFinishedDownload != null) {
			...
			Vector<DownloadLink> links = lastFinishedDownload.getFilePackage()
					.getDownloadLinks();
		...
		unrar.autoFolder = JDUtilities.getConfiguration().getIntegerProperty(
				PROPERTY_AUTOFOLDER_COUNT, 2);

		if (JDUtilities.getConfiguration().getBooleanProperty(
				Unrar.PROPERTY_ENABLE_EXTRACTFOLDER, false)) {
			String efolder = JDUtilities.getConfiguration().getStringProperty(
					Unrar.PROPERTY_EXTRACTFOLDER, null);
			if (efolder != null) {
	...
	public boolean getWaitForTermination() {
		return JDUtilities.getConfiguration().getBooleanProperty(
				Unrar.PROPERTY_WAIT_FOR_TERMINATION, false);
	}
	...
	public void run() {
		...
		if (IS_RUNNING) {
			logger
					.warning("Process is in queue, there is already an unrar process running");
		}
		while (IS_RUNNING) {
			if (c++ == 1200) {
				IS_RUNNING = false;
		...
		if (lastFinishedDownload == null) {
			...
			folders.add(JDUtilities.getConfiguration()
		...
		while (iter.hasNext()) {
			...
			if (JDUtilities.getController().isContainerFile(file)) {
				if (JDUtilities.getConfiguration().getBooleanProperty(
						Configuration.PARAM_RELOADCONTAINER, true)) {
					JDUtilities.getController().loadContainerFile(file);
				}
			}
		}
		Boolean DELETE_MERGEDFILES = JDUtilities.getConfiguration()
				.getBooleanProperty(Unrar.PROPERTY_DELETE_MERGEDFILES, true);
		if (lastFinishedDownload != null
				&& JDUtilities.getConfiguration().getBooleanProperty(
						Unrar.PROPERTY_USE_HJMERGE, true)) {
			Merge.mergeIt(new File(lastFinishedDownload.getFileOutput()),
					unrar.followingFiles, DELETE_MERGEDFILES,
					unrar.extractFolder);
			Iterator<mergeFile> miter = mergeFiles.iterator();
			while (miter.hasNext()) {
				mergeFile mergeFile = miter.next();
				Merge.mergeIt(mergeFile, unrar.followingFiles,
						DELETE_MERGEDFILES, unrar.extractFolder);
			}
		}
		IS_RUNNING = false;
		setCallCode(Interaction.INTERACTION_CALL_SUCCESS);
		Interaction
				.handleInteraction(Interaction.INTERACTION_AFTER_UNRAR, null);
	}
