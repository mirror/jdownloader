 ...
 * 
 */
public class JDController implements PluginListener, ControlListener,
		UIListener {
	...
	private void initInteractions() {
		Vector<Interaction> interactions = (Vector<Interaction>) JDUtilities
				.getSubConfig(Configuration.CONFIG_INTERACTIONS).getProperty(
						Configuration.PARAM_INTERACTIONS,
	...
	public int getDownloadStatus() {
		if (watchdog == null || watchdog.isAborted()
				&& downloadStatus == DOWNLOAD_RUNNING) {
	...
	public void controlEvent(ControlEvent event) {
		...
		case ControlEvent.CONTROL_SINGLE_DOWNLOAD_FINISHED:
			...
			if (this.getMissingPackageFiles(lastDownloadFinished) == 0) {
				Interaction
						.handleInteraction(
								Interaction.INTERACTION_DOWNLOAD_PACKAGE_FINISHED,
								this);
			...
			if (lastDownloadFinished.getStatus() == DownloadLink.STATUS_DONE
					&& Configuration.FINISHED_DOWNLOADS_REMOVE
							.equals(JDUtilities
									.getConfiguration()
									.getProperty(
											Configuration.PARAM_FINISHED_DOWNLOADS_ACTION))) {
				logger.info("REM1");
	...
	public void stopDownloads() {
		if (getDownloadStatus() == DOWNLOAD_RUNNING) {
			...
			fireControlEvent(new ControlEvent(this,
					ControlEvent.CONTROL_DOWNLOAD_TERMINATION_ACTIVE, this));

			watchdog.abort();
			setDownloadStatus(DOWNLOAD_NOT_RUNNING);
			fireControlEvent(new ControlEvent(this,
					ControlEvent.CONTROL_DOWNLOAD_TERMINATION_INACTIVE, this));
		}
	}
	...
	public void uiEvent(UIEvent uiEvent) {
		...
		case UIEvent.UI_SAVE_CONFIG:
			...
			JDUtilities.saveObject(null, JDUtilities.getConfiguration(),
					...
					JDUtilities.CONFIG_PATH.split("\\.")[0], "."
							+ JDUtilities.CONFIG_PATH.split("\\.")[1],
		...
		case UIEvent.UI_INTERACT_RECONNECT:
			if (getRunningDownloadNum() > 0) {
				logger
						.info("Es laufen noch Downloads. Breche zum reconnect Downloads ab!");
				stopDownloads();
			}
		...
		case UIEvent.UI_INTERACT_UPDATE:
			new JDInit().doWebupdate(JDUtilities.getConfiguration()
					.getIntegerProperty(Configuration.CID, -1), true);
			break;
		}
	}
	...
	public void saveDownloadLinks(File file) {
		...
		JDUtilities.saveObject(null, downloadLinks, file, "links", "dat",
				Configuration.saveAsXML);
	}
	...
	public String createDLCString(Vector<DownloadLink> links) {
		...
		InputSource inSourceHeader = new InputSource(
				new StringReader(
						"<header><generator><app></app><version/><url></url></generator><tribute></tribute><comment/><category/></header>"));
		InputSource inSourceContent = new InputSource(new StringReader(
				"<content/>"));

		try {
			Document content = factory.newDocumentBuilder().parse(
					inSourceContent);
			Document header = factory.newDocumentBuilder()
					.parse(inSourceHeader);
			Node header_generator_app = header.getFirstChild().getFirstChild()
					.getChildNodes().item(0);
			Node header_generator_version = header.getFirstChild()
					.getFirstChild().getChildNodes().item(1);
			Node header_generator_url = header.getFirstChild().getFirstChild()
					.getChildNodes().item(2);
			header_generator_app.appendChild(header.createTextNode(JDUtilities
					.Base64Encode("JDownloader")));
			header_generator_version.appendChild(header
					.createTextNode(JDUtilities.Base64Encode(JDUtilities
							.getRevision())));
			header_generator_url.appendChild(header.createTextNode(JDUtilities
					.Base64Encode("http://jdownload.ath.cx")));

			Node header_tribute = header.getFirstChild().getChildNodes()
					.item(1);
			...
			if (cfg.getBooleanProperty("ASK_ADD_INFOS", false)) {
				...
				element.appendChild(header.createTextNode(JDUtilities
						.Base64Encode(this.getUiInterface()
								.showUserInputDialog("Uploader Name"))));

				if ((cfg.getStringProperty("UPLOADERNAME", null) != null && cfg
						.getStringProperty("UPLOADERNAME", null).trim()
						.length() > 0)) {
					element = header.createElement("name");
					header_tribute.getFirstChild().appendChild(element);
					element.appendChild(header.createTextNode(JDUtilities
							.Base64Encode(cfg.getStringProperty("UPLOADERNAME",
									null))));
			...
			Node header_comment = header.getFirstChild().getChildNodes()
					.item(2);
			Node header_category = header.getFirstChild().getChildNodes().item(
					3);
			if (cfg.getBooleanProperty("ASK_ADD_INFOS", false)) {

				header_comment.appendChild(header.createTextNode(JDUtilities
						.Base64Encode(this.getUiInterface()
								.showUserInputDialog("Comment"))));

			}
			if (cfg.getBooleanProperty("ASK_ADD_INFOS", false)) {
				header_category.appendChild(header.createTextNode(JDUtilities
						.Base64Encode(this.getUiInterface()
								.showUserInputDialog("Category"))));
			} else {
				header_category.appendChild(header.createTextNode(JDUtilities
						.Base64Encode("various")));
			...
			for (int i = 0; i < packages.size(); i++) {
				...
				if (packages.get(i) == null) {
					filePackage.setAttribute("name", JDUtilities
							.Base64Encode("various"));
				} else {
					filePackage.setAttribute("name", JDUtilities
							.Base64Encode(packages.get(i).getName()));
				}

				content.getFirstChild().appendChild(filePackage);

				Vector<DownloadLink> tmpLinks = this.getPackageFiles(packages
						.get(i), links);
				...
				for (int x = 0; x < tmpLinks.size(); x++) {
					...
					if (packages.get(i) != null) {
						pw.appendChild(content.createTextNode(JDUtilities
								.Base64Encode(packages.get(i).getPassword())));
						comment.appendChild(content.createTextNode(JDUtilities
								.Base64Encode(packages.get(i).getComment())));
			...
			String ret = JDUtilities.xmltoStr(header).substring(ind1)
					+ JDUtilities.xmltoStr(content).substring(ind2);
	...
	public String encryptDLC(String xml) {
		...
		URL[] services;
		try {
			services = new URL[] { new URL("http://dlcrypt3.ath.cx/service.php") };
			...
			while (url < services.length) {
				...
				try {
					...
					RequestInfo ri = Plugin.getRequestWithoutHtmlCode(
							services[url], null, null, false);
					...
					ri = Plugin.postRequest(new URL(ri.getLocation()), null,
							null, null, "jd=1&srcType=jdtc&data=" + key, true);
		...
		if (cipher != null) {
			...
			if (this
					...
					.showConfirmDialog(
							JDLocale
									.L("sys.dlc.success",
											"DLC encryption successfull. Run Testdecrypt now?"))) {
				loadContainerFile(file);
				return;
			}
			return;
	...
	private Vector<DownloadLink> loadDownloadLinks(File file) {
		try {
			if (file.exists()) {
				Object obj = JDUtilities.loadObject(null, file,
						Configuration.saveAsXML);
				if (obj != null && obj instanceof Vector) {
					...
					while (iterator.hasNext()) {
						...
						if (localLink.getStatus() == DownloadLink.STATUS_DONE
								&& Configuration.FINISHED_DOWNLOADS_REMOVE_AT_START
										.equals(JDUtilities
												.getConfiguration()
												.getProperty(
														Configuration.PARAM_FINISHED_DOWNLOADS_ACTION))) {
						...
						try {
							pluginForHost = JDUtilities.getPluginForHost(
									localLink.getHost()).getClass()
									.newInstance();
						...
						try {
							if (localLink.getContainer() != null) {
								pluginForContainer = JDUtilities
										.getPluginForContainer(localLink
												.getContainer());
						...
						if (pluginForContainer != null) {
							localLink
									.setLoadedPluginForContainer(pluginForContainer);
							pluginForContainer.addPluginListener(this);
						}
						if (pluginForHost == null) {
							logger.severe("couldn't find plugin("
									+ localLink.getHost()
									+ ") for this DownloadLink."
									+ localLink.getName());
						}
					}
					return links;
	...
	public void loadContainerFile(File file) {
		...
		Vector<PluginForContainer> pluginsForContainer = JDUtilities
				.getPluginsForContainer();
		Vector<DownloadLink> downloadLinks = new Vector<DownloadLink>();
		PluginForContainer pContainer;
		ProgressController progress = new ProgressController("Containerloader",
				pluginsForContainer.size());
		...
		for (int i = 0; i < pluginsForContainer.size(); i++) {
			...
			progress.setStatusText("Containerplugin: "
					+ pContainer.getPluginName());
			if (pContainer.canHandle(file.getName())) {
				...
				try {
					...
					fireControlEvent(new ControlEvent(this,
							ControlEvent.CONTROL_PLUGIN_DECRYPT_ACTIVE,
							pContainer));
					...
					fireControlEvent(new ControlEvent(this,
							ControlEvent.CONTROL_PLUGIN_DECRYPT_INACTIVE,
							pContainer));
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public boolean initDownloadLinks() {
		...
		downloadLinks = loadDownloadLinks(JDUtilities
				.getResourceFile("links.dat"));
	...
	public Vector<DownloadLink> getPackageFiles(DownloadLink downloadLink) {
		...
		while (iterator.hasNext()) {
			...
			if (downloadLink.getFilePackage() == nextDownloadLink
					.getFilePackage())
	...
	public int getPackageReadyNum(DownloadLink downloadLink) {
		...
		while (iterator.hasNext()) {
			...
			if (downloadLink.getFilePackage() == nextDownloadLink
					.getFilePackage()
					&& nextDownloadLink.getStatus() == DownloadLink.STATUS_DONE)
				i++;
		}
		return i;
	}
	...
	public int getRunningDownloadNum() {
		...
		while (iterator.hasNext()) {
			nextDownloadLink = iterator.next();
			if (nextDownloadLink.getStatus() == DownloadLink.STATUS_DOWNLOAD_IN_PROGRESS)
				ret++;
	...
	public String getCaptchaCodeFromUser(Plugin plugin, File captchaAddress) {
		String captchaCode = uiInterface.getCaptchaCodeFromUser(plugin,
				captchaAddress);
		return captchaCode;
	}
	...
	public String getLastCaptchaImage() {
		if (this.lastCaptchaLoaded == null)
			return "";
	...
	public boolean reconnect() {
		...
		logger.info("Reconnect: "
				+ JDUtilities.getConfiguration().getBooleanProperty(
						Configuration.PARAM_DISABLE_RECONNECT, true));
		if (JDUtilities.getConfiguration().getBooleanProperty(
				Configuration.PARAM_DISABLE_RECONNECT, false)) {
			logger
					.finer("Reconnect is disabled. Enable the CheckBox in the Toolbar to reactivate it");
			return false;
		}
		...
		String type = JDUtilities.getConfiguration().getStringProperty(
				Configuration.PARAM_RECONNECT_TYPE, null);
		if (type == null) {
		...
		if (ret) {
			...
			while (iterator.hasNext()) {
				...
				if (i.getRemainingWaittime() > 0) {
					...
					logger.finer("REset GLOBALS: "
							+ ((PluginForHost) i.getPlugin()));
		...
		if (ret) {
			Interaction.handleInteraction(
					Interaction.INTERACTION_AFTER_RECONNECT, this);
		}
		return ret;
	}
