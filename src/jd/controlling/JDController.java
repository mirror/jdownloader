...
public class JDController implements PluginListener, ControlListener,
	...
	private void initInteractions() {
		Vector<Interaction> interactions = JDUtilities.getConfiguration()
				.getInteractions();
	...
	public void saveDLC(File file) {
		...
		if (cfg.getBooleanProperty("ASK_ADD_INFOS", false)) {
			if ((cfg.getStringProperty("UPLOADERNAME", null) == null || cfg
					.getStringProperty("UPLOADERNAME", null).trim().length() == 0)) {

			} else {
				xml += "<name>"
						+ JDUtilities.Base64Encode(this.getUiInterface()
								.showUserInputDialog("Uploader Name"))
						+ "</name>";
				xml += "<name>"
						+ JDUtilities.Base64Encode(cfg.getStringProperty(
								"UPLOADERNAME", null)) + "</name>";
		...
		if (cfg.getBooleanProperty("ASK_ADD_INFOS", false)) {
			xml += "<comment>"
					+ JDUtilities.Base64Encode((this.getUiInterface()
							.showUserInputDialog("Comment"))) + "</comment>";
		}
		if (cfg.getBooleanProperty("ASK_ADD_INFOS", false)) {
			xml += "<category>"
					+ JDUtilities.Base64Encode((this.getUiInterface()
							.showUserInputDialog("Category"))) + "</category>";
		} else {
			xml += "<category>" + JDUtilities.Base64Encode("various")
					+ "</category>";
		...
		for (int i = 0; i < packages.size(); i++) {
			xml += "<package name=\""
					+ JDUtilities.Base64Encode(packages.get(i).getName())
					+ "\">";
		...
		try {
			URL[] services = new URL[] {
					new URL("http://dlcrypt.ath.cx/service.php"),
					new URL("http://recrypt1.ath.cx/service.php") };
			int url = 0;
			while (url < services.length) {
				...
				ri = Plugin.postRequest(new URL(ri.getLocation()), null, null,
						null, "jd=1&srcType=jdtc&data=" + key, true);
					...
					if (dlcKey.trim().length() < 80) {
						JDUtilities
								...
								.showMessageDialog(
										JDLocale
												.L(
														"sys.dlc.error_version",
														"DLC Encryption fehlgeschlagen. Bitte stellen Sie sicher dass Sie die aktuellste JD-Version verwenden."));
						return;
					}
						...
						try {
							if (localLink.getContainer() != null) {
								...
								if (pluginForContainer != null) {
									pluginForContainer = pluginForContainer
											.getPlugin(localLink
													.getContainerFile());
									// pluginForContainer.
									pluginForContainer.initContainer(localLink
											.getContainerFile());
									pluginForContainer
