...
public class IPConnectionState {
    ...
    /**
     * CReates an Entry for an invalid ip. For example offline, or wrong ip syntax.
     *
     * @throws NullPointerException
     *             if e is null
     * @param e
    ...
    private boolean equalsCause(final IPConnectionState currentIP) {
        ...
        if (this.cause.getMessage().equals(currentIP.cause.getMessage())) {
            return true;
        }
        return false;
    }
    ...
    /**
     * return true, if the ip, or the error cause are the same. returns falls, only if the COnnectionstate is different.
     *
     * @param currentIP
     * @return
     */
    ...
    /**
     * returns if this connections state is offline, and has NO valid external ip
     *
     * @return
     */
