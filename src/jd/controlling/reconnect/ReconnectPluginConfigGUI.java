...
public class ReconnectPluginConfigGUI extends SwitchPanel implements ActionListener, DefaultEventListener<StorageEvent<?>> {
    ...
    private void initGUI() {
        ...
        this.add(new JLabel(T._.jd_controlling_reconnect_plugins_ReconnectPluginConfigGUI_initGUI_comboboxlabel()), "split 3,shrinkx,gapleft 37");
        ...
        this.add(Factory.createHeader(T._.gui_config_reconnect_test(), JDTheme.II("gui.images.reconnect_selection", 32, 32)), "spanx");

        final JPanel p = new JPanel(new MigLayout("ins 0, wrap 6", "[]5[right]20[right]20[right]20[right]20[right]", "[][]"));

        p.add(this.btnTest = new JButton(T._.gui_config_reconnect_showcase_reconnect()), "spanx,shrinkx, aligny top,wrap");
        this.btnTest.setIcon(JDTheme.II("gui.images.config.reconnect", 16, 16));
        this.btnTest.addActionListener(this);

        p.add(this.lblDuration = new JLabel(T._.gui_config_reconnect_showcase_time()));
        this.lblDuration.setEnabled(false);

        p.add(this.lblTime = new JLabel("---"));
        this.lblTime.setEnabled(false);

        p.add(new JLabel(T._.gui_config_reconnect_showcase_currentip()));
        ...
        p.add(this.lblStatusMessage = new JLabel(T._.gui_config_reconnect_showcase_message_none()), "spanx 2");
        this.lblStatusMessage.setEnabled(false);

        p.add(this.lblBeforeIpLabel = new JLabel(T._.gui_config_reconnect_showcase_lastip()));
    ...
    private void testReconnect() {
        ...
        final ProgressController progress = new ProgressController(T._.gui_warning_reconnect_pleaseWait(), 100, "gui.images.reconnect");

        ReconnectPluginConfigGUI.LOG.info("Start Reconnect");
        this.lblStatusMessage.setText(T._.gui_warning_reconnect_running());
            ...
            public void run() {
                ...
                if (Reconnecter.getInstance().forceReconnect()) {
                    if (JSonWrapper.get("DOWNLOAD").getBooleanProperty(Configuration.PARAM_GLOBAL_IP_DISABLE, false)) {
                        progress.setStatusText(T._.gui_warning_reconnectunknown());
                    } else {
                        progress.setStatusText(T._.gui_warning_reconnectSuccess());
                    }
                    new EDTRunner() {
                        ...
                        protected void runInEDT() {
                            if (JSonWrapper.get("DOWNLOAD").getBooleanProperty(Configuration.PARAM_GLOBAL_IP_DISABLE, false)) {
                                ReconnectPluginConfigGUI.this.lblStatusMessage.setText(T._.gui_warning_reconnectunknown());
                            } else {
                                ReconnectPluginConfigGUI.this.lblStatusMessage.setText(T._.gui_warning_reconnectSuccess());
                            }
                    ...
                    progress.setStatusText(T._.gui_warning_reconnectFailed());
                    progress.setColor(Color.RED);
                    new EDTRunner() {

                        @Override
                        protected void runInEDT() {
                            ReconnectPluginConfigGUI.this.lblStatusMessage.setText(T._.gui_warning_reconnectFailed());
