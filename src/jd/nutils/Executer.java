...
public class Executer extends Thread implements Runnable {
	public static final String CODEPAGE = OSDetector.isWindows() ? "ISO-8859-1"
			: "UTF-8";
		...
		public void run() {
			...
			try {
				fireEvent(
						dynbuf,
						0,
						this == Executer.this.sbeObserver ? Executer.LISTENER_ERRORSTREAM
				...
				while (!eof) {
					...
					if (num <= 0) {
						...
						try {
							line = new String(dynbuf.getLast(num), codepage)
									.trim();
	...
	public void run() {
		...
		if (runIn != null && runIn.length() > 0) {
			if (new File(runIn).exists()) {
				...
				if (new File(params.get(0)).getParentFile().exists()) {
					...
					System.out.println("Working directory " + runIn
							+ " does not exist!");
				}
			}
		}
		...
		try {
			...
			sbeObserver = new StreamObserver(process.getErrorStream(),
					errorStreamBuffer);
			sbeObserver.setName(this.getName() + " ERRstreamobserver");
			sboObserver = new StreamObserver(process.getInputStream(),
					inputStreamBuffer);
			...
			interrupted();
			while ((sbeObserver != null && this.sbeObserver.isAlive())
					|| (sboObserver != null && this.sboObserver.isAlive())) {
				Thread.sleep(50);
			}
