 ...
 */
  ...
  */
   ...
   * @return Linked List of Diff objects.
   */
  protected LinkedList<Diff> diffCompute(String text1, String text2,
                                          boolean checkLines) {
    ...
    for (int d = 0; d < maxD; d++) {
      ...
      for (int k = -d; k <= d; k += 2) {
        ...
        if (x == text1.length() && y == text2.length()) {
          ...
          LinkedList<Diff> a = diffPath1(vMap1, text1.substring(0, x),
                                          text2.substring(0, y));
      ...
      if (doubleEnd) {
        ...
        for (int k = -d; k <= d; k += 2) {
          ...
          if (done) {
            ...
            LinkedList<Diff> a
                = diffPath1(vMap1, text1.substring(0, text1.length() - x),
                             text2.substring(0, text2.length() - y));
            a.addAll(diffPath2(vMap2, text1.substring(text1.length() - x),
   ...
   */
  public LinkedList<Diff> diffPath1(List<Set<Long>> vMap,
      ...
      while (true) {
        if (vMap.get(d).contains(diffFootprint(x - 1, y))) {
          ...
          assert (text1.charAt(x) == text2.charAt(y))
                 : "No diagonal.  Can't happen. (diffPath1)";
   ...
   * @return LinkedList of Diff objects.
   */
  public LinkedList<Diff> diffPath2(List<Set<Long>> vMap,
                                        String text1, String text2) {
    LinkedList<Diff> path = new LinkedList<Diff>();
    int x = text1.length();
    int y = text2.length();
  ...
  public void diffCleanupSemantic(LinkedList<Diff> diffs) {
    ...
    while (thisDiff != null) {
      if (thisDiff.operation == Operation.EQUAL) {
        ...
        if (lastequality != null && (lastequality.length() <= lengthChanges1)
            && (lastequality.length() <= lengthChanges2)) {
    ...
    while (thisDiff != null) {
      if (thisDiff.operation == Operation.EQUAL) {
        ...
        if (lastequality != null
            && ((pre_ins && preDel && post_ins && postDel)
                || ((lastequality.length() < diffEditCost / 2)
                    && ((pre_ins ? 1 : 0) + (preDel ? 1 : 0)
                        + (post_ins ? 1 : 0) + (postDel ? 1 : 0)) == 3))) {
          //System.out.println("Splitting: '" + lastequality + "'");
          // Walk back to offending equality.
          while (thisDiff != equalities.lastElement()) {
            thisDiff = pointer.previous();
  ...
  public int matchBitap(String text, String pattern, int loc) {
    ...
    if (bestLoc != -1) {
      scoreThreshold = Math.min(matchBitapScore(0, bestLoc, loc,
          scoreTextLength, pattern), scoreThreshold);
    }
    ...
    for (int d = 0; d < pattern.length(); d++) {
      ...
      binMid = binMax;
      while (binMin < binMid) {
        if (matchBitapScore(d, binMid, loc, scoreTextLength, pattern)
            < scoreThreshold) {
          binMin = binMid;
        } else {
          binMax = binMid;
      ...
      for (int j = finish - 1; j >= start; j--) {
        ...
        if ((rd[j] & matchmask) != 0) {
          double score = matchBitapScore(d, j, loc, scoreTextLength,
                                          pattern);
      ...
      if (matchBitapScore(d + 1, loc, loc, scoreTextLength, pattern)
          > scoreThreshold) {
   ...
   */
  private double matchBitapScore(int e, int x, int loc,
                                  int scoreTextLength, String pattern) {
    int d = Math.abs(loc - x);
    return (e / (float) pattern.length() / matchBalance)
        + (d / (float) scoreTextLength / (1.0 - matchBalance));
  }
