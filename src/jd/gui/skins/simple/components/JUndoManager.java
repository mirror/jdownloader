...
public class JUndoManager {
     ...
     * is, characters entered sequentially will be grouped together and undone
     * as a group. Any attribute changes will be considered as part of the group
     * and will therefore be undone when the group is undone.
     */
    public static class CompoundUndoManager extends UndoManager implements UndoableEditListener, DocumentListener {
        ...
        /*
         * The Action to Undo changes to the Document. The state of the Action
         * is managed by the CompoundUndoManager
         */
        ...
        /*
         * The Action to Redo changes to the Document. The state of the Action
         * is managed by the CompoundUndoManager
         */
        ...
        //
        /*
         * Updates to the Document as a result of Undo/Redo will cause the Caret
         * to be repositioned
         */
    ...
    private static HashMap<JTextComponent, CompoundUndoManager> managermap = new HashMap<JTextComponent, CompoundUndoManager>();

    public static synchronized void addManager(JTextComponent comp) {
        synchronized (managermap) {
            if (managermap.containsKey(comp)) return;
            managermap.put(comp, new CompoundUndoManager(comp));
        }
    }

    public static synchronized void delManager(JTextComponent comp) {
        synchronized (managermap) {
            if (!managermap.containsKey(comp)) return;
            managermap.remove(comp);
        }
    }

    public static synchronized Action Undo(final JTextComponent comp) {
        synchronized (managermap) {
            if (managermap.containsKey(comp)) {
                if (managermap.get(comp).canUndo()) {
                    try {
                        return managermap.get(comp).getUndoAction();
                    } catch (Exception e) {
                    }
                }
            }
    ...
    public static synchronized boolean canUndo(JTextComponent comp) {
        synchronized (managermap) {
            if (managermap.containsKey(comp)) return managermap.get(comp).canUndo();
        }
        return false;
    }

    public static synchronized boolean canRedo(JTextComponent comp) {
        synchronized (managermap) {
            if (managermap.containsKey(comp)) return managermap.get(comp).canRedo();
        }
        return false;
    }

    public static synchronized Action Redo(JTextComponent comp) {
        synchronized (managermap) {
            if (managermap.containsKey(comp)) {
                if (managermap.get(comp).canRedo()) {
                    try {
                        return managermap.get(comp).getRedoAction();
                    } catch (Exception e) {
                    }
                }
            }
