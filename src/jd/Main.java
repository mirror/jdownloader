...
public class Main {
	...
	public static void main(String args[]) {
		...
		if (System.getProperty("os.name").toLowerCase().indexOf("mac") >= 0) {
			...
			logger
					.info("com.apple.mrj.application.apple.menu.about.name=jDownloader");
			...
			if (currentArg.equals("--new-instance") || currentArg.equals("-n")) {
				...
				if (currentArg.equals("--rotate") || currentArg.equals("-r")) {
					...
					if (currentArg.matches("[\\d]+"))
						rtime = (int) Integer.parseInt(currentArg);
					else
						rtime = 0;
		...
		if (!newInstance
				&& tryConnectSocketClient(JDUtilities.arrayToString(args, ";"))) {
	...
	private static void extract(String path, long rtime) {
		while(true)
		{
		...
		if (path != null)
			unrar.extractFolder = new File(path);
	...
	private void go() {
		...
		/*
		 * Übergangsfix. Die Interactiosn wurden in eine subconfig verlegt.
		...
		logger
				.info("jd.appDir="
						+ JDUtilities.getCurrentWorkingDirectory(null));
		...
		try {
			logger
					.info(Plugin
							.headRequest(
									new URL(
											"http://share.gulli.com/files/989404514/Indi.part11.rar.html"),
									null, null, false).getHtmlCode());
	...
	private static Boolean tryConnectSocketClient(String args) {
		...
		logger.info("Socket search lasts "
			...
			public void run() {
				...
				while (success == false && tries < 10) {
					...
					try {
						...
						serverSocket.bind(new InetSocketAddress("localhost",
								port));
				...
				try {
					...
					while (acceptSuccess == true) {
						...
						try {
							...
							logger.info(clientSocket.getLocalAddress() + " - "
									+ clientSocket.getInetAddress());
							if (!clientSocket.getLocalAddress().equals(
									clientSocket.getInetAddress())) {
								clientSocket.close();
								break;
	...
	/**
	 * 
	 * TODO - beim adden im hintergrund bleiben
	 * 
	 */
	...
	private static void showCmdHelp() {
		...
		String[][] help = new String[][] {
				...
				{
						"-e --extract <targetPath>\t -r --rotate <seconds> (optional)\t",
