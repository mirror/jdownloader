...
public class Main {
    ...
    private void list() {
        ...
        // <MENU>
        // <CAPTION>Sub Menu 1 Caption</CAPTION>
        // <ICON_URL></ICON_URL>
        // <HINT></HINT>
        ArrayList<String> prev = new ArrayList<String>();
        prev.add("4shared.com");
        prev.add("uploaded.to");
        prev.add("rapidshare.com");
        prev.add("megaupload.com");
        prev.add("hotfile.com");
        prev.add("share-online.biz");
        prev.add("sharingmatrix.com");
        prev.add("netload.in");

        for (HostPluginWrapper w : HostPluginWrapper.getHostWrapper()) {
            if (prev.contains(w.getHost())) {
                sb.append("<MENU_ITEM>\r\n");
                sb.append("   <CAPTION>" + w.getHost() + "</CAPTION>\r\n");
                sb.append("   <ICON_URL>http://jdownloader.org/_media/fav/" + w.getHost().toLowerCase() + ".png</ICON_URL>\r\n");
                sb.append("   <DATA>\r\n");
                sb.append("      <TYPE>LINK</TYPE> \r\n");
                sb.append("      <LINK>\r\n");
                sb.append("         <TARGET>NEW</TARGET>\r\n");
                sb.append("         <URL>http://jdownloader.org/hoster/" + w.getHost().toLowerCase() + "</URL>\r\n");
                sb.append("      </LINK>\r\n");
                sb.append("   </DATA>\r\n");
                sb.append("</MENU_ITEM>\r\n\r\n");
            }
        ...
        sb.append("<SEPARATOR>SEP</SEPARATOR>");
        ArrayList<HostPluginWrapper> list = HostPluginWrapper.getHostWrapper();
        int sep = 15;
        int steps = list.size() / sep;
        String user = "coalado";
        String pass = "4BSainOqlQPapY69ThQ0";
        Browser br = new Browser();
        wikiLogin(br, user, pass);
        for (int stepcount = 0; stepcount < sep; stepcount++) {
            sb.append("<MENU>");
            char start = list.get(stepcount * steps).getHost().toUpperCase().charAt(0);
            char end = list.get(Math.min((stepcount + 1) * steps, list.size() - 1)).getHost().toUpperCase().charAt(0);

            sb.append("<CAPTION>" + start + "-" + end + "</CAPTION>");
            sb.append("<ICON_URL></ICON_URL>");
            sb.append("<HINT></HINT>");

            for (int i = stepcount * steps; i <= Math.min((stepcount + 1) * steps, list.size() - 1); i++) {
                HostPluginWrapper w = list.get(i);
                try {
                    new Browser().getPage("http://" + w.getHost());
                    // createWiki(br, w, user, pass);
                    sb.append("<MENU_ITEM>\r\n");
                    sb.append("   <CAPTION>" + w.getHost() + "</CAPTION>\r\n");
                    sb.append("   <ICON_URL>http://jdownloader.org/_media/fav/" + w.getHost().toLowerCase() + ".png</ICON_URL>\r\n");
                    sb.append("   <DATA>\r\n");
                    ...
                    System.out.println("Bad HOst: " + w.getHost());
                }
            }
    ...
    private void wikiLogin(Browser br, String user, String pass) {
        try {
    ...
    private void createWiki(Browser br, HostPluginWrapper w, String user, String pass) {

        try {
            br.getPage("http://jdownloader.net:8081/hoster/" + w.getHost() + "?do=edit");
            if (br.containsHTML("created") || br.containsHTML("auto")) {
                br.getPage("http://jdownloader.net:8081/hoster/" + w.getHost() + "?do=edit&s=lng_en");
                Form[] forms = br.getForms();
                forms[0].setPreferredSubmit(0);
                StringBuilder t = new StringBuilder();

                if (w.getHost().equalsIgnoreCase("adrive.com")) {

                    int dot = 1;
                }
                Browser loader = new Browser();
                loader.forceDebug(true);
                loader.setFollowRedirects(true);
                loader.getPage("http://en.wikipedia.org/w/index.php?title=Special%3ASearch&search=" + w.getHost() + "&go=Go");
                // {
                t.append("\r\n\r\n\r\n /*\r\nRemove this if you edit this page: auto\r\n*/\r\n\r\n\r\n");
                t.append("====== " + w.getHost() + "======");
                if (!loader.containsHTML("Search results - Wikipedia")) {

                    String title = loader.getRegex("<title>(.*?) \\- Wikipedia, the free encyclopedia</title>").getMatch(0);

                    loader.getPage(loader.getURL() + "?printable=yes");
                    String text = loader.getRegex("<.?table.?>.*?(<p>.*?)<table id=\"toc\"").getMatch(0);
                    if (text == null) {
                        text = loader.getRegex("<p>(.*?)</p>").getMatch(0);
                    ...
                    loader = null;
                    if (text != null) {
                        t.append("\r\n<blockquote from: wikipedia.org - " + title + ">" + text + "</blockquote>\r\n\r\n");
                    }
                    // }
                    // 
                ...
                t.append("\r\n===== Premium =====");
                t.append("\r\n^JD Support^Resumeable^Paralell Downloads^Chunkload^");
                t.append("\r\n|" + y(w.getPlugin().isPremiumEnabled()) + "|" + y(true) + "|" + y(w.getPlugin().getMaxSimultanPremiumDownloadNum() > 1) + "|" + y(true) + "|");

                t.append("\r\n===== Free =====");
                t.append("\r\n^JD Support^Resumeable^Paralell Downloads^Chunkload^");
                t.append("\r\n|YES|NO|" + y(w.getPlugin().getMaxSimultanFreeDownloadNum() > 1) + "|" + y(w.getPlugin().getMaxSimultanFreeDownloadNum() > 1) + "|");
                t.append("\r\n===== Links =====");
                t.append("\r\n  * [[" + "http://" + w.getHost() + "|" + w.getHost().split("\\.")[0] + " Website]]");

                t.append("\r\n  * [[" + "http://svn.jdownloader.org/repositories/changes/jd/trunk/src/jd/plugins/hoster/" + w.getPlugin().getClass().getSimpleName() + ".java|Plugin History]]");
                if (w.getPlugin().getBuyPremiumUrl() != null) {

                    t.append("\r\n  * [[" + w.getPlugin().getBuyPremiumUrl() + "|Buy Premium]]");
                }
                t.append("\r\n===== Issues =====\r\n");
                t.append("<html><div style=\"overflow:hidden\"><iframe frameborder=\"0\" scrolling=\"no\" src=\"http://svn.jdownloader.org/search/index/jd?q=" + w.getHost() + "\" style=\"width:660px;height:1650px;margin-top:-320px\"></iframe></div></html>");
                forms[0].getInputField("wikitext").setValue(t.toString());
    ...
    private String y(boolean premiumEnabled) {
        // TODO Auto-generated method stub
        return premiumEnabled ? "YES" : "NO";
    }
