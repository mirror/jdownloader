...
public class MyBot extends PircBot {
	...
	public File log = new File("ircLog.txt");
			...
			public void run() {
				try {
					...
					if (message.matches("[\\s]*!last [\\d]+")) {
						...
						String[] logr = Utilities.getLocalFile(log).split(
								"\r\n");
						if (n > 30) {
							sendMassmes(
									sender,
					...
					if (message.matches("[\\s]*!rm ignore.*")) {
						...
						try {
							if (message.matches("[\\s]*!del[\\s]*[\\d]+")) {
								...
								sendMassmes(sender, sender, messagesdb, 0,
										0);
							}
						} catch (Exception e) {
							e.printStackTrace();
						...
						if (settings.autoOp.contains(name)) {
							sendMassmes(sender, sender, name
									+ " ist bereichts in der autoopliste",
									0);
							return;
						}
						if (userRights(sender, ch2) != UserInfo.OP) {
						...
						if (settings.autoVoice.contains(name)) {
							sendMassmes(sender, sender, name
									+ " ist bereichts in der autovoiceliste",
									0);
							return;
						}
						if (userRights(sender, ch2) != UserInfo.OP) {
					...
					} else if (message.matches("[\\s]*!addprem .*:.*\\@.*")) {
						String[] info = new Regexp(message,
								"[\\s]*!addprem[\\s]*(.*?)[\\s]?:[\\s]?(.*?)[\\s]?\\@[\\s]?(.*)")
								.getMatches()[0];
						if (info == null)
					...
					if (channel.charAt(0) == '#') {
						...
						try {
							Utilities.writeLocalFile(log, "(" + dateS + ") "
									+ sender + ": " + message + "\r\n", true);
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					if (message.matches("[\\s]*![\\S]{0,8}to[\\S]*[\\s]*.*")) {
						...
						try {
							...
							try {
								String name = new Regexp(message,
										"[\\s]*!wiki[\\s]*(.*)").getFirstMatch().replaceAll("\\s", "_");
