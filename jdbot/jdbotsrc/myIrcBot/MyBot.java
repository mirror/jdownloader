...
public class MyBot extends PircBot {
	...
	public String getSvn() {
		RequestInfo post;
		try {
			post = Utilities.getRequest(new URL(
					"http://www.syncom.org/projects/jdownloader/timeline"),
					null, null, false);
			String[] svn = post
					.getRegexp(
							"<h2>([\\d]+)/([\\d]+)/[\\d]+:</h2><dl><dt class=\"changeset\"><a href=\".*?\"><span class=\"time\">([\\d]+):([\\d]+)</span> Changeset <em>.([\\d]+).</em> by (.*?)</a></dt><dd class=\"changeset\">(.*?)</dd>")
					.getMatches()[0];
			settings.svnRev = Integer.parseInt(svn[4]);
			Utilities.saveObject(settings, config);
			return "Es gab eine Quellcodeänderung von "
					+ svn[5]
					+ " am "
					+ svn[1]
					+ "."
					...
					+ svn[3]
					+ " Uhr "
					+ (svn[6].endsWith("--") ? "" : svn[5]
							+ ": \""
							+ svn[6].replaceAll("[\n\r]", " ").replaceAll(
									"\\&hellip\\;", "...") + "\" ")
					+ ((settings.svnRev > version) ? " es wird bald die Version 0."
							+ ((double) settings.svnRev)
							/ 1000
							+ " oder höher verfügbar sein!"
							: "");
				...
				try {
					for (int i = 0; i < settings.autoVoice.size(); i++) {
						if (settings.autoVoice.get(i).equals(sender)) {
							if (userRights(sender, channel) > 0)
						...
						try {
							...
							if (sender.matches("(?is).*" + mess[0] + ".*")) {
								...
								if (nachri.matches(".*%%cmd%%.*")) {
									try {
										String reg = new Regexp(nachri,
												"%%cmd%%(.*)").getFirstMatch();
										onMessage(channel, sender, login,
												hostname, reg);
										return;
									} catch (Exception e) {
										e.printStackTrace();
								...
								if (nouser)
									...
									sendMassmes(sender, channel, sender + ": "
											+ nachri, 0);
	...
	public boolean fastLogedin(String sender, String channel) {
		...
		for (int i = 0; i < users.length; i++) {
			if (users[i].equals(sender)) {
				if (users[i].isOp() || users[i].hasVoice())
					return true;
				break;
			}
		}
	...
	public void onMessage(final String channel, final String sender,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					final String ch2 = (channel.equals(sender)) ? Channel[0]
							: channel;
					// System.out.println(message);
					if (message.matches("[\\s]*!addprem .*:.*\\@.*")
							|| new Regexp(
									message,
									"[\\s]*﻿﻿Account[\\s]*(created|angelegt)[\\s]*.LOGIN.(.*?)[\\s]*(PASSWORT|PASSWORD)(.*?)[\\s]*.(Please.write.down|Bitte notieren).")
									.count() > 0) {
						String[] info;
						// boolean b =
						...
						if (new Regexp(
								...
								"[\\s]*Account[\\s]*(created|angelegt[\\s]*.LOGIN.(.*?)[\\s]*(PASSWORT|PASSWORD)(.*?)[\\s]*.(Please.write.down|Bitte notieren).")
								.count() > 0) {
							String[] rs = new Regexp(message, "[\\s]*﻿﻿Account[\\s]*(created|angelegt)[\\s]*.LOGIN.(.*?)[\\s]*(PASSWORT|PASSWORD)(.*?)[\\s]*.(Please.write.down|Bitte notieren).")
							.getMatches()[0];
							info = new String[] { rs[1], rs[3],
									"rapidshare.com" };
						} else
							info = new Regexp(message,
									"[\\s]*!addprem[\\s]*(.*?)[\\s]?:[\\s]?(.*?)[\\s]?\\@[\\s]?(.*)")
									.getMatches()[0];
						if (info == null)
							return;
							...
							sendMassmes(
									sender,
									sender,
									"der premiumaccount ist schon in der liste",
									0);
							return;
						}
						if (userRights(sender, ch2) != UserInfo.OP) {
							sendMassmes(sender, sender,
									"diese funktion dürfen nur ops verwenden",
									0);
							return;
						}
						settings.premAccs.add(info);
						Utilities.saveObject(settings, config);
						sendMassmes(sender, sender, info[0] + ":" + info[1]
								+ " wurde zu den Premiumaccounts hinzugefügt",
								0);
						return;
					}

					if (delayedUserMsg)
						delayedUserMsg = false;
					if (delayedMsg && fastLogedin(sender, ch2))
						delayedMsg = false;
					if (message.matches("[\\s]*!last [\\d]+")) {
						int n = Integer.parseInt(new Regexp(message,
								"last ([\\d]+)").getFirstMatch());
						String[] logr = Utilities.getLocalFile(getLog(channel))
								.split("\r\n");
						if (n > 30) {
							sendMassmes(
									sender,
									sender,
									"es sind maximal 30 Nachrichten erlaubt verwenden sie stattdessen !log",
									0);
							return;
						}
						String out = "";
						...
						return;
					} else if (message.matches("[\\s]*!log[\\s]*")
							&& logedin(sender, ch2)) {
						try {
						...
						if (message.matches("[\\s]*!svn[\\s]*rev[\\s]*[\\d]+")
								&& logedin(sender, ch2)) {
							version = Integer.parseInt(new Regexp(message,
									"[\\s]*!svn[\\s]*rev[\\s]*([\\d]+)")
									.getFirstMatch());
						} else {
					...
					if (message.matches("[\\s]*!rm ignore.*")) {
						...
						try {
							if (message.matches("[\\s]*!rm ignore[\\s]*[\\d]+")) {
								...
								sendMassmes(
										sender,
										sender,
										settings.ignore.get(n)
												+ " wurde aus der ignoreierliste entfernt",
										0);
								settings.ignore.remove(n);
								Utilities.saveObject(settings, config);
							} else if (message
									.matches("[\\s]*!rm ignore[\\s]*[^\\s].*")) {
								String name = new Regexp(message,
										"[\\s]*!rm ignore[\\s]*(.*)")
										.getFirstMatch().trim();
						...
						try {
							if (message.matches("[\\s]*!del[\\s]*[\\d]+")) {
								...
								sendMassmes(
										...
										messages.get(n)
												+ " :"
												+ " wurde aus der Nachrichtenliste entfernt",
										0);
								messages.remove(n);
								...
								for (int i = 0; i < messages.size(); i++) {
									...
									if (ob.regExp.contains(name)) {
										...
										sendMassmes(
												...
												sender,
												ob
														+ " wurde aus der messageliste entfernt",
												0);
									}
								}
								Utilities.saveObject(messages, messageFile);
						...
						try {
							if (message
								...
								sendMassmes(
										sender,
										sender,
										settings.joinMessages.get(n)[0]
												+ "|"
												+ settings.joinMessages.get(n)[1]
												+ " : wurde aus der joinliste entfernt",
										0);
								settings.joinMessages.remove(n);
								Utilities.saveObject(settings, config);
							} else if (message
									.matches("[\\s]*!rm[\\s]join[\\s]*[^\\s].*")) {
								String name = new Regexp(message,
										"[\\s]*!rm[\\s]join[\\s]*(.*)")
										.getFirstMatch().trim();
								for (int i = 0; i < settings.joinMessages
										.size(); i++) {
									...
									if (ob[0].matches("(?is).*" + name + ".*")) {
										...
										sendMassmes(
												...
												ob[0]
														+ "|"
														+ ob[1]
														+ " wurde aus der joinMessagesliste entfernt",
												0);
									}
									Utilities.saveObject(settings, config);
								}
								...
								for (int i = 0; i < settings.joinMessages
										.size(); i++) {
									messagesdb += "\r\n[" + i + "]: "
											+ settings.joinMessages.get(i)[0]
											+ ":" + " "
											+ settings.joinMessages.get(i)[1];
								}
								sendMassmes(sender, sender, messagesdb, 0, 0);
							}
						} catch (Exception e) {
						...
						if (!logedin(rights, sender))
							return;
						try {
							String[] info = new Regexp(message,
									"[\\s]*!add[\\s]*"
											+ "\\(\"(.*?)\"\\)[\\s]*(.*)")
									.getMatches()[0];
							if (info != null && info[0] != null
									...
									&& !info[0].matches("[\\s]*")
									&& !info[1].matches("[\\s]*")) {
								if (info[1].matches(".*%%raw%%.*")
										&& rights != UserInfo.OP) {
									sendMassmes(sender, sender,
											"für %%raw%% benötigt man op", 0);
								}
								...
								sendMassmes(
										...
										info[0]
												+ "|"
												+ info[1]
												+ ": wurde zur Nachrichtenliste hinzugefügt",
										0);
								Utilities.saveObject(messages, messageFile);
							} else {
							...
							if (info != null && info[0] != null
								...
								sendMassmes(
										...
										info[0]
												+ "|"
												+ info[1]
												+ " : wurde zur joinMessagesliste hinzugefügt",
										0);
								settings.joinMessages.add(new String[] {
										info[0], info[1] });
								Utilities.saveObject(settings, config);
							} else {
								String messagesdb = "Folgende Nachrichten sind in der joinMessages Datenbank:";
						...
						if (!logedin(sender, ch2))
							return;
						String name = new Regexp(message,
								"[\\s]*!ignore[\\s]*(.*)").getFirstMatch()
								.trim();
						if (name != null && !name.matches("[\\s]*")) {
							if (settings.ignore.contains(name)) {
								sendMassmes(sender, sender, name
										+ " wird bereichts ignoriert", 0);
							} else {
								sendMassmes(
										sender,
										sender,
										name
												+ " wird jetzt für die Nachrichtentexte ignoriert",
										0);
						...
						String name = message.replaceFirst(
								"[\\s]*!autoop[\\s]*", "");
						if (settings.autoOp.contains(name)) {
							sendMassmes(sender, sender, name
									+ " ist bereichts in der autoopliste", 0);
							return;
						}
						if (userRights(sender, ch2) != UserInfo.OP) {
							sendMassmes(sender, sender,
						...
						if (userRights(name, channel) == 0) {
							sendMassmes(
									sender,
									sender,
									"du kannst nur angemeldeten Benutzern autoop erteilen",
									0);
							return;
						}
						op(channel, name);
						settings.autoOp.add(name);
						sendMassmes(sender, sender, name + " hat jetzt autoop",
								0);
						Utilities.saveObject(settings, config);
						return;
						...
						if (settings.autoOp.contains(name)) {
							...
							Utilities.saveObject(settings, config);
							sendMassmes(sender, sender, name
									+ " wurde von autoop entfernt", 0);

						} else
							sendMassmes(sender, sender, name
									+ " ist nicht in der autoopliste", 0);
						...
						if (settings.autoVoice.contains(name)) {
							sendMassmes(sender, sender, name
									+ " ist bereichts in der autovoiceliste", 0);
							return;
						}
						if (userRights(sender, ch2) != UserInfo.OP) {
							sendMassmes(sender, sender,
						...
						if (userRights(name, channel) == 0) {
							sendMassmes(
									sender,
									sender,
									"du kannst nur angemeldeten Benutzern autovoice erteilen",
									0);
							return;
						}
						voice(channel, name);
						...
						if (settings.autoVoice.contains(name)) {
							...
							Utilities.saveObject(settings, config);
							sendMassmes(sender, sender, name
									+ " wurde von autovoice entfernt", 0);
						} else
							sendMassmes(sender, sender, name
									+ " ist nicht in der autovoiceliste", 0);
						...
						while (iter.hasNext()) {
							try {
								...
								if (strings[2] != null
										&& strings[2]
												.matches("(?is).*(rapidshare|rscomrs.com).*")) {

									if (!Utilities.rsaccValid(strings[0],
											strings[1])) {
										iter.remove();
										Utilities.saveObject(settings, config);
										continue;
						...
						try {
							if (message.matches("[\\s]*!delprem[\\s]*[\\d]+")) {
								int n = Integer.parseInt(new Regexp(message,
										"[\\s]*!delprem[\\s]*([\\d]+)")
										.getFirstMatch());
								sendMassmes(sender, sender, "der premium acc "
										+ settings.premAccs.get(n)[0]
										+ " wurde aus der liste entfernt", 0);
								settings.premAccs.remove(n);
								Utilities.saveObject(settings, config);
							} else if (message
									.matches("[\\s]*!delprem[\\s]*[^\\s].*")) {
								String name = new Regexp(message,
										"[\\s]*!delprem[\\s]*(.*)")
										.getFirstMatch().trim();
					...
					if (channel.charAt(0) == '#') {
						...
						String dateS = date.getHours() + ":"
								+ date.getMinutes() + ":" + date.getSeconds();
						try {
							Utilities.writeLocalFile(getLog(channel), "("
									+ dateS + ") " + sender + ": " + message
									+ "\r\n", true);
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					if (message.matches("[\\s]*![\\S]{0,8}to[\\S]*[\\s]*.*")) {
						...
						try {
							String name = new Regexp(message,
									"[\\s]*!whois[\\s]*(.*)").getFirstMatch();
							UserInfo user = userInfo(name);
							String adress = InetAddress.getByName(
									user.hostname.replaceFirst(".*\\@", ""))
									.getHostAddress();
							RequestInfo request = Utilities
									.postRequest(
											new URL(
											...
											null, null, null,
											"cid=0&c=0&Template=iplocator.htm&ipaddress="
													+ adress, true, 10000,
											10000);
							String[] infos = request
									.getRegexp(
											"<td align=\"right\">Country</td>.*?value=\"(.*?)\".*?<td align=\"right\">Region</td>.*?value=\"(.*?)\".*?<td align=\"right\">City</td>.*?value=\"(.*?)\".*?<td align=\"right\">TimeZone</td>.*?value=\"(.*?)\"")
									.getMatches()[0];
							String time = "";
							if (infos[0].matches("(?is)germany"))
								...
								Date dt = new Date();
								time = " es ist bei ihm ";
								String[] timem = infos[3]
										.replaceAll("[-+]", "").split(":");

								time += dt.getHours()
										+ (((infos[3].contains("-") ? -1 : 1) * Integer
												.parseInt(timem[0])) - 1)
										+ ":"
										+ (dt.getMinutes() + (((infos[3]
												.contains("-") ? -1 : 1) * Integer
												.parseInt(timem[1]))));
								try {
							...
							sendMassmes(sender, channel, name + " aus "
									+ infos[0] + " wohnt in " + infos[2] + " "
									+ infos[1] + time, 0);
						} catch (Exception e) {
							e.printStackTrace();
						...
						try {
							String name = new Regexp(message,
									"[\\s]*!wiki[\\s]*(.*)").getFirstMatch()
									.replaceAll("\\s", "_");

							RequestInfo request = Utilities.getRequest(new URL(
									"http://wiki.jdownloader.org/index.php?search="
											+ name + "&go=Seite"), "", "",
									false);

							if (request
									.containsHTML("<h1 class=\"firstHeading\">Suchergebnisse</h1>")) {
								String[] links = null;
								if (!request
										.containsHTML("<a name=\"Keine_.C3.9Cbereinstimmungen_mit_Seitentiteln\">")) {
									try {
										links = Utilities
												.getHttpLinks(
														request
																.getRegexp(
																		"Seitentiteln</span>(.*?)</ol>")
																.getFirstMatch(),
								...
								if (links == null || links.length < 1
									...
									request = Utilities
											.getRequest(
													new URL(
															"http://wiki.jdownloader.org/index.php?title=JDownloader_Wiki:Portal"),
													"", "", false);
									String html = request.getHtmlCode();
									html = html
											.substring(html
													.indexOf("showTocToggle(); } </script>"));
									html = html.substring(0, html
											.indexOf("NewPP limit report"));
									String[] cats = html
											.split("<h2> <span class=\"mw-headline\"");

									HashMap<String, Integer> map = new HashMap<String, Integer>();
									for (int i = 0; i < cats.length; i++) {
										int count = new Regexp(cats[i], name)
												.count();
										if (count > 0) {
											map
													.put(
									...
									if (map.size() > 0) {
										map = (HashMap<String, Integer>) Utilities
												.revSortByValue(map);
										sendMassmes(sender, channel, map
								...
								sendMassmes(sender, channel, request
										.getLocation(), 0);
							}
						...
						try {
							if (!settings.ignore.contains(sender.replaceAll(
									"[\\W]", "."))) {
								sendMassmes(
										sender,
										sender,
										"sie werden jetzt für die Nachrichtentexte ignoriert",
										0);
						...
						while (iter.hasNext()) {
							try {
								BotMessage botMessage = (BotMessage) iter
										.next();
								if (message2.matches("(?is).*"
										+ botMessage.regExp + ".*")) {
									...
									if (nachri.matches(".*!nouser .*")) {
										nachri = nachri.replaceFirst(
												"!nouser ", "");
										nouser = true;
									}
									if (nachri.matches(".*!delay=[\\d]+ .*")) {
										delay = Integer.parseInt(new Regexp(
												nachri, ".*!delay=([\\d]+).*")
												.getFirstMatch());
										nachri = nachri.replaceFirst(
												"!delay=[\\d]+ ", "");
									...
									if (!say && logit) {
										if (botMessage.users.contains(sender))
											return;
										User[] users = getUsers(channel);
										for (int i = 0; i < users.length; i++) {
										...
										Utilities.saveObject(messages,
												messageFile);
									}
									nachri = nachri.replaceAll("%%user%%",
											sender);
									...
									if (nouser)
										sendMassmes(sender, channel, nachri,
												delay, userDelay);
									else
										sendMassmes(sender, channel, sender
												+ ": " + nachri, delay,
	...
	public void sendMassmes(String sender, String channel, String message,
			int delayOnVoice) {
		sendMassmes(sender, channel, message, 5, delayOnVoice, 0);
	}

	public void sendMassmes(String sender, String channel, String message,
			int delayOnVoice, int delayOnUser) {
		sendMassmes(sender, channel, message, 5, delayOnVoice, delayOnUser);
	}

	public void sendMassmes(String sender, String channel, String message,
			int max, int delayOnVoice, int delayOnUser) {
		if (delayOnUser > 0) {
			delayedUserMsg = true;
			try {
	...
	public boolean logedin(int rights, String sender) {
		if (rights > 1)
			...
			sendMassmes(sender, sender,
					"du bist auf dem Server nicht angemeldet", 0);
			return false;
		}
		sendMassmes(
				sender,
				sender,
				"du hast in diesem Channel nicht das recht den Bot zu administrieren",
				0);
		return false;
	}
	...
	public ServerInfo[] getServerInfos(final String message, int waitOn,
			int from, int to, String contain) {
	...
	public UserInfo userInfo(String sender) {
		...
		ServerInfo[] info = getServerInfos("whois " + sender, 318, 310, 320,
				sender);
		UserInfo uinfo = new UserInfo();
		for (int i = 0; i < info.length; i++) {
			if (info[i].code == 311) {
				try {
					uinfo.hostname = new Regexp(info[i].response,
							".*?([\\S]\\=.*?)[\\s]*\\*[\\s]*:").getFirstMatch()
							.replaceFirst("[\\s]", "@");
				} catch (Exception e) {
					// TODO: handle exception
				...
				uinfo.channels = info[i].response.replaceFirst(
						".*?[\\s]*.*?[\\s]*:", "");
			} else if (info[i].code == 320) {
				uinfo.identified = info[i].response
						.contains("is identified to services");
			}
		}
		return uinfo;
	}
	...
	private int userRights(UserInfo userInfo, String channel) {
		...
		else if (userInfo.identified)
			return UserInfo.IDENTIFIED;
