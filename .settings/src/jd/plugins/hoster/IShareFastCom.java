...
@HostPlugin(revision = "$Revision: 15994 $", interfaceVersion = 2, names = { "isharefast.com" }, urls = { "http://(www\\.)?isharefast\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class IShareFastCom extends PluginForHost {

    private String              BRBEFORE     = "";

    private static final String PASSWORDTEXT = "(<br><b>Password:</b> <input|<br><b>Passwort:</b> <input)";

    private static final String COOKIE_HOST  = "http://isharefast.com";

    public boolean              NOPREMIUM    = false;

    public IShareFastCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    // XfileSharingProBasic Version 2.3.0.1, added dllink regexes
    @Override
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    dllink = new Regex(BRBEFORE, "type=\"button\" value=\"Download or Play\\(iPhone,iPad\\)\" onclick=\"location\\.href=\\'(http://.*?)\\'\"").getMatch(0);
                    if (dllink == null) {
                        dllink = new Regex(BRBEFORE, "(\"|\\')(http://file\\d+\\.isharefast\\.com/files/\\d+/[a-z0-9]+/.*?)(\"|\\')").getMatch(1);
                        if (dllink == null) {
