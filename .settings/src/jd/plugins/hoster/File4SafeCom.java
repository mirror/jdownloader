...
@HostPlugin(revision = "$Revision: 16016 $", interfaceVersion = 2, names = { "file4safe.com" }, urls = { "http://(www\\.)?file4safe\\.com/[a-z0-9]{12}" }, flags = { 2 })
public class File4SafeCom extends PluginForHost {
    ...
    public void checkErrors(DownloadLink theLink, boolean checkAll, String passCode, boolean loggedIn) throws NumberFormatException, PluginException {
        ...
        if (brbefore.contains(" can download files up to ") || brbefore.contains("Upgrade your account to download bigger files") || brbefore.contains("This file reached max downloads") || brbefore.contains(">Upgrade your account to download larger files") || brbefore.contains(">To download you must register")) {
    ...
    public void handlePremium(DownloadLink link, Account account) throws Exception {
        ...
        if (nopremium) {
            ...
            if (dllink == null) {
                ...
                dllink = br.getRedirectLocation();
                if (dllink == null) {
                    checkErrors(link, true, passCode, true);
                    dllink = getDllink();
                }
            }
            if (dllink == null) {
