...
@HostPlugin(revision = "$Revision: 16650 $", interfaceVersion = 2, names = { "orangefiles.me" }, urls = { "https?://(www\\.)?orangefiles\\.me/[a-z0-9]{12}" }, flags = { 0 })
public class OrangeFilesMe extends PluginForHost {
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws Exception {
        ...
        // size isn't on the first page
        if (correctedBR.contains("\"download1\"")) {
            postPage(br.getURL(), "op=download1&usr_login=&id=" + new Regex(link.getDownloadURL(), "/([A-Za-z0-9]{12})$").getMatch(0) + "&fname=" + Encoding.urlEncode(filename) + "&referer=&method_free=Free+Download");
        }
