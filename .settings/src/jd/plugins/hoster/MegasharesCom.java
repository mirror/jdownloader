 ...
 * einbauen
 */
@HostPlugin(revision = "$Revision: 16932 $", interfaceVersion = 2, names = { "megashares.com" }, urls = { "http://[\\w\\.]*?(d[0-9]{2}\\.)?megashares\\.com/(.*\\?d[0-9]{2}=[0-9a-zA-Z]{7}|dl/[0-9a-zA-Z]{7}/)" }, flags = { 2 })
public class MegasharesCom extends PluginForHost {

    private final String                   UserAgent   = "JD_" + "$Revision: 16932 $";
    ...
    private void handleErrors(DownloadLink link) throws PluginException {
        ...
        if (br.containsHTML("You have reached.*?maximum download limit")) {
            ...
            if (wait > 65 * 60 * 1000l) {
                /* user report that this is enough waittime to get new ticket */
                wait = 65 * 60 * 1000l;
            }
