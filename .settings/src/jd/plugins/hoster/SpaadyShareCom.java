...
@HostPlugin(revision = "$Revision: 15422 $", interfaceVersion = 2, names = { "spaadyshare.com" }, urls = { "http://(www\\.)?spaadyshare\\.com/[A-Za-z0-9]{12}/.+" }, flags = { 0 })
public class SpaadyShareCom extends PluginForHost {
    ...
    private static final String COOKIE_HOST   = "http://spaadyshare.com";

    public boolean              NOPREMIUM     = false;

    public SpaadyShareCom(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks) throws Exception, PluginException {
        ...
        if (BRBEFORE.contains("\"download1\"")) {
            br.postPage(downloadLink.getDownloadURL(), "op=download1&usr_login=&id=" + new Regex(downloadLink.getDownloadURL(), COOKIE_HOST.replace("http://", "") + "/" + "([A-Za-z0-9]{12})").getMatch(0) + "&fname=" + Encoding.urlEncode(downloadLink.getName()) + "&referer=&method_free=Free+Download");
            doSomething();
        }
    ...
    // XfileSharingProBasic Version 2.1.1.4, modified for big A-Z in regexes,
    // added 1 extra filesize regex
    @Override
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        String filename = new Regex(BRBEFORE, "You have requested.*?http://.*?[A-Za-z0-9]{12}/(.*?)</font>").getMatch(0);
        if (filename == null) {
        ...
        if (filesize == null) {
            ...
            if (filesize == null) {
                ...
                if (filesize == null) {
                    filesize = new Regex(BRBEFORE, "</span>File Size: (.*?)<br>").getMatch(0);
                }
            }
        }
