...
@HostPlugin(revision = "$Revision: 15422 $", interfaceVersion = 2, names = { "hostfil.es" }, urls = { "http://(www\\.)?hostfil\\.es/((\\?d|download\\.php\\?id)=[A-Z0-9]+|((en|ru|fr|es)/)?file/[0-9]+/)" }, flags = { 0 })
public class HostFilEs extends PluginForHost {
    ...
    public HostFilEs(PluginWrapper wrapper) {
        super(wrapper);
    ...
    public AvailableStatus requestFileInformation(DownloadLink parameter) throws Exception {
        ...
        if (filesize == null) {
            ...
            if (filesize == null) {
                filesize = br.getRegex("<td><b>File size:</b></td>[\t\n\r ]+<td>(.*?)</td>").getMatch(0);
            }
        }
