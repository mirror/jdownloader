...
@HostPlugin(revision = "$Revision: 15994 $", interfaceVersion = 2, names = { "netuploaded.com" }, urls = { "http://(www\\.)?netuploaded\\.com/[a-z0-9]{12}" }, flags = { 2 })
public class NetUploadedCom extends PluginForHost {
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks) throws Exception, PluginException {
        ...
        if (dllink == null) {
            ...
            if (dlForm == null) {
                if (br.containsHTML("name=\"down_direct\" value=\"1\"")) throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error");
    ...
    // XfileSharingProBasic Version 2.3.0.0, added 1 dllink regex, added extra
    // errorhandling
    @Override
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    ...
                    if (dllink == null) {
                        if (dllink == null) dllink = new Regex(BRBEFORE, "\\'(http://(www\\.)?\\d+\\.netuploaded\\.com/files/.*?)\\'").getMatch(0);
                        if (dllink == null) {
