...
@DecrypterPlugin(revision = "$Revision: 14979 $", interfaceVersion = 2, names = { "wupload.com" }, urls = { "http://(www\\.)?wupload\\..*?/folder/[0-9a-z]+" }, flags = { 0 })
public class WUploadComFolder extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        if (links == null || links.length == 0) links = br.getRegex("\"(http://(www\\.)?wupload\\.[A-Za-z\\.]+/file/\\d+/.*?)\"").getColumn(0);
        String[] folders = br.getRegex("\"(http://(www\\.)?wupload\\.[A-Za-z\\.]+/folder/[0-9a-z]+)\"").getColumn(0);
