...
public class PixelGrid {
	...
	public void setPixelValue(int x, int y, int value) {
		try {
			grid[x][y] = Color.HSBtoRGB((float) 0.0, (float) 0.0,
					(float) ((double) value / owner.getColorValueFaktor()));
		} catch (ArrayIndexOutOfBoundsException e) {
			logger.severe("Nicht im grid; [" + x + "][" + y
					+ "] grid: " + getWidth() + "/" + getHeight());
			e.printStackTrace();
	 ...
	 * Dass die Parameter gleich bleiben sollte amn immerschauen das sman sich
	 * in einem wertebereich von 0-0xffffff bewegt. Je nach captchatyp kanne s
	 * erforderlich sein den farbraum anzupassen
	 * 
	 * @param x
	 * @param y
	 */
	public int getPixelValue(int x, int y) {
		...
		try {
			//UTILITIES.trace(grid[x][y]);
			//return grid[x][y];
			Color c = new Color(grid[x][y]);
		//c.RGBtoHSB(r, g, b, hsbvals)
			float[] hsb= new float[3];
			Color.RGBtoHSB(c.getRed(), c.getGreen(), c
					.getBlue(),hsb);
			...
			for (int y = 0; y < newHeight; y++) {
				...
				for (int gx = 0; gx < faktor; gx++) {
					for (int gy = 0; gy < faktor; gy++) {
						int newX = x * faktor + gx;
						int newY = y * faktor + gy;
						setPixelValue(newX, newY, newGrid, isElement((int) v,
								(int) (avg * contrast)) ? 0
	...
	public void cleanBackgroundBySample(int px, int py, int width, int height) {
		...
		for (int x = 0; x < getWidth(); x++) {
			for (int y = 0; y < getHeight(); y++) {
				...
				if (dif < (getMaxPixelValue() * owner
						.getBackgroundSampleCleanContrast())) {
...
if(getWidth()<=0 ||getHeight()<=0){
	...
	public void invert() {
		for (int y = 0; y < getHeight(); y++) {
			for (int x = 0; x < getWidth(); x++) {
				setPixelValue(x, y, (int) getMaxPixelValue()
						- getPixelValue(x, y));
			}
		}
	}
